...
class REFPROPInstance(object):
    ...
    def __init__(self, dll, managed_handle = None):
        ...
        self._FNCRPTdlloutput_tuple = namedtuple('FNCRPTdlloutput',["D","P","T","ierr","herr"])
        ...
        self._FNCRPTdll = self._getfcn(self.dll, 'FNCRPTdll')
    ...
    def FNCRPTdll(self,z):
        """
        D,P,T,ierr,herr = FNCRPTdll(z)
        """
        z = (len(z)*ct.c_double)(*z)
        D = ct.c_double()
        P = ct.c_double()
        ...
        if self._FNCRPTdll is None: raise ValueError("The function FNCRPTdll could not be loaded from the shared library.") 
        self._FNCRPTdll(z,ct.byref(D),ct.byref(P),ct.byref(T),ct.byref(ierr),herr,255)

        return self._FNCRPTdlloutput_tuple(D.value,P.value,T.value,ierr.value,trim(herr.raw))

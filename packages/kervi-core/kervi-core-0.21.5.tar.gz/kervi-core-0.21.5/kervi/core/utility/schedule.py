...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __init__(self, interval, scheduler, job_func):
        ...
        self.job_func = job_func  # the job job_func to run
        self.job_args = []
        self.job_keywords = None
        self.unit = None  # time units, e.g. 'minutes', 'hours', ...
        self.at_time = None  # optional time at which this job runs
        self.to_time = None  # optional time at which this job should stop
        self.to_delta = None
    ...
    def __repr__(self):
        ...
        args = [repr(x) if is_repr(x) else str(x) for x in self.job_args]
        kwargs = ['%s=%s' % (k, repr(v))
                  for k, v in self.job_keywords.items()]
    ...
    def until(self, time_str):
        """
        Specify a particular time that the job should terminate.
        :param time_str: A string in one of the following formats: `HH:MM:SS`,
            `HH:MM`,`:MM`, `:SS`. The format must make sense given how often
            the job is repeating; for example, a job that repeats every minute
            should not be given a string in the form `HH:MM:SS`. The difference
        ...
        minute = int(minute)
        second = int(second)
        self.to_time = datetime.time(hour, minute, second)
        if self.to_time < self.at_time:
            if self.unit == 'days' or self.start_day:
                self.to_time = datetime.datetime(1, hour, minute, second)
            elif self.unit == 'hours':
                self.to_time = datetime.time(1, minute, second)
            elif self.unit == 'minutes':
                self.to_time = datetime.time(0, 1, second)

        dateTimeAt = datetime.datetime.combine(datetime.date.today(), self.at_time)
        dateTimeTo = datetime.datetime.combine(datetime.date.today(), self.to_time)
        self.to_delta = dateTimeTo - dateTimeAt
        return self
    ...
    def _run(self):
        return self.job_func(*self.job_args, **self.job_keywords)
...
_sc= _ScheduleThread(default_scheduler)
_sc.start()
...
class BuildImage(object):
    ...
    def _apply_patches(self):
        ...
        ins_cfg = self._config.instance_config
        main_repo = [repo for repo in ins_cfg.get('repositories', []) if repo.get('main_repo')]
        if not main_repo:
            _logger.error('Main repo not specified in the json config, skipping patches')
            return res
        patches_path = path.join(self.instance_path, main_repo[0].get('path'), 'patches.txt')
        if not path.exists(patches_path):
            return res
        with open(patches_path, 'r') as obj:
            patches = obj.read()
        if not patches:
            return res
        ...
        for patch in patches:
            if not patch.strip():
                _logger.debug('Empty line in the patches file, skipping')
                continue

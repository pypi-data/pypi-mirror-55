...
def on_init_voi():
    ...
    if size is not None:
        ...
        voi_center = np.array([vmi.imCenter(patient_image)[0], original_target[1], original_target[2]])
        voi_size = np.array([vmi.imSize(patient_image)[0], size, size])
        init_voi()
...
def init_voi():
    ...
    global voi_image
    voi_image = vmi.imReslice(patient_image, voi_cs, voi_size, -1024)
...
def init_guide():
    ...
    pd = vmi.pdSmoothWindowedSinc(pd, 20)
    aceta_prop.clone(pd)
...
def LeftButtonPressRelease(**kwargs):
    global patient_image, spcut_radius, pelvis_cs, cup_radius, cup_cs, cup_RA, cup_RI, hole_radius
    global xray_image
    if kwargs['picked'] is dicom_box:
        patient_image = read_dicom()
        if patient_image is not None:
            original_slice.clone(patient_image)
            original_slice.setSlicePlaneOrigin_Center()
            original_slice.setSlicePlane_Coronal()
...
if __name__ == '__main__':
    ...
    patient_name = str()
    patient_image = vtk.vtkImageData()

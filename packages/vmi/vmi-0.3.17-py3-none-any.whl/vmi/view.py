...
class View(QGLWidget, Menu, Mouse):
    ...
    def mouseExec(self, stack, button, event, picked=None, **kwargs):
        if picked is None:
            if self._PView:
                picked = self._PView
            if self._PProp:
        ...
        for m in [m for m in stack if isinstance(m, Mouse)]:
            for mbe in m.mouse[button][event]:
    ...
    def pickPt_FocalPlane(self, pos: Union[QPoint, List[int]] = None) -> np.ndarray:
        """
        拾取鼠标指针在焦平面上的点
    ...
    def pickVt_FocalPlane(self) -> np.ndarray:
        ...
        :return: [dx, dy, dz]
        """
        pt = np.array(self.pickFPlane(self._MousePos))
        ptLast = np.array(self.pickFPlane(self._MousePosLast))
        vt = pt - ptLast
        return vt
    ...
    def cameraCS(self):
        cs = vmi.CS4x4()
        cs.origin(self.pickPt_FocalPlane(QPoint(0, 0)))
        cs.axis(0, self.cameraVt_Right())
        cs.axis(1, -self.cameraVt_Up())
        cs.axis(2, np.array(self.cameraVt_Look()))
        return cs

    def setCamera_FPlane(self, cs: vmi.CS4x4, height: float):
    ...
    def setCamera_Focal(self, focal=Union[np.ndarray, List[float]]):
    ...
    def updateInTime(self):
        """及时刷新视图"""
        for v in vmi.appViews:
            v._UpdateAtOnce = True
...
copyreg.pickle(View, pickle_View)

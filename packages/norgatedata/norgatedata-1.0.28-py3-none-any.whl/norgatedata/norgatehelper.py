...
def create_pandas_dataframe(symbol,r,pandas_dataframe=None,lowercase_columns=False):
    ...
    pdf.tz_localize("UTC",copy=False)
    pdf.name = symbol
...
def decode_date(content,format='iso'):
    """
    Turns dates into nicer Python dates
    """
    if len(content) == 0:
        return None
    if (type(content) == bytes):
        date = bytes.decode(content)
    else:
        date = content
    if date == 'now':
    ...
    try:
        ...
        if format == 'numpy-datetime64':
            return np.datetime64(date, tz='UTC')
    except:
        logger.warn("Invalid date or format provided for conversion " + str(date) +  " into format " + format)
        return None

...
def get_task_definition(config):
    ...
    if "EXECUTION_MODE" in config and "EXECUTION_MODE" == "FARGATE":
        task["logConfiguration"]["options"]["awslogs-stream-prefix"] = config['FAMILY']
        task["networkMode"] = "awsvpc"
        task["requiresCompatibilities"] =  ["FARGATE"]
    return [task]

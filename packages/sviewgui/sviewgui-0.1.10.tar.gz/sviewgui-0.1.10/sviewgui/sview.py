...
class Csviwer(Qw.QMainWindow):
    ...
    def graphDraw_Numeric_Syntax(self):
        ...
        if self.color_var == "None_": # simple plot (scatter)
            ...
            # ----------------------------
            isNumeric = True
            for comp in data[self.subcolor_var]:
                if isinstance(comp,str):
                    isNumeric = False
                    break
            if self.ui.cmb_subcolor.currentText() in self.NUMERIC_HEADER_LIST:
                if self.PLOT_TYPE == SCATTER:
                    clb = plt.colorbar(self.ax1.scatter(data[self.x_var].replace([np.inf, -np.inf],np.nan), data[self.y_var].replace([np.inf, -np.inf], np.nan), c = self.data[self.subcolor_var], cmap = "cmo."+ cmap, s = marker_size, alpha = alpha),ax=self.ax1)
            ...
            # ----------------------------
            isNumeric = True
            for comp in data[self.subsubcolor_var]:
                if isinstance(comp,str):
                    isNumeric = False
                    break
            if self.ui.cmb_subsubcolor.currentText() in self.NUMERIC_HEADER_LIST:
                if self.PLOT_TYPE == SCATTER:
                    clb = plt.colorbar(self.ax1.scatter(data2[self.x_var].replace([np.inf, -np.inf],np.nan), data2[self.y_var].replace([np.inf, -np.inf], np.nan), c = self.csv[self.subsubcolor_var], cmap = "cmo."+ cmap, s = marker_size, alpha = alpha),ax=self.ax1)

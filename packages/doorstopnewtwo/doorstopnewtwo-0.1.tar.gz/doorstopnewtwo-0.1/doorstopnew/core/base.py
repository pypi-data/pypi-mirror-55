...
class BaseFileObject(metaclass=abc.ABCMeta):
    ...
    def _load(text, path, **kwargs):
        ...
        '''
        Check for file extension and load library according to the file.
        If file extension is .yml load the yaml library load_yaml()
        else load the markdown library load_markdown()
        '''
        
        if ".yml" in path:
            return common.load_yaml(text, path, **kwargs)
        else:
            return common.load_markdown(text, path, **kwargs)

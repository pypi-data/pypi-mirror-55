...
from doorstopnew import common, settings
from doorstopnew.common import DoorstopError, DoorstopInfo, DoorstopWarning
from doorstopnew.core import editor
from doorstopnew.core.base import (
    BaseFileObject,
    BaseValidatable,
    add_item,
    auto_load,
...
from doorstopnew.core.types import UID, Level, Prefix, Stamp, Text, to_bool
...
class Item(BaseValidatable, BaseFileObject):  # pylint: disable=R0902
    ...
    def __init__(self, document, path, root=os.getcwd(), **kwargs):
        ...
        '''
        Commenting out defalut text value for markdown file
        '''
    ...
    def new(
        ...
        ''' For YML use EXTENSIONS[0] and For Markdown use EXTENSIONS[2] '''
        #filename = str(uid) + Item.EXTENSIONS[0]
        filename = str(uid) + Item.EXTENSIONS[2]
    ...
    def load(self, reload=False):
        ...
        try:
            ...
            log.debug("Set attribute values")

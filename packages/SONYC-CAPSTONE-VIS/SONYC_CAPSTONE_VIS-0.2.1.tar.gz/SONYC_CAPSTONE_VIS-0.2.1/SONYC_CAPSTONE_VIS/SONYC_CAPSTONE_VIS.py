...
class vis:
    ...
    def radar_plot(features, figsize=(15,15), y_ticks = [58,64,70]):
        '''
        this function is used for dense plot
        args: features -- pandas DataFrame
            figsize -- figure size to set
    ...
    def cluster_vis(features, PCA=True, n_components = 64):
...
class fetch_data:
    ...
    def time_stamp_to_datetime(sample_ind):
        timestamp_df = pd.DataFrame.from_dict(sample_ind, orient='index')
    ...
    def fetch_feature_spl_match(features = None, sensors = None, time_stamps = None):
        spl_root_path = '/beegfs/work/sonyc/new_indices/2017/sonycnode-'
        fix = '.sonyc_recording_index.h5'
        res = []
        if features:
            sensors = features.sensor_id.unique()
            time_stamps = features.timestamp.unique()
    ...
    def fetch_fulll_annotation(file_path='/beegfs/work/sonyc/ust/features/l3-mel256-music-512/44.1k'):

...
class TestsParserDesc(BaseParserDesc):
...
class DocsParserDesc:
    description = 'docs command help'
...
class InstallParserDesc:
    ...
    def add_arguments(parser):
        ...
        docs_group.add_argument(
            '--docs-library-dir', '--docs_library_dir', action='store',
            default='',
            help=(
        ...
        docs_group.add_argument(
            '--docs-output-dir', '--docs_output_dir', action='store',
            default='',
            help='choose folder in which to output generated docs')
...
def top_parser():
    ...
    parser.add_argument(
        '--coverage',
        action='store', default=None,
        help='enable code coverage computation')
    ...
    add_subparser(subparsers, 'install', InstallParserDesc)
    add_subparser(subparsers, 'uninstall', UninstallParserDesc)
    add_subparser(subparsers, 'tests', TestsParserDesc)
    ...
    add_subparser(subparsers, 'spyder', ExternParserDesc('spyder'))
    add_subparser(subparsers, 'ipython', ExternParserDesc('ipython'))
    add_subparser(subparsers, 'nosetests', ExternParserDesc('nosetests'))
    add_subparser(subparsers, 'pyflakes', ExternParserDesc('pyflakes'))
    add_subparser(subparsers, 'pylint', ExternParserDesc('pylint'))
...
def run_install(parsed):
    ...
    def generate_docs():
        ...
        oss.Popen_no_console([
            sys.executable, __file__, 'sy', '-L5', '--generate-docs',
            '--docs-library-dir', parsed.docs_library_dir,
            '--docs-output-dir', parsed.docs_output_dir]).communicate()
    ...
    def register_linux():
        ...
        oss.create_desktop_entry()
        print('Register file association (syx).')
        oss.create_mime_type_association()
    ...
    install_all = parsed.all or not any([
        ...
        parsed.compile_all,
        parsed.register,
        parsed.generate_docs,
        parsed.set_preference,
    ])
    ...
    if install_all or parsed.generate_docs:
        generate_docs()
...
def run_sympathy(parsed):
    ...
    platform_args = ([sy_launch.replace('.pyc', '.py'),
                      'launch',
...
def run(app=None):
    ...
    internal_choices = [
        'cli', 'gui', 'viewer', 'install', 'uninstall', 'clear', 'launch']
    ...
    if parsed.coverage:
        ...
        coverage.prepare_coverage(parsed.coverage)
        coverage_tester = coverage.CoverageTester()
        coverage_tester.start_coverage()
        # TODO(erik): Improve argument handling for coverage.  The following
        # lines forcefully clean up coverage arguments to avoid issues parsing
        # the args passed to launch sub-processes.
        coverage_index = sys.argv.index('--coverage')
        sys.argv[coverage_index: coverage_index + 2] = []

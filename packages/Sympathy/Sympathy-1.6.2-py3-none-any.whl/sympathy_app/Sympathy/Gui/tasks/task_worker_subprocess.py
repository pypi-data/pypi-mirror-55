...
from Gui.tasks.task_worker_subprocess import debug_worker, _debug_worker_global
...
def _warn_for_unsupported_qt(unsupported_qt):
    if unsupported_qt is None:
        # Since we support having both PySide2 and PyQt5 in the same python
        # environment, for spyder support. We must ensure that PyQt5 is not
        # loaded except for use with spyder, since this may cause all sorts of
        # problems.

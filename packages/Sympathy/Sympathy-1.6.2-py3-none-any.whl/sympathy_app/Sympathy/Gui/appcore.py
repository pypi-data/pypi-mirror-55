...
class AppCore(QtCore.QObject):
    ...
    abort_node_requested = QtCore.Signal(str)
    ...
    node_error_message = QtCore.Signal(str, str)
    all_execution_has_finished = QtCore.Signal()
    node_output_received = QtCore.Signal(str, node_result.NodeResult)
    node_output_stream = QtCore.Signal(int, str, str, str)
    output_message = QtCore.Signal(str)
    node_progress = QtCore.Signal(str, float)
    ...
    def message_input(self, taskid, msg):
        if msg.type == msgmod.StdoutMessage:
            kind = 'Notice'
            ident, text = msg.data
            self.node_output_stream.emit(taskid, ident, kind, text)
    ...
    def remove_flow(self, flow_):
        ...
        if (settings.instance().session_temp_files ==
    ...
    @QtCore.Slot(str, node_result.NodeResult)
    ...
    @QtCore.Slot(str, str, node_result.NodeResult, bool)
    ...
    @QtCore.Slot(str, node_result.NodeResult)
    def execute_node_parameter_view_done(self, full_uuid, result):
        ...
        try:
            ...
            # The flow containing the node must have closed.
            return

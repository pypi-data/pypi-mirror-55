...
def packages():
    ...
    required_file = os.path.join(
        Gui.settings.instance()['install_folder'], 'Package', 'requires.txt')
...
class PythonInfo(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._sympathy_version = QtWidgets.QLineEdit(Gui.version.version)
...
class IssueReport(QtWidgets.QWidget):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'python': python_dict,
            'python_hash': Gui.user_statistics.basic_hash(python_dict),
            'python_packages': python_packages,
            'python_packages_hash': Gui.user_statistics.basic_hash(
                python_packages),
            'system': system_dict,
            'system_hash': Gui.user_statistics.basic_hash(system_dict)
        }
...
class IssueReportSender(QtWidgets.QDialog):
    ...
    def _send_issue_report(self):
        ...
        self._sender = SenderThread(
            f'{Gui.user_statistics.api_url()}/issue/',
            headers=Gui.user_statistics.api_headers(),
            data=res,
            timeout=2.0)

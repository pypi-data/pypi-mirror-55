...
class NodeTaskManager(TaskManager):
    node_is_queued = QtCore.Signal(str)
    node_execution_started = QtCore.Signal(str)
    execute_node_done = QtCore.Signal(str, node_result.NodeResult)
    execute_child_node_done = QtCore.Signal(str, str,
                                            node_result.NodeResult, bool)
    port_done = QtCore.Signal(flow.Port)
    node_is_aborting = QtCore.Signal(str, list)
    node_has_aborted = QtCore.Signal(str, list)
    node_progress_changed = QtCore.Signal(str, float)
    child_node_progress_changed = QtCore.Signal(
        str, str, float)
    node_output = QtCore.Signal(str, node_result.NodeResult)
    ...
    def handle_message(self, message):
        ...
        if cmd == task_worker2.DONE_TASK:
            ...
            if child_results:
                ...
                self.execute_node_done.emit(full_uuid, result)
...
class ValidateTaskManager(BaseTaskManager):
    validate_node_done = QtCore.Signal(str, node_result.NodeResult)
    node_output = QtCore.Signal(str, node_result.NodeResult)

    def _handle_task_done(self, taskid, full_uuid, task, result):
        if full_uuid:
            self.validate_node_done.emit(full_uuid, result)
            self.node_output.emit(full_uuid, result)
...
class ExeCore(QtCore.QObject):
    ...
    execute_child_node_done = QtCore.Signal(
        str, str, node_result.NodeResult, bool)
    validate_node_done = QtCore.Signal(str, node_result.NodeResult)
    node_progress_changed = QtCore.Signal(str, float)
    child_node_progress_changed = QtCore.Signal(
        str, str, float)
    # node_status_changed = QtCore.Signal(str, str)
    message_output = QtCore.Signal(int, Message)
    ...
    node_has_aborted = QtCore.Signal(str, list)
    node_is_aborting = QtCore.Signal(str, list)
    node_allowed_to_execute_request = QtCore.Signal(str)
    node_output = QtCore.Signal(str, node_result.NodeResult)
    ...
    def _connect(self):
        ...
        self._node_task_manager.node_output.connect(
            self.node_output)
        self._port_viewer_task_manager.node_output.connect(
            self.node_output)
        ...
        tm.execute_node_parameter_view_done[
            str, node_result.NodeResult].connect(

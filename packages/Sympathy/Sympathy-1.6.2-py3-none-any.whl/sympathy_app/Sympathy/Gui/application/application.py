...
def connect_app_and_exe(app_core, exe_core):
    ...
    app_core.abort_node_requested[str].connect(
        exe_core.abort_node)
    ...
    exe_core.execute_node_done[str, node_result.NodeResult].connect(
        app_core.execute_node_done)
    exe_core.execute_child_node_done[
        str, str, node_result.NodeResult, bool].connect(
            app_core.execute_child_node_done)
    exe_core.node_output[str, node_result.NodeResult].connect(
        app_core.node_output)
    exe_core.node_has_aborted[str, list].connect(
        app_core.node_has_aborted)
    exe_core.node_is_aborting[str, list].connect(
        app_core.set_node_is_aborting)
    exe_core.validate_node_done[str, node_result.NodeResult].connect(
        app_core.validate_node_done)
    exe_core.node_progress_changed[str, float].connect(
        app_core.update_node_progress)
    exe_core.child_node_progress_changed[
        str, str, float].connect(
    ...
    exe_core.node_is_queued[str].connect(
        app_core.set_node_status_queued)
    exe_core.node_execution_started[str].connect(
...
def create_gui(args, sys_args):
    ...
    gui_manager.GuiManager(main_window_, app_core, parent=app)
    main_window_.show()
    main_window_.raise_()
...
def start_cli_application(args, sys_args):
    ...
    polling_timer = user_statistics.create_polling_timer()
    polling_timer.start()

...
class SubflowView(BaseNodeView):
    ...
    def contextMenuEvent(self, event):
        ...
        if self._ports_menu:
            for ports, menu, ports_menu, basefunc in [
                    (input_ports, self._input_menu,
                     self._input_menu, self._create_input),
                    (output_ports, self._output_menu,
    ...
    def _handle_save_subflow_as_node(self):
        dialog = flowinfo.SaveSubflowAsNodeWizard(self._model)

        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            flow_info = self._model.get_flow_info()
            flow_info['identifier'] = dialog.details()['identifier']
            flow_info['tag'] = dialog.details()['tag']

            library_path = os.path.join(
                dialog.library()['root_path'], 'Library')

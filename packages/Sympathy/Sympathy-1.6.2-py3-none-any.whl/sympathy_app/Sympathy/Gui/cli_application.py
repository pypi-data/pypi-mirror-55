...
class Application(QtCore.QObject):
    ...
    def _connect(self):
        ...
        self._app_core.node_output_stream.connect(
            self.print_node_output_stream_message)
    ...
    def run(self):
        if self._args.generate_docs:
            self._app_core.reload_documentation(
                library=self._args.docs_library_dir or None,
                output_folder=self._args.docs_output_dir or None)
    ...
    def print_node_output(self, full_uuid, output, category='OUTPUT'):
        timestring = datetime.datetime.isoformat(datetime.datetime.today())

        if output.has_error():
            self._last_output = None
            self._error = True
            key, formatted_message = common.format_output_exception(output)
            category = key.upper()
            if formatted_message:
                formatted_message = common.RED(formatted_message)
                formatted_message = '{} {} {}\n{}'.format(
    ...
    def print_node_output_stream_message(self, taskid, ident, kind, text):
        ...
        if use_banner and kwargs:
            stream_result = StreamNodeResult(stderr=text)
            self.print_node_output(ident, stream_result, 'NOTICE')
        else:

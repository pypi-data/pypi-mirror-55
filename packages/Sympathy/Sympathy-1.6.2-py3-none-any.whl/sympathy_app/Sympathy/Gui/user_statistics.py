...
def create_polling_timer():
    from Qt import QtCore
    res = QtCore.QTimer()
    res.setInterval(1000 * timer_poll_interval)
    res.timeout.connect(user_running_sympathy)
    return res
...
class ThreadPoster(object):
    ...
    def __init__(self, url, headers, ignore_ssl_errors=None):
        ...
        self._exit_marker = queue.Empty()
        self._in_exit = threading.Event()
    ...
    def _loop(self):
        ...
        while True:
            ...
            if item is self._exit_marker:
                self._in_exit.set()
                return

            data = json.dumps(item, cls=DictEncoder)
            ...
            try:
                res = requests.post(self._url, **kwargs)
    ...
    def send_and_exit(self, data):
        self.send(data)
        try:
            self._queue.put_nowait(self._exit_marker)
        except Exception:
            pass
        else:
            self._in_exit.wait()
...
class StandardCollector(StatsCollector):
...
class StandardSender(StatsSender):
    send_interval = standard_sender_interval

    def __init__(self, collector):
        super().__init__(collector)
        self._poster = ThreadPoster(
            f'{api_url()}/stat/',
            headers=api_headers())
    ...
    def send(self):
        messages = self._collector.pop_messages()
...
_standard_collector = StandardCollector()
collectors = [_standard_collector]
senders = [StandardSender(_standard_collector)]

_send_timer = time.time()

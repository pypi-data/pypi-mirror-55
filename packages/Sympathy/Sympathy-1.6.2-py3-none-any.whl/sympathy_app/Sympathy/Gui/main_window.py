...
from sympathy.utils.prim import nativepath, uri_to_path
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _init_error_view(self):
        ...
        self._app_core.node_output_stream.connect(
            self._error_view.add_node_output_stream_message)
    ...
    def open_documentation(self, docs_section):
        ...
        or one of the special values 'index' or 'node_library'.
        """
        ...
        docs_root = [os.environ['SY_APPLICATION_DIR'], '..', 'Docs']
        doc_path = []
        docs = {
            'index': 'index.html',
        ...
        if docs_section in docs:
            ...
            source_file = uri_to_path(library_node.source_uri)
            ...
            path_in_library = os.path.dirname(os.path.relpath(
                source_file,
                uri_to_path(library_node.library)))
            doc_path = (['src', 'Library'] + path_in_library.split('/') +
                        [page_name + '.html'])

            if library_node.library_id not in [
                    'org.sysess.builtin', 'org.sysess.sympathy']:
                docs_root = [uri_to_path(library_node.library), '..', 'Docs']

...
from sympathy.utils.prim import uri_to_path, localuri
...
class UnlinkSubflowCommand(UndoCommandBase,
    ...
    def __init__(self, subflow, **kwargs):
        ...
        linked_info = self._linked_subflow.get_flow_info()
    ...
    def redo(self):
        if self._unlinked_subflow is None:
            ...
            subflow.set_flow_info(self._unlinked_info)
...
class LinkSubflowToLibraryCommand(UndoCommandBase):
    ...
    def __init__(self, subflow, flow_info, filename, library_path,
        ...
        self._set_flow_info_cmd = SetFlowInfo(self._subflow, flow_info)
    ...
    def redo(self):
        ...
        if self._first_run:
            try:
                ...
                saved = Gui.common.save_flow_to_file(self._subflow, filename)
                if not saved:
                    raise Exception('Failed to save library flow')

                flow_data = Gui.library_creator.get_flow_data(filename)
            ...
            self._library_node = library_node
            self._subflow.library_node = library_node
            ...
            self._set_flow_info_cmd.redo()
            self._subflow._library_node = self._library_node
            if self._link_subflow_cmd:
                self._link_subflow_cmd.redo()
            self._subflow._library_node = self._library_node

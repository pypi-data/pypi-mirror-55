...
class Client(BaseClient):
    ...
    def cancel_key_deletion(
        ...
        For more information about scheduling and canceling deletion of a CMK, see `Deleting Customer Master Keys <https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html>`__ in the *AWS Key Management Service Developer Guide* .

        The result of this operation varies with the key state of the CMK. For details, see `How Key State Affects Use of a Customer Master Key <https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def connect_custom_key_store(
        ...
        """
        Connects or reconnects a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ to its associated AWS CloudHSM cluster.
        ...
        To connect a custom key store, its associated AWS CloudHSM cluster must have at least one active HSM. To get the number of active HSMs in a cluster, use the `DescribeClusters <https://docs.aws.amazon.com/cloudhsm/latest/APIReference/API_DescribeClusters.html>`__ operation. To add HSMs to the cluster, use the `CreateHsm <https://docs.aws.amazon.com/cloudhsm/latest/APIReference/API_CreateHsm.html>`__ operation.

        The connection process can take an extended amount of time to complete; up to 20 minutes. This operation starts the connection process, but it does not wait for it to complete. When it succeeds, this operation quickly returns an HTTP 200 response and a JSON object with no properties. However, this response does not indicate that the custom key store is connected. To get the connection state of the custom key store, use the  DescribeCustomKeyStores operation.

        During the connection process, AWS KMS finds the AWS CloudHSM cluster that is associated with the custom key store, creates the connection infrastructure, connects to the cluster, logs into the AWS CloudHSM client as the ` ``kmsuser`` crypto user <https://docs.aws.amazon.com/kms/latest/developerguide/key-store-concepts.html#concept-kmsuser>`__ (CU), and rotates its password.

        The ``ConnectCustomKeyStore`` operation might fail for various reasons. To find the reason, use the  DescribeCustomKeyStores operation and see the ``ConnectionErrorCode`` in the response. For help interpreting the ``ConnectionErrorCode`` , see  CustomKeyStoresListEntry .

        To fix the failure, use the  DisconnectCustomKeyStore operation to disconnect the custom key store, correct the error, use the  UpdateCustomKeyStore operation if necessary, and then use ``ConnectCustomKeyStore`` again.

        If you are having trouble connecting or disconnecting a custom key store, see `Troubleshooting a Custom Key Store <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def create_custom_key_store(
        ...
        """
        Creates a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ that is associated with an `AWS CloudHSM cluster <https://docs.aws.amazon.com/cloudhsm/latest/userguide/clusters.html>`__ that you own and manage.

        This operation is part of the `Custom Key Store feature <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ feature in AWS KMS, which combines the convenience and extensive integration of AWS KMS with the isolation and control of a single-tenant key store.

        Before you create the custom key store, you must assemble the required elements, including an AWS CloudHSM cluster that fulfills the requirements for a custom key store. For details about the required elements, see `Assemble the Prerequisites <https://docs.aws.amazon.com/kms/latest/developerguide/create-keystore.html#before-keystore>`__ in the *AWS Key Management Service Developer Guide* .
        ...
        For help with failures, see `Troubleshooting a Custom Key Store <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html>`__ in the *AWS Key Management Service Developer Guide* .
        ...
        **Request Syntax**
        ...
        :param CloudHsmClusterId: **[REQUIRED]**
          ...
          Identifies the AWS CloudHSM cluster for the custom key store. Enter the cluster ID of any active AWS CloudHSM cluster that is not already associated with a custom key store. To find the cluster ID, use the `DescribeClusters <https://docs.aws.amazon.com/cloudhsm/latest/APIReference/API_DescribeClusters.html>`__ operation.

        :type TrustAnchorCertificate: string
        :param TrustAnchorCertificate: **[REQUIRED]**

          Enter the content of the trust anchor certificate for the cluster. This is the content of the ``customerCA.crt`` file that you created when you `initialized the cluster <https://docs.aws.amazon.com/cloudhsm/latest/userguide/initialize-cluster.html>`__ .
    ...
    def create_grant(
        ...
        To perform this operation on a CMK in a different AWS account, specify the key ARN in the value of the ``KeyId`` parameter. For more information about grants, see `Grants <https://docs.aws.amazon.com/kms/latest/developerguide/grants.html>`__ in the * *AWS Key Management Service Developer Guide* * .
        ...
        **Request Syntax**
        ...
        :param GranteePrincipal: **[REQUIRED]**
          ...
          To specify the principal, use the `Amazon Resource Name (ARN) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`__ of an AWS principal. Valid AWS principals include AWS accounts (root), IAM users, IAM roles, federated users, and assumed role users. For examples of the ARN syntax to use for specifying a principal, see `AWS Identity and Access Management (IAM) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arn-syntax-iam>`__ in the Example ARNs section of the *AWS General Reference* .
        ...
        :param Constraints:
          ...
          Allows a cryptographic operation only when the encryption context matches or includes the encryption context specified in this structure. For more information about encryption context, see `Encryption Context <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context>`__ in the * *AWS Key Management Service Developer Guide* * .
        ...
        :param GrantTokens:
          ...
          For more information, see `Grant Tokens <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#grant_token>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def create_key(
        ...
        """
        Creates a customer managed `customer master key <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ (CMK) in your AWS account.

        You can use a CMK to encrypt small amounts of data (up to 4096 bytes) directly. But CMKs are more commonly used to encrypt the `data keys <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#data-keys>`__ that are used to encrypt data.

        To create a CMK for imported key material, use the ``Origin`` parameter with a value of ``EXTERNAL`` .

        To create a CMK in a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ , use the ``CustomKeyStoreId`` parameter to specify the custom key store. You must also use the ``Origin`` parameter with a value of ``AWS_CLOUDHSM`` . The AWS CloudHSM cluster that is associated with the custom key store must have at least two active HSMs in different Availability Zones in the AWS Region.
        ...
        **Request Syntax**
        ...
        :param Policy:
          ...
          * If you don't set ``BypassPolicyLockoutSafetyCheck`` to true, the key policy must allow the principal that is making the ``CreateKey`` request to make a subsequent  PutKeyPolicy request on the CMK. This reduces the risk that the CMK becomes unmanageable. For more information, refer to the scenario in the `Default Key Policy <https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html#key-policy-default-allow-root-enable-iam>`__ section of the * *AWS Key Management Service Developer Guide* * .

          * Each statement in the key policy must contain one or more principals. The principals in the key policy must exist and be visible to AWS KMS. When you create a new AWS principal (for example, an IAM user or role), you might need to enforce a delay before including the new principal in a key policy because the new principal might not be immediately visible to AWS KMS. For more information, see `Changes that I make are not always immediately visible <https://docs.aws.amazon.com/IAM/latest/UserGuide/troubleshoot_general.html#troubleshoot_general_eventual-consistency>`__ in the *AWS Identity and Access Management User Guide* .

          If you do not provide a key policy, AWS KMS attaches a default key policy to the CMK. For more information, see `Default Key Policy <https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html#key-policy-default>`__ in the *AWS Key Management Service Developer Guide* .
        ...
        :param Origin:
          ...
          When the parameter value is ``AWS_CLOUDHSM`` , AWS KMS creates the CMK in an AWS KMS `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ and creates its key material in the associated AWS CloudHSM cluster. You must also use the ``CustomKeyStoreId`` parameter to identify the custom key store.

        :type CustomKeyStoreId: string
        :param CustomKeyStoreId:

          Creates the CMK in the specified `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ and the key material in its associated AWS CloudHSM cluster. To create a CMK in a custom key store, you must also specify the ``Origin`` parameter with a value of ``AWS_CLOUDHSM`` . The AWS CloudHSM cluster that is associated with the custom key store must have at least two active HSMs, each in a different Availability Zone in the Region.
        ...
        :param BypassPolicyLockoutSafetyCheck:
          ...
          .. warning::
            ...
            For more information, refer to the scenario in the `Default Key Policy <https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html#key-policy-default-allow-root-enable-iam>`__ section in the * *AWS Key Management Service Developer Guide* * .
          ...
          - *(dict) --*
            ...
            - **KeyMetadata** *(dict) --*
              ...
              - **Arn** *(string) --*
                ...
                The Amazon Resource Name (ARN) of the CMK. For examples, see `AWS Key Management Service (AWS KMS) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arn-syntax-kms>`__ in the Example ARNs section of the *AWS General Reference* .
              ...
              - **KeyState** *(string) --*
                ...
                For more information about how key state affects the use of a CMK, see `How Key State Affects the Use of a Customer Master Key <https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html>`__ in the *AWS Key Management Service Developer Guide* .
              ...
              - **CustomKeyStoreId** *(string) --*
                ...
                A unique identifier for the `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ that contains the CMK. This value is present only when the CMK is created in a custom key store.

              - **CloudHsmClusterId** *(string) --*

                The cluster ID of the AWS CloudHSM cluster that contains the key material for the CMK. When you create a CMK in a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ , AWS KMS creates the key material for the CMK in the associated AWS CloudHSM cluster. This value is present only when the CMK is created in a custom key store.
              ...
              - **KeyManager** *(string) --*
                ...
                The manager of the CMK. CMKs in your AWS account are either customer managed or AWS managed. For more information about the difference, see `Customer Master Keys <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def decrypt(
        ...
        **Request Syntax**
        ...
        :param EncryptionContext:
          ...
          The encryption context. If this was specified in the  Encrypt function, it must be specified here or the decryption operation will fail. For more information, see `Encryption Context <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context>`__ .
    ...
    def delete_custom_key_store(
        ...
        """
        Deletes a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ . This operation does not delete the AWS CloudHSM cluster that is associated with the custom key store, or affect any users or keys in the cluster.

        The custom key store that you delete cannot contain any AWS KMS `customer master keys (CMKs) <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ . Before deleting the key store, verify that you will never need to use any of the CMKs in the key store for any cryptographic operations. Then, use  ScheduleKeyDeletion to delete the AWS KMS customer master keys (CMKs) from the key store. When the scheduled waiting period expires, the ``ScheduleKeyDeletion`` operation deletes the CMKs. Then it makes a best effort to delete the key material from the associated cluster. However, you might need to manually `delete the orphaned key material <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key>`__ from the cluster and its backups.
    ...
    def describe_custom_key_stores(
        ...
        """
        Gets information about `custom key stores <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ in the account and region.
        ...
        For help repairing your custom key store, see the `Troubleshooting Custom Key Stores <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html>`__ topic in the *AWS Key Management Service Developer Guide* .
            ...
            - **CustomKeyStores** *(list) --*
              ...
              - *(dict) --*
                ...
                - **TrustAnchorCertificate** *(string) --*
                  ...
                  The trust anchor certificate of the associated AWS CloudHSM cluster. When you `initialize the cluster <https://docs.aws.amazon.com/cloudhsm/latest/userguide/initialize-cluster.html#sign-csr>`__ , you create this certificate and save it in the ``customerCA.crt`` file.
                ...
                - **ConnectionState** *(string) --*
                  ...
                  A value of ``FAILED`` indicates that an attempt to connect was unsuccessful. For help resolving a connection failure, see `Troubleshooting a Custom Key Store <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html>`__ in the *AWS Key Management Service Developer Guide* .
                ...
                - **ConnectionErrorCode** *(string) --*
                  ...
                  * ``USER_LOCKED_OUT`` - The ``kmsuser`` CU account is locked out of the associated AWS CloudHSM cluster due to too many failed password attempts. Before you can connect your custom key store to its AWS CloudHSM cluster, you must change the ``kmsuser`` account password and update the password value for the custom key store.
                  ...
                  For help with connection failures, see `Troubleshooting Custom Key Stores <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def describe_key(
        ...
        You can use ``DescribeKey`` on a predefined AWS alias, that is, an AWS alias with no key ID. When you do, AWS KMS associates the alias with an `AWS managed CMK <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ and returns its ``KeyId`` and ``Arn`` in the response.
        ...
        **Request Syntax**
        ...
        :param KeyId: **[REQUIRED]**
          ...
          If you specify a predefined AWS alias (an AWS alias with no key ID), KMS associates the alias with an `AWS managed CMK <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ and returns its ``KeyId`` and ``Arn`` in the response.
    ...
    def disable_key_rotation(
        ...
        """
        Disables `automatic rotation of the key material <https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html>`__ for the specified customer master key (CMK). You cannot perform this operation on a CMK in a different AWS account.
    ...
    def enable_key_rotation(
        ...
        """
        Enables `automatic rotation of the key material <https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html>`__ for the specified customer master key (CMK). You cannot perform this operation on a CMK in a different AWS account.

        You cannot enable automatic rotation of CMKs with imported key material or CMKs in a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ .
    ...
    def generate_data_key(
        ...
        **Request Syntax**
        ...
        :param EncryptionContext:
          ...
          For more information, see `Encryption Context <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def generate_random(
        ...
        By default, the random byte string is generated in AWS KMS. To generate the byte string in the AWS CloudHSM cluster that is associated with a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ , specify the custom key store ID.
        ...
        **Request Syntax**
        ...
        :param CustomKeyStoreId:
          ...
          Generates the random byte string in the AWS CloudHSM cluster that is associated with the specified `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ . To find the ID of a custom key store, use the  DescribeCustomKeyStores operation.
    ...
    def get_key_rotation_status(
        ...
        """
        Gets a Boolean value that indicates whether `automatic rotation of the key material <https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html>`__ is enabled for the specified customer master key (CMK).
    ...
    def get_parameters_for_import(
        ...
        **Request Syntax**
        ...
        :param WrappingAlgorithm: **[REQUIRED]**
          ...
          The algorithm you will use to encrypt the key material before importing it with  ImportKeyMaterial . For more information, see `Encrypt the Key Material <https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-encrypt-key-material.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def list_retirable_grants(
        ...
        **Request Syntax**
        ...
        :param RetiringPrincipal: **[REQUIRED]**
          ...
          To specify the retiring principal, use the `Amazon Resource Name (ARN) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`__ of an AWS principal. Valid AWS principals include AWS accounts (root), IAM users, federated users, and assumed role users. For examples of the ARN syntax for specifying a principal, see `AWS Identity and Access Management (IAM) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arn-syntax-iam>`__ in the Example ARNs section of the *Amazon Web Services General Reference* .
    ...
    def put_key_policy(
        ...
        For more information about key policies, see `Key Policies <https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def re_encrypt(
        ...
        Unlike other operations, ``ReEncrypt`` is authorized twice, once as ``ReEncryptFrom`` on the source CMK and once as ``ReEncryptTo`` on the destination CMK. We recommend that you include the ``"kms:ReEncrypt*"`` permission in your `key policies <https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html>`__ to permit reencryption from or to the CMK. This permission is automatically included in the key policy when you create a CMK through the console. But you must include it manually when you create a CMK programmatically or when you set a key policy with the  PutKeyPolicy operation.
    ...
    def schedule_key_deletion(
        ...
        If you schedule deletion of a CMK from a `custom key store <https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html>`__ , when the waiting period expires, ``ScheduleKeyDeletion`` deletes the CMK from AWS KMS. Then AWS KMS makes a best effort to delete the key material from the associated AWS CloudHSM cluster. However, you might need to manually `delete the orphaned key material <https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key>`__ from the cluster and its backups.

        You cannot perform this operation on a CMK in a different AWS account.

        For more information about scheduling a CMK for deletion, see `Deleting Customer Master Keys <https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html>`__ in the *AWS Key Management Service Developer Guide* .
    ...
    def update_custom_key_store(
        ...
        **Request Syntax**
        ...
        :param CloudHsmClusterId:
          ...
          Enter the cluster ID of the cluster that you used to create the custom key store or a cluster that shares a backup history and has the same cluster certificate as the original cluster. You cannot use this parameter to associate a custom key store with an unrelated cluster. In addition, the replacement cluster must `fulfill the requirements <https://docs.aws.amazon.com/kms/latest/developerguide/create-keystore.html#before-keystore>`__ for a cluster associated with a custom key store. To view the cluster certificate of a cluster, use the `DescribeClusters <https://docs.aws.amazon.com/cloudhsm/latest/APIReference/API_DescribeClusters.html>`__ operation.

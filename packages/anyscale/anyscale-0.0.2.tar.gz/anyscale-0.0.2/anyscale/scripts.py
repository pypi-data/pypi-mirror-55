...
def session_start(command, args, shell, snapshot, name):
    ...
    snapshot_directory, commit_hash, commit_patch = download_snapshot(snapshot)
    for run in session_runs:
        ...
        runner = AnyscaleSessionRunner(session_id, session_name)
        ...
        try:
            runner.sync_session(snapshot, snapshot_directory,
                    commit_hash, commit_patch, True)
        except click.Abort as e:
...
def session_execute(command, args, shell, name):
    ...
    for session in sessions:
        print("-- Executing command on session {}:".format(session["name"]))
        runner = AnyscaleSessionRunner(session["id"], session["name"])
        runner.execute_command(command, config, user_command=True)

...
class SiteLayoutTraverser(ResourceTraverser):
    ...
    def __init__(self, *args, **kwargs):
        super(SiteLayoutTraverser, self).__init__(*args, **kwargs)

        # Disable theming that diazo-rules are not applied for raw layout
        request = getRequest()
        needle = '++%s++' % self.name
        if needle in request.getURL() and not ISubRequest.providedBy(request):
...
class ContentLayoutTraverser(ResourceTraverser):
    ...
    def __init__(self, *args, **kwargs):
        super(ContentLayoutTraverser, self).__init__(*args, **kwargs)
...
class DefaultSiteLayout(BrowserView):
    ...
    def __call__(self):
        # Disable theming so that diazo-rules are not applied for raw layout
        if not ISubRequest.providedBy(self.request):

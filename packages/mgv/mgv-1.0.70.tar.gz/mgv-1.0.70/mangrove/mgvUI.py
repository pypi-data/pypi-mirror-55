...
class MgvTaskOutput(QtCore.QThread):
    ...
    def run(self):
        for line in iter(self.fid.readline, b''):
            self.signal.emit(line.decode('utf-8'), self.sign)
...
class MgvDownload(object):
    ...
    def selectType(self, i):
        if i > -1:
            ...
            self.root.dateLabel.setText('Uploaded at ' + codecs.decode(x['date'], 'utf-8'))
            self.root.userLabel.setText('By ' + codecs.decode(x['author'], 'utf-8'))

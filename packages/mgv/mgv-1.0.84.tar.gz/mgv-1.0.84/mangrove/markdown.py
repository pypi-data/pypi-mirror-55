...
class Element :
    ...
    def toxml(self):
        ...
        if self.nodeName in ['p', 'li', 'ul', 'ol',
            ...
            if "dir" not in self.attribute_values.keys():
                if self.bidi :
                    bidi = self.bidi
                else :
                    bidi = self.doc.bidi
    ...
    def _handleInlineWrapper (self, line) :
        ...
        for pattern in self.inlinePatterns :
            ...
            while i < len(parts) :
                ...
                x = parts[i]
                if (sys.version_info[0] < 3 and isinstance(x, (str, unicode))) or (sys.version_info[0] >= 3 and isinstance(x, (bytes, str))):
                ...
                for child in node.childNodes :
                    if isinstance(child, TextNode):
                        ...
                        if result:
                            ...
                            for item in result:
                                if (sys.version_info[0] < 3 and isinstance(item, (str, unicode))) or (
                                        sys.version_info[0] >= 3 and isinstance(item, (bytes, str))):
                                    if len(item) > 0:

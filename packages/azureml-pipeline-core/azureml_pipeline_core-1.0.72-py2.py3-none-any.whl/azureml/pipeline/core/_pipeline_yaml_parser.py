...
class _PipelineYamlParser(object):
    ...
    def load_yaml(workspace, filename, _workflow_provider=None, _service_endpoint=None):
        ...
        for step_name in steps_section:
            ...
            if _ADLA_STEP in step:
                input_list = list(inputs.values())
                output_list = list(outputs.values())

                common_parameters = _PipelineYamlParser._get_step_parameters(step, _ADLA_STEP)
            ...
            if _AZUREBATCH_STEP in step:
                input_list = list(inputs.values())
                output_list = list(outputs.values())
                common_parameters = _PipelineYamlParser._get_step_parameters(step, _AZUREBATCH_STEP)
    ...
    @staticmethod
    def _get_step_parameters(step, step_type):
        ...
        if step_type in [_DATATRANSFER_STEP, _DATABRICKS_STEP, _AZUREBATCH_STEP] and _NAME not in step_section:
            raise ValueError('Step %s does not contain a name ' % step_type)
    ...
    def _get_azurestep_parameters(step):
        step_section = step[_AZUREBATCH_STEP]
    ...
    def _get_databrickstep_parameter(step):
        step_section = step[_DATABRICKS_STEP]

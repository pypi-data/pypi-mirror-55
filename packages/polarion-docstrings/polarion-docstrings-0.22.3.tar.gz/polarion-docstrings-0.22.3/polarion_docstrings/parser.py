...
class DocstringParser:
    ...
    def parse(self):
        ...
        if linked_items and linked_items.value:
            sep = "," if "," in linked_items.value else " "
            linked_items_list = [item.strip('" ') for item in linked_items.value.split(sep)]
            linked_items_list = [item for item in linked_items_list if item]
            docstring_dict[self.SECTIONS.linked_items] = ValueRecord(
                linked_items.lineno, linked_items.column, linked_items_list
            )

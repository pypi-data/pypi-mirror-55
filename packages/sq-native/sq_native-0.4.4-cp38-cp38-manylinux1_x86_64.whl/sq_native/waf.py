...
def get_waf_lib():
    ...
    if _waf_lib is None:
        lib = get_lib()
        lib.powerwaf_initializePowerWAFWithDiag.argstype = [
            c_char_p, c_char_p, POINTER(c_char_p)]
        lib.powerwaf_initializePowerWAFWithDiag.restype = c_bool
...
def initialize_with_diag(rule_name, rule_data):
    ...
    try:
        ret = get_waf_lib().powerwaf_initializePowerWAFWithDiag(
            rule_name, rule_data, byref(diag_ptr))
        if diag_ptr:
...
def run(rule_name, parameters, budget):
    ...
    return PWRet(get_waf_lib().powerwaf_runPowerWAF(

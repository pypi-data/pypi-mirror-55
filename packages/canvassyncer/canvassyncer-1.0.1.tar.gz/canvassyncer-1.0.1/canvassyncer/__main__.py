...
class CanvasSyncer:
        ...
        for fileName, fileUrl in files.items():
            ...
            if fileSize > self.settings['filesizeThresh']:
                ...
                if isDownload not in ['y', 'Y']:
                    ...
                    continue
            self.downloadFiles.append(f"{self.courseCode[courseID]}{fileName} ({round(fileSize, 2)}MB)")
    ...
    def sync(self):
        ...
        if self.total_cnt == 0:
            ...
            print(f"Start to download following files! Total size: {round(self.download_size, 2)}MB")

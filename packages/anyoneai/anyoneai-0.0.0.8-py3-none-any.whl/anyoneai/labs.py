...
def getXML():
    if "filename" in request.args:
        ...
        xml_file = open(os.path.join(os.getcwd(), filename),'r')
        xml = xml_file.read()
        xml_file.close()
        if xml != "":
            return xml
...
def execute():
    if "filename" in request.args:
        filename = request.args["filename"]
        process = subprocess.Popen(["python", filename.replace(".aai", ".py")], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        def read_process():
            while True:
...
def service_labs():
    with open(os.path.join(ROOT_PATH, 'labs.json')) as labs:
        data = json.load(labs)

    ...
    for i in range(0,total_users):
        ...
        if user_rec: # check if dict has some data or not
            for k,v in user_rec.items():
                if v==tuple(chipping):
                    makeChippingCode(user_no,bit,total_users) # If dict has same chipping code re calculate new one
...
def getCommunicationData(Listening_node , Talking_node , total_users_1):
    if( sum(np.multiply(channel,user_rec[Talking_node]))/total_users_1>=0):

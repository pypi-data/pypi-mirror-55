...
class OpenADRVenAgent:
    ...
    @property
    @orm.db_session
    def active_reports(self) -> List[database.Report]:
        query = orm.select(
    ...
    @property
    @orm.db_session
    def inactive_reports(self) -> List[database.Report]:
        query = orm.select(
    ...
    def active_or_pending_reports(self) -> List[database.Report]:
        ...
        if self.inactive_reports:
            reports += self.inactive_reports
    ...
    def _get_reports(
        ...
        if active_only:
            reports = filter(
                lambda x: x.status
                not in [
                    enums.ReportStatus.COMPLETED.value,
                    enums.ReportStatus.CANCELED.value,
                ],
                reports,
            )
        if started_after:
    ...
    def metadata_report(self, specifier_id):
        ...
        if self.get_report_for_report_specifier_id(specifier_id):
            """ if there is already a report with this specifier id, return it """
            return self.get_report_for_report_specifier_id(specifier_id)

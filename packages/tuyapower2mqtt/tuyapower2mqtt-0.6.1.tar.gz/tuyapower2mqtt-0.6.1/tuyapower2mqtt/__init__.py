...
# Publish to MQTT service
    print('HOST: ' + mqttcreds[1] + ' PORT: ' + mqttcreds[2])
    mqttc = mqtt.Client(mqttcreds[3])
    mqttc.username_pw_set(mqttcreds[3], mqttcreds[4])
    mqttc.connect(mqttcreds[1], mqttcreds[2])
    mqttc.publish(mqttcreds[5]+deviceid, ret, retain=mqttcreds[6])
    mqttc.loop(2)

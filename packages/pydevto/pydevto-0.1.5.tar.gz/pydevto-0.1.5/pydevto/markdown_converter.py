...
class MarkdownConverter(object):
    ...
    def process_tag(self, node, children_only=False):
        ...
        for el in node.children:
            if isinstance(el, NavigableString):
                ...
                if next(el.children).name == "code":
                    text += (
                        "\n```\n" + str(next(next(el.children).children)) + "\n```\n"
                    )
                else:
                    text += self.process_tag(el)
            elif el.name == "code":
                text += "`" + str(next(el.children)) + "`"
            else:
                text += self.process_tag(el)
    ...
    def convert_code(self, el, text):
        return "`{text}`".format(text=text) if text else ""

    def convert_pre(self, el, text):
        return "`x{text}`".format(text=text) if text else ""

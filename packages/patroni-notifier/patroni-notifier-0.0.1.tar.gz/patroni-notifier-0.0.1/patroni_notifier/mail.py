...
class Template(object):
    ...
    def render_blocks(self, template_name, **context):
        ...
        for block in template.blocks:
            blocks[block] = self._render_context(
                template, template.blocks[block], **context
            )
        return blocks
    ...
    def render(self, template_name, block, **context):
        ...
        """
        template = self._get_template(template_name)
        return self._render_context(template, template.blocks[block], **context)
...
class Mail(object):
    ...
    def send_template(self, template, to, reply_to=None, **context):
        ...
        return self.send(
            to=to, subject=subject, body=body, reply_to=reply_to, **mail_data
        )
        ...
        for rb in required_blocks:
            if rb not in blocks:
                raise AttributeError(
                    "Template error: block '%s' is missing from '%s'" % (rb, template)
                )

        mail_params = {"subject": blocks["subject"].strip(), "body": blocks["body"]}

...
class NFLTeamAnalytics:
    ...
    class TeamsStatsDict(dict):
        def __init__(self, statsDF, teamsDF):
            self._statsDF = statsDF
            self._teamsDF = teamsDF
        ...
        def __getitem__(self, key):
            ...
            _retDF = self._teamsDF.copy()
            for key in ['statValue','count','nGames','nDrives','rank','avg','stddev','stddev/avg','stdFactor','absStdFactor', 'nItems'
                        #, 'statName','statObject','dimensions','seasons','calculation'
                        ]:
                _retDF[key] = _df[key]
            _retDF['teamName'] = _retDF['teamFullname']
            return _retDF.fillna(0.0).sort_values('statValue', ascending=False).drop(columns=['season', 'city', 'teamFullname'])
    ...
    def _read_all_metrics(self, seasons):
        if not os.path.exists(self.all_team_stats_filename):
            ...
            self.generator.nfldata.bqu.download_from_gcp(NFL_GCP_BUCKET, self.all_team_stats_filename, self.all_team_stats_filename)
    ...
    def _build_all_metrics(self, seasons):
        ...
        self.generator.nfldata.bqu.upload_file_to_gcp(NFL_GCP_BUCKET, self.all_team_stats_filename, self.all_team_stats_filename)
    ...
    def get_stat_iterator(self, statName, season, statObject):
        ...
        _statsDF = self.allTeamsStatsDF.query('statName=="{}" and seasons=="{}" and statObject=="{}"'.format(statName, str([season]), statObject))
        return self.TeamsStatsDict(_statsDF, self.teams)

    @contendo_classfunction_logger
    def get_dimentions(self) -> list:
        return list(self.generator.dimentionsDict.keys())
...
def test():
    ...
    ostats = NFLTeamAnalytics(seasons=['2019-regular'])
    ...
    statsDict = ostats.get_stat_iterator(statName='completions', season='2019-regular', statObject='offenseTeam')
    for key in statsDict:
        logger.info(key)
        #logger.info(statsDict[key])
        break
    logger.info('Done.')

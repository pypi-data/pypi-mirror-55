...
class NFLStatsQueries():
    ...
    def get_team_counters(self, object, dimensions, season='2019-regular'):
        ...
        if _key not in self.cachedTeams:
            ...
            _filteredDF = _df#.query('{}'.format(self._get_dimentions_condition(dimensions)).replace('df.', '').replace('(True) &', '').replace('True &', ''))
    ...
    def pbp_get_stat(self, statName, object, dimensions=['all'], season='2019-regular', aggfunc=None, playType=None, filter='True', trace=False):
        ...
        if statObject=='team':
            ...
            finalDF = teamsDF.join(
                ...
                #on=_keys,
                how='left',
            ).fillna(0).sort_values('statValue', ascending=False)
...
def test():
    ...
    df = generator.pbp_get_stat(statName='passAndRush', object='offenseTeam', dimensions=['conversion4', '4thQ'], season='2019-regular')

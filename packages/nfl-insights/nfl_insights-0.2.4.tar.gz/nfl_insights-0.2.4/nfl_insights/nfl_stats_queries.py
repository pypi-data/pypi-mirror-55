...
class NFLStatsQueries():
    ...
    def pbp_get_stat(self, statName, object, dimensions=['all'], seasons=['2019-regular'], aggfunc=None, playType=None, filter='True', trace=False, playerId = None, teamId=None):
        ...
        if statObject=='team':
            finalDF.columns=['teamId', 'statValue', 'count', 'nGames']
            ...
            finalDF.columns=['playerId', 'position', 'teamId', 'statValue', 'count', 'nGames']
...
def test():
    ...
    df = generator.pbp_get_stat(statName='yardsAfterPass', object='defenseTeam', dimensions=['3rdDown', '2ndQ'], seasons=['2019-regular'])

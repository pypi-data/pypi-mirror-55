    ...
    def _create_play(self, play, parent=None):
        try:
            ...
            if not parent: # main play
                ...
                if self.prevPlay is not None:
                    # check conversion
                    if self.prevPlay['teamInPossession_id'] == _newplay['teamInPossession_id'] and self.gc.down == 1:
                        _newplay['isConversion'] = True
                    else:
                        _newplay['isConversion'] = False

                    # handle Drives
                    if self.prevPlay['teamInPossession_id'] != _newplay['teamInPossession_id']:
                        self.gameDrive +=1
            ...
            # set drive
            _newplay['gameDrive'] = self.gameDrive

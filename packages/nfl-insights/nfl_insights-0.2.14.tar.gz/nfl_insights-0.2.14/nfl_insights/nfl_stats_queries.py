...
class NFLStatsQueries():
    ...
    def pbp_get_composed_stat(self, compstat, object, dimensions=['all'], season='2019-regular', filter='True'):
        ...
        numeratorDF = self.pbp_get_stat(numerator, object, dimensions, season, filter=filter)#.set_index(_keys)
        if numeratorDF.shape[0]==0:
            return numeratorDF
        #
        # get the denominator data
        denominatorDF = self.pbp_get_stat(denominator, object, dimensions, season, filter=filter)#.set_index(_keys)
        ...
        #df['statValue'] = df['statValue']/df.query('statValue_dn!=0')['statValue_dn']*compstatDef['StatRatio']
        ...
        for col in df.columns:
            if col.find('_dn')==-1 or col==denominatorDef['AggField']+'_dn':
                retCols.append(col)
        retDF = df[retCols].fillna(0)
        retDF.set_index(_keys, inplace=True)
        return retDF
...
def test():
    ...
    #df = generator.pbp_get_composed_stat('punts', 'kickingTeam', dimensions=['all', 'all'], season='2019-regular')
    df = generator.pbp_get_stat(statName='offensePoints', object='offenseTeam', dimensions=['all', 'all'], season='2019-regular')
    #df = generator.pbp_get_stat(statName='defensePoints', object='defenseTeam', dimensions=['all', 'all'], season='2019-regular')
    #df = generator.pbp_get_stat(statName='kickingPoints', object='kickingTeam', dimensions=['all', 'all'], season='2019-regular')
    #df = generator.pbp_get_stat(statName='retrievingPoints', object='retrievingTeam', dimensions=['all', 'all'], season='2019-regular')

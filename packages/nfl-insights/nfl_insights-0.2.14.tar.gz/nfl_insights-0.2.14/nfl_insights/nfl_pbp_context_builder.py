...
class NFLPbpContextBuilder:
    ...
    def _create_play(self, play, parent=None, playStatus=None):
        try:
            ...
            _newplay['awayScore'] = self._playContext.score[0]
            _newplay['homeScore'] = self._playContext.score[1]
            #
            ...
            if not _isSubplay: # main play
                ...
                # handle score-diff
                _newplay['scoringPlay'] = self._playContext.scoringPlay
                ...
                if  self._playContext.offensePoints>0:
                    _newplay['offensePoints1'] = self._playContext.offensePoints
                if  self._playContext.defensePoints>0:
                    _newplay['defensePoints1'] = self._playContext.defensePoints
                if  self._playContext.kickingPoints>0:
                    _newplay['kickingPoints1'] = self._playContext.kickingPoints
                if  self._playContext.retrievingPoints>0:
                    _newplay['retrievingPoints1'] = self._playContext.retrievingPoints
    ...
    def _calculate_points(self, newplay):
        ...
        if 'offenseTouchdown' in newplay:
            _newplay['offensePoints'] = 6
            return
        if 'defenseTouchdown' in newplay:
            _newplay['defensePoints'] = 6
            return
        if 'returnTouchdown' in newplay:
            _newplay['retrievingPoints'] = 6
            return

...
def verify(self):
    '''
    User verification using Authenticate.credentials
    '''
    ...
    resp_verify = requests.get(verify_URL
                               ,auth=(self.username, password))
    return resp_verify, password
    

def specs_from_input(self,password):
    '''
    
    Create spec file from 'specs' input
    ...
    resp_create_specs = requests.post(create_specs_url
                          , data = specs_data
                          , auth = (self.username,password))
    return resp_create_specs

def specs_from_analyzer(self, resp_specs, password):
    '''
    
    Create a spec file from the output of the spec analyzer
...
def specs_from_folder(self,resp_specs):
    '''
    
    Get spec file from created folder where data is uploaded.
    ...
    for spec_file in specs_json['list']:
        if 'RAMIEL' in spec_file['angelName']:
            if self.specFileName:
                if self.specFileName == spec_file['fileName'].split('.')[0]:
                ...
                if createdDate_curr > createdDate:
                    ...
                    spec_file_name = spec_file['fileName'].split('.')[0]
                    valid_specFileName = 1
    return valid_specFileName, spec_file_name


def create_folder(self,password):
    '''
    ...
    resp_CreateFolder = requests.post(createFolder_URL
                                      ,data = folder_data
                                      ,auth=(self.username, password))
    return resp_CreateFolder
...
class simSearch():
    ...
    def __init__(self,authenticate_creds,*,DataUploaded = True,fileName = '', specs={},
        ...
        resp_verify, password = verify(self)
    ...
    def fit(self, *, X='', instanceName = ''):
        ...
        if instanceName:
            ...
            if self.DataUploaded:
                ...
                if self.specs:
                    ## Create spec file
                    resp_create_specs = specs_from_input(self,password)
                    ...
                    if resp_specs.status_code == 200:
                        valid_specFileName, spec_file_name = specs_from_folder(self,resp_specs)
                    else:
                    ...
                    elif not valid_specFileName and not self.specs:
            ...
            ## New data not uploaded yet is being used to train angel
                ...
                if not self._folder_exists:
                    resp_CreateFolder = create_folder(self,password)
                ...
                if not self.specs:
                    ...
                    if resp_specs.status_code == 200:
                        resp_create_specs = specs_from_analyzer(self, resp_specs, password)
                        if not resp_create_specs.status_code == 200:
                            raise AttributeError(resp_create_specs.content.decode())
                    else:
                        raise AttributeError(resp_specs.content.decode())

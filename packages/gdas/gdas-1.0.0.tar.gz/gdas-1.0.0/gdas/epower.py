...
###############################################################################
import gdas,time,math,os,scipy,lal,lalburst,numpy
...
def excess_power(ts_data,                      # Time series from magnetic field data 
    ...
    if impulse:
        ...
        fd_psd = types.FrequencySeries(flat_data, 1./segment_length, ts_data.start_time)
    else:
        # Create overall PSD using Welch's method
        fd_psd = psd.welch(ts_data.astype(numpy.float64),avg_method=psd_estimation,seg_len=seg_len,seg_stride=seg_stride)
    if make_plot:

...
def _all_outputs(datadriftdetector, start_time, end_time, run_id=None, daily_latest_only=True):
    ...
    metrics_got = _get_metrics(datadriftdetector, start_time, end_time, run_id, daily_latest_only)
    ...
    # if run_id is None, pick only scheduled runs' results
        ...
        if len(metrics_schedulerun) < 1:
            ...
            logger.debug("Filtered in {} records of scheduled/backfill runs.".format(len(metrics_schedulerun)))
...
def _get_metrics(datadriftdetector, start_time, end_time, run_id, daily_latest_only=True):
    """

    :param datadriftdetector:
    :param daily_latest_only:
    :return:
    """
    ...
    if drift_type == DATADRIFT_TYPE_DATASET:
        _download_from_blob_metrics(datadriftdetector, local_temp_root, metrics, latest_pipeline_times, None,
                                    start_time, end_time, run_id)
    # Only model based drift needs check all services.
    elif drift_type == DATADRIFT_TYPE_MODEL:
        all_services = services
        for s in all_services:
            _download_from_blob_metrics(datadriftdetector, local_temp_root, metrics, latest_pipeline_times, s,
                                        start_time, end_time, run_id)
...
def _download_from_blob_metrics(datadriftdetector, local_temp_root, target_metrics, latest_pipeline_times, service,
                                start_time, end_time, run_id):
    ...
    # find nothing, which means no files on blob storage, means outputs were not generated.
        ...
        if latest_run_time:
            error_msg += " Given latest run accomplished at {}, a possible reason is historical runs were failed. " \
                         "Please check run history for more details.".format(latest_run_time)
        else:
...
def _show(datadriftdetector, start_time=datetime.min, end_time=datetime.max, with_details=False):
    ...
    metrics = _get_metrics(datadriftdetector, start_time, end_time, run_id=None, daily_latest_only=True)
    ...
    for c, c_metrics in contents.items():
        ...
        if drift_type == DATADRIFT_TYPE_DATASET:
            ...
            fig_key += c
            environment += "DataDrift Monitor For\n\n" \
                           "Model Name : {}\nModel Version : {}                            \nService Name : {}".\
                format(model_name, model_version, c)

        figure = _generate_plot_figure(environment, ordered_content)
...
def _generate_plot_figure(environment, ordered_content):
    ...
    plt.suptitle(environment, fontsize=24)
    ...
    plt.title(DRIFT_MAGNITUDE_TITLE + "\n", fontsize=18)
    ...
    plt.ylabel(Y_LABEL_PERCENTAGE, fontsize=16)
    plt.xticks(xrange, rotation=15)
    plt.title(DRIFT_CONTRIBUTION_TITLE + "\n", fontsize=18)

...
def orientation_map(data,start=0,end=None,xstep=32,ystep=32,patchw=32):
    ...
    orient = np.nan * np.zeros([len(gridy),len(gridx),end-start])
    dzeta  = np.nan * np.zeros([len(gridy),len(gridx),end-start])
    Q = np.zeros_like(n_maskQ)
    Q2 = np.zeros([2,2,ny,nx,nt])
    ...
    for t,ti in enumerate(range(start,end)): # Loop on the movie frames
        for i,xi in enumerate(gridx): # Loop over the grid
            for j,yj in enumerate(gridy):
                ...
                S = np.fft.fftshift(np.abs(np.fft.fft2(patch.dot(w)) ** 2))
                ...
                Q2[1,1,j,i,t] -= 0.5
                Q2[:,:,j,i,t] *= np.sqrt(2)
                orient[j,i,t],dzeta[j,i,t] = main_direction(Q2[:,:,j,i,t])
    return orient, dzeta

...
class SenseDataService(object):
    ...
    # 22于鑫用
    @catch_except_log
    def get_infos_by_terms(self, names_list: list):
        _names_list = json.dumps(names_list, ensure_ascii=False)
        with grpc.insecure_channel(self._host + ":" + self._port,
                                   options=[('grpc.max_send_message_length', 30 * 1024 * 1024),
                                            ('grpc.max_receive_message_length', 30 * 1024 * 1024)]) as channel:
            _stub = stock_pb2_grpc.StockInfStub(channel)
            _response = _stub.get_infos_by_terms(stock_pb2.Request(entity_name=_names_list))
            # print(_response)
            _status = _response.status
            if _status.code != 0:
                sd.log_error("get_infos_by_terms return error:" + _status.msg)
                return None
            _result_list = json.loads(_response.txt)
            return _result_list
...
if __name__ == '__main__':
    ...
    r = sense_data.get_infos_by_terms(['石化油服', '振静股份'])
    print(r)

...
class StructBase(object):
    '''a StructBase is an abstract class to create the other data structures
    '''
    pass


class Struct(StructBase, dict):
    '''a Struct is a dict based class that allows for printing 
    ...
    '''
    def __init__(self, name=None, attType=None, required=None, 
    ...
    def set(self, value):
        ...
        '''
        # If it's a list with another type inside
        if isinstance(self.attType, list) and self.attType:
    ...
    def to_dict(self):
        ...
        if self.validate():
            ...
            for name, att in self.items():
                ...
                # If empty but don't hide, show as empty
                if not att.value:
                    value = lookup.get(att.attType, [])
                    result[att.jsonName] = value

                # if it's a list, parse through it

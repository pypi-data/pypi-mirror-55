...
URL_RECOGNITIONS_BASE = "https://{}.sighthoundapi.com/v1/recognition"
...
def get_names(recognitions: Dict) -> List[Dict]:
    """
    Get the list of the people.
    """
    names = []
    for obj in recognitions["objects"]:
        ...
        if not obj["objectType"] == "person":
            continue
        name = {}
        name["recognitionConfidence"] = obj["faceAnnotation"]["recognitionConfidence"]        
        if (name["confidence"] > 0.5):
            name["name"] = obj["objectId"]
        else:
            name["name"] = ""
        names.append(name)
    return names
...
def run_recognition(
    image_encoded: str, api_key: str, url_recognition: str
) -> requests.models.Response:
    """Post an image to Sighthound."""
    headers = {"Content-type": "application/json", "X-Access-Token": api_key}
    response = requests.post(
...
class cloud:
    ...
    def __init__(self, api_key: str, mode: str = "dev"):
        ...
        self._url_recognition = URL_RECOGNITIONS_BASE.format(mode)
    ...
    def recognize(self, image: bytes) -> Dict:
        ...
        response = run_recognition(
            encode_image(image), self._api_key, self._url_recognition
        )
        if response.status_code == HTTP_OK:
            return response.json()
        elif response.status_code == BAD_API_KEY:

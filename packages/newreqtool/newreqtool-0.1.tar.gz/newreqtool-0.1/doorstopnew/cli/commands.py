...
from doorstopnew import common, server
from doorstopnew.cli import utilities
from doorstopnew.core import editor, exporter, importer, publisher
from doorstopnew.core.builder import build

log = common.logger(__name__)

''' Adding header for markdown file '''
headers = '-----'.split()
...
def run_add(args, cwd, _, catch=True):
    ...
    with utilities.capture(catch=catch) as success:
        ...
        for _ in range(args.count):
            ...
            filename = os.path.basename(cwd+"/"+str(item)+".md")
            realpath = item.relpath
            modifiedPath = realpath.replace("@/", "")
            filename = cwd + "/" +  modifiedPath

            '''
            Add headers to the markdown file which is created 
            ----
            key : value
            key : value
            ----
            ...
            # adding header1 to the markdown file
            for line in fileinput.input([filename], inplace=True):
                if fileinput.isfirstline():
                    print('\t'.join(headers))
                print(line)

            # adding header2 to the markdown file 
            header = "-----"

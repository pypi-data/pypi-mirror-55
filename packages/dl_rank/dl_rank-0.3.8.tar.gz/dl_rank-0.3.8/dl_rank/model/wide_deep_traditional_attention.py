    ...
    def _forward_model(self, is_training, *args, **kwargs):
        ...
        with tf.variable_scope('trunk'):
            ...
            if self.mode == 'train':
                ...
                self.sequence = sequence_emb                    # placeholder: embedding_lookup_2/Reshape_1
                self.itemid = tf.squeeze(itemid_emb, axis=1)    # placeholder: trunk/Squeeze
...
def _linear(xs,output_size,bias,bias_start=0., scope=None):
    with tf.variable_scope(scope or 'linear_layer'):
        ...
        if bias:
            ...
            # if scope == "attention_output_attention_layer":
            #     tfprint = tf.print(W, bias_name, summarize=10000)

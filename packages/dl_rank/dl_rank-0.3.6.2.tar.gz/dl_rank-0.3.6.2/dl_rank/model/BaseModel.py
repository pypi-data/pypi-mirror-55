            ...
            with tf.variable_scope(feature_scope, reuse=tf.AUTO_REUSE, partitioner=partitioner) as scope:
                if f_type == 'category':
                    ...
                    if f_tran == 'vocabulary_list':
                        ...
                        f_size = f_param['size']
                        fill_value = str(f_param['fill']) if 'fill' in f_param else ''#'0'
                    else:
                    ...
                    elif 'default' in f_param:
                        fill_value = f_param['default']
                    else:
                        fill_value = ''
                    if isinstance(fill_value, str):
                        fill_value = tf.get_variable(initializer=tf.random.normal([], 0.0, 0.1), name='{}_fill_value'.format(fill_value))

                    tail_value = f_param['tail'] if 'tail' in f_param else 0
                    if isinstance(tail_value, str):
                        tail_value = tf.get_variable(initializer=tf.random.normal([], 0.0, 0.1), name='{}_tail_value'.format(tail_value))

...
class BaseParser(object):
    ...
    def first_parse_fn(features, params, teriary_delim):
        ...
        def sparse_str2dense_num(f_input, field_num, type, null_value):
            ...
            f_output = layers.to_dense(f_input, depth=field_num, default_value=null_value, out_type=type)
            mask = tf.not_equal(f_output, null_value)
        ...
        for f in deep:
            ...
            if f_type == 'category':
                if combiner == 'none':
                    ...
                    if f in table.keys():
                        features_f_indice = table[f].lookup(features[f])
                        features_f_indicator = layers.to_dense(features_f_indice, depth=abs(num), default_value=null_value)
                        f_mask = tf.not_equal(features_f_indicator, null_value)
                    else:

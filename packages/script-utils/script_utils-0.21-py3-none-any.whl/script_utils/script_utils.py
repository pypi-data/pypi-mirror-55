...
try:
    ROOT = environ['MILION_ROOT']
except:
    print("ustaw zmienną środowiskową MILION_ROOT! (np. /home/milion, najlepiej w .bashrc)")
    exit(1)

SETTINGS_FNAME = join(ROOT, 'settings_local.yaml')

if not isfile(SETTINGS_FNAME):
    assert False, "Brak pliku z konfiguracją w katalogu głównym! (wzór: settings_local_example)"

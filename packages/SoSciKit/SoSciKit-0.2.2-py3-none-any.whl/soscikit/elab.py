...
class Output():
    ...
    def cluster_output(self, request, dataset):
        data = dataset
    ...
    def ncrosstab_classification_ouput(self, dataset):
...
class Operation():
    ...
    def create_subset(self, dataset):
        ...
        dataset_subset.to_excel("./static/uploads/" + new_filename)
    ...
    def action(self, request):
        ...
        if g.form["select_action"] == "monovariate":
            return render_template("monovariate/monovariate_selection.html",
                                   variable_name_type=self.variable_name_type,
                                   dataset=self.selected_file)
        ...
        if g.form["select_action"] == "bivariate_and_regression":
            return render_template("bivariate/bivariate_selection.html",
                                   variable_name_type=self.variable_name_type_obj,
                                   dataset=self.selected_file,
                                   variable_name_type_obj=self.variable_name_type_obj)

        if g.form["select_action"] == "multi cross tab":
            # qui si potrebbe mettere un vincolo alle variabili che hanno max numero di categorie
        ...
        if g.form["select_action"] == "k-mean":
            ...
            return redirect(url_for("profiling_output"))

...
def snake(v, seperator=u'_'):
    ...
    'pascal_case'
    """
    # Not use re '(?=[A-Z])' because it not work in python2.7
    ret = u''
    for char in v:
...
def _split(v, delimiters=SEPERATORS):
    ...
    for index, i in enumerate(v):
        if i in delimiters:
            delimiter_pos.append(index)
    delimiter_pos.append(-1)
    for i in xrange(0, len(delimiter_pos)):
        ret.append(v[delimiter_pos[i]:delimiter_pos[i+1]])
    return

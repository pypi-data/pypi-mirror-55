...
def write_mime_to_disk(path, member_id, server_id, mime):
    try:
        ...
        filename = os.path.join(member_dir, server_id) + '.eml'
...
def fetch_approvers_export_shards(approvers, request_id, cs_client):
    ...
    for a in approvers:
        threads.append(
            gevent.spawn(cs_client.getEDiscoveryShards, a, a.org_info.org_id, request_id))
        ...
        for member_id, member_key_shards in approver_shards.iteritems():
            if len(member_key_shards) > 0:
                for key_shard in member_key_shards:
                    ...
                    encrypted_shard = ExportShard.fromDict(key_shard)
                    decrypted_shard = encrypted_shard.decryptShard(
                        approver.getKeyWithVersion(encrypted_shard.sharee_key_version),
                        members[member_id].getPublicUserKeyWithVersion(encrypted_shard.sharer_key_version)
                    )
...
def approve_and_sign_request(approvers, request_id, cs_client):
    def _approve(approver):
        requests = cs_client.getUserApprovals(approver, u'pending', u'pending', True, 0, 10)
        for r in requests['approvals']:
            if r['request_id'] == request_id:
                # TODO: need to avoid re-approval failures and just consider them a pass
                cs_client.respondToOrgApproval(
...
def validate_approvers(approvers, cs_client):
    ...
    for a in approvers:
        threads.append(
            gevent.spawn(cs_client.getUserDevices, a))

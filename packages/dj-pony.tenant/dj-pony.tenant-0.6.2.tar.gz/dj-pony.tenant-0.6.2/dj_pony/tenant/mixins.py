...
def default_tenant_function():
    ...
    if current_tenant is not None:
        # TODO: Should this function validate that the tenant from this
        #  queryset is the same as the one returned by get_current_tenant() ?
        return Tenant.objects.get(pk=current_tenant.pk)
    return Tenant.objects.filter(slug=get_setting("DEFAULT_TENANT_SLUG")).first()

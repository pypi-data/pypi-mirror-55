...
def tabulate_bids(client, project_label, path=".", subject_labels=None,
    ...
    for sesnum, session in enumerate(sessions):
        ...
        logger.info("Gathering sequence info data for %s (%d/%d)...", session.label, sesnum+1,
                    num_sessions)
    ...
    concat_df = pd.concat(final_dfs)

    if dry_run:
        print(concat_df)
    else:
        concat_df.to_csv("{}/{}_SeqInfo.tsv".format(path, project_label),
                  sep="\t", index=False)

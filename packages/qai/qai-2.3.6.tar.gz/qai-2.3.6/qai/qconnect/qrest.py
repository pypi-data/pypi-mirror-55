...
def get_cpu_quota_within_docker():
    ...
    if cpu_shares.exists():
        with cpu_shares.open('rb') as r:
            ...
            cpu_cores = math.ceil(request_cpu_shares / 1024) if request_cpu_shares > 0 else multiprocessing.cpu_count()
            print(f"CPU request limit: {round(request_cpu_shares / 1024, 2)}, will spin {cpu_cores} worker(s).")
    else:

...
def nnmf(genomes, nfactors, init="random"):
    ...
    if init == "random":
        #NIMFA
        nmf = nimfa.Nmf(genomes, max_iter=100000, rank=nfactors, update='divergence', seed = "nndsvd",objective='div', test_conv= 30)
        nmf_fit = nmf()
        W = nmf_fit.basis()
        H = nmf_fit.coef()
        ...
        model = NMF(n_components=nfactors, max_iter=1000000, solver= "mu",init= init, tol=0.00000001, beta_loss = 'kullback-leibler', verbose=False)
...
def nmf(genomes=1, totalProcesses=1, resample=True, gpu=False):   
    ...
    if resample == True:
        bootstrapGenomes= BootstrapCancerGenomes(genomes, seed = None)
...
#############################################################################################################
"""
def decipher_signatures(genomes=[0], i=1, totalIterations=1, cpu=-1, mut_context="96", resample=True, seeds = None, init="random", normalization_cutoff=1, gpu=False):

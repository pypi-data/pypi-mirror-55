...
class Path(object):
    def __init__(self, ct_root="/", fs_root="/", name=None):
    ...
    def get(self, ct_root=None, fs_root=None, name=None):
        p = self.item(ct_target=ct_root, fs_target=fs_root)
        return Path(ct_root=p.ct, fs_root=p.fs, name=name or self.name)
...
class PathObject(object):
    ...
    def isdir(self):
        os.path.isdir(self.fs)

    def isfile(self):
        os.path.isfile(self.fs)

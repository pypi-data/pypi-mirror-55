...
{  // called in an infinite loop by the main program
	...
	//sample_level("read mic or DGT", cSamples, mic_count, CLIP16);
	...
	if (MicPlayback.handle) {		// Mic playback: send mic I/Q samples to a sound card
		//sample_level("MicPlayback.handle", cSamples, mic_count, CLIP16);
		mic_play_volume = 1.0;
		...
		// play mic samples
		//sample_level("play MicPlayback", cSamples, mic_count, CLIP16);
...
static void AddCard(struct sound_dev * dev, PyObject * pylist)
{
	...
	if (dev->name[0]) {
		v = Py_BuildValue("(ssiii)", dev->stream_description, dev->name, dev->sample_rate, dev->dev_latency, dev->dev_error + dev->dev_underrun);

...
{  /* Call with keyword arguments ONLY; change local parameters */
	...
	"bscope_bytes", "bscope_endian", "bscope_size", "bandscopeScale", NULL} ;
	int nbytes, read_error, clip, bscope_size;

	nbytes = read_error = clip = bscope_size = -1;
	if (!PyArg_ParseTupleAndKeywords (args, keywds, "|iiiiiiiid", kwlist,
	&quisk_is_vna, &nbytes, &py_sample_rx_endian, &read_error, &clip,
	&py_bscope_bytes, &py_bscope_endian, &bscope_size, &bandscopeScale))
		return NULL;
	if (nbytes != -1) {

...
class ControlPeer:
    ...
    def __init__(self, brick_config, flow_director, kafka_bootstrap_servers):
        self.loop = asyncio.get_event_loop()
        self.kafka_bootstrap_servers = kafka_bootstrap_servers

        self.flows = create_flows(
            self, brick_config, requests_flows_config(flow_director)
        )
...
def requests_flows_config(flow_director):
    """ask the flow director for the configuration of the flows"""
    response = requests.get("http://" + flow_director + "/flows")

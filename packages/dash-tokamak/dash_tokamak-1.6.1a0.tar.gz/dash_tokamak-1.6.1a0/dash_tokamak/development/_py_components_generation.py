...
from dash_tokamak.development.base_component import _explicitize_args
from dash_tokamak.exceptions import NonExistentEventException
...
def generate_class_file(typename, props, description, namespace):
    ...
    import_string =\
        ...
        "from dash_tokamak.development.base_component import " + \
...
def prohibit_events(props):
    ...
    if 'dashEvents' in props or 'fireEvents' in props:
        raise NonExistentEventException(
            'Events are no longer supported by dash_tokamak. Use properties instead, '
...
def reorder_props(props):
    """If "children" is in props, then move it to the front to respect dash_tokamak
    convention.
...
def map_js_to_py_types_prop_types(type_object):
    ...
    return dict(
        ...
        element=lambda: 'dash_tokamak component',
        node=lambda: 'a list of or a singular dash_tokamak '

...
def generate_components(
    ...
    extract_path = pkg_resources.resource_filename("dash_tokamak", "extract-meta.js")
    ...
    if rprefix is not None:
        ...
        if not os.path.exists("R"):
            os.makedirs("R")
        if os.path.isfile("dash_tokamak-info.yaml"):
            with open("dash_tokamak-info.yaml") as yamldata:
...
def cli():
    parser = argparse.ArgumentParser(
        prog="dash_tokamak-generate-components",
        formatter_class=_CombinedFormatter,
        description="Generate dash_tokamak components by extracting the metadata "

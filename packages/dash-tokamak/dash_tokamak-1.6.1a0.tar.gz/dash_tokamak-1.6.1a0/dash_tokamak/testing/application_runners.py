...
from dash_tokamak.testing.errors import (
    NoAppFoundError,
    TestingTimeoutError,
    ServerCloseError,
)
import dash_tokamak.testing.wait as wait
...
def import_app(app_file, application_name="app"):
    """Import a dash_tokamak application from a module. The import path is in dot
    ...
    :type app_file: str
    :param application_name: The name of the dash_tokamak application instance.
    ...
    try:
        ...
        raise NoAppFoundError(
            "No dash_tokamak `app` instance was found in {}".format(app_file)
        )
    return app
...
class ThreadedRunner(BaseDashRunner):
    """Runs a dash_tokamak application in a thread.

    This is the default flavor to use in dash_tokamak integration tests.
    """
...
class ProcessRunner(BaseDashRunner):
    """Runs a dash_tokamak application in a waitress-serve subprocess.
    ...
    def start(
        ...
        logger.debug("start dash_tokamak process with %s", args)
        ...
        if os.path.isfile(app) and os.path.exists(app):
            ...
            if not cwd:
                for entry in inspect.stack():
                    if "/dash_tokamak/testing/" not in entry[1].replace("\\", "/"):

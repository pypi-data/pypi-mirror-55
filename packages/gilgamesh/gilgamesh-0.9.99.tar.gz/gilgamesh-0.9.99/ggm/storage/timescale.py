...
class KTimescale(GK):
            ...
            try:
                ...
                if sig == 'measurements':
                    ...
                    try:
                        ...
                        await dbq.psend_err(f'no limit or offset in chunk query {str(e)}')
                        continue
                    ...
                    print(len(records))
                    await dbq.psnd(records, compression=True)
                    continue
    ...
    async def write_data(self):
        ...
        while True:
            ...
            # FIXME should not happen anymore -- remove when sure..
            if len(data) == 0:
                self.glog.debug(f"incoming data empty: {dev_id} {measurement}")
                continue

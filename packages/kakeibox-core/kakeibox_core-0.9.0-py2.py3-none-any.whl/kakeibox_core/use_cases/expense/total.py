...
class GetTotalExpense(UseCaseStrategy):
    ...
    def _list_per_type(self, start, end):
        return self.storage_data_bridge.list_per_type(start, end, 'EXP')

    def _sum_all_expenses(self, items):
        total = 0
        for item in items:
            total += item['amount']
    ...
    def execute(self, start_time, end_time):
        try:
            ...
            value = self._sum_all_expenses(incomes)

...
def get_var_units(files, key):
    from numpy import unique, nanargmax
...
def read_nc_files_divevars(files, keys, verbose=True):
...
def load_multiple_vars(files, variable_names, return_merged=False, verbose=True,
    ...
    for dim_name, var_names in dims_dict.items():
        ...
        if dim_name is 'string':
            ...
            df = read_nc_files_divevars(files, var_names, verbose)
        for col in df:
            df[col] = to_numeric(df[col], errors='coerce')

        # converting times that have 'seconds since 1970' units
        df = process_time(files, df)

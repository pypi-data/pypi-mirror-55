...
def get_all_images(folder, owner='jmulla'):
    ...
    images = []
    for df in glob.iglob('{folder}/**/Dockerfile'.format(folder=folder), recursive=True):
...
def make_image(image, dryrun=False, tags=[]):
    ...
    try:
        ...
        returnCode, _ = run('docker build . {tagsLine} -t {imm}:latest'.format(imm=image["image"], tagsLine=tagsLine), dry=dryrun)
    finally:

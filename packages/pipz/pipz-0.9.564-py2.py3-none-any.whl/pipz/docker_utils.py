...
import os
from pipz.run_utils import run
...
def get_all_images(folder, owner='jmulla'):
    ...
    images = []
    for df in glob.iglob(f'{folder}/**/Dockerfile', recursive=True):
        df_parts = df.split(os.sep)
        docker_image = f'{owner}/{df_parts[-2]}'
...
def make_images(image_chain, dryrun=False, tags=[]):
    for image in image_chain:
        ...
        if result != 0:
            raise Exception(f"Problem building image: {image['image']}")
...
def make_image(image, dryrun=False, tags=[]):
    ...
    try:
        ...
        os.chdir(folder)
        tagsLine = ' '.join([f'-t {images["image"]}:{tag}' for tag in tags])
        returnCode, _ = run(f'docker build . {tagsLine} -t {image["image"]}:latest', dry=dryrun)
    finally:

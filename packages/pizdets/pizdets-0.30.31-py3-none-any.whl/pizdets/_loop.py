...
def experiment_loop(self):
    ...
    schedule.every().minute.at(":00").do(pause_mixers_and_measure_OD,self=self)
    schedule.every().minute.at(":05").do(dilute_if_necessary,self=self)
    schedule.every().minute.at(":30").do(pause_mixers_and_measure_OD,self=self)
    schedule.every().minute.at(":35").do(dilute_if_necessary,self=self)

    for t in range(7): # wash peace and vacuum tubes to prevent drying out
        schedule.every().day.at("09:30").do(make_dilution,self=self, tube=t, peace_volume=1)

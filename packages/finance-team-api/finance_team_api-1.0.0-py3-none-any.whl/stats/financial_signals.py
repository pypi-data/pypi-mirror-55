...
class momentum_signals:
    def __init__(self, price_data, seq_len, timestep, use_columns=None):
    ...
    # 일반적인 수익률 측정 방식
    def periodic_return(self, method='hpr'):
        """
        ...
        if method == 'hpr':
            ...
            return geomtric_ret, self.datetimeIndex[:, -1]
    ...
    # 일반적인 변동성 측정방식
    def periodic_risk(self, freq, mode='std', ewma_lambda=0.94):
        """
        ...
        if self.input_data_type_check == True:
            ...
            simple_return = simple_return.iloc[freq:]
        ...
        if mode == 'std':
            batch_simple_return, datetime_index = convert_frame_to_numpy(data=simple_return,
                                                         seq_len=self.seq_len,
                                                         timestep=self.timestep)
            vol = np.std(batch_simple_return
                         , axis=1) * np.sqrt(constant)
            return vol, datetime_index[:, -1]
        ...
        # EWMA Volatility
            ...
            sumproduct_value = np.sum(element_wise_production
                                      ,axis=1) * constant
            ewma_vol = np.sqrt(sumproduct_value)
            return ewma_vol, datetime_index[:, -1]
        ...
        # Downside Deviation
            ...
            return downside_ret, datetime_index[:, -1]
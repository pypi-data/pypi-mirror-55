...
def reformat_data(data):
    ...
    if (len(asset_class[asset_class.index == asset_class]) == 1) & (asset_class[asset_class.index == asset_class].index[0] != 'Cash'):
    ...
    return data.astype(float), num_to_dict, arrange['Asset_Class']
...
def kelly_function(array, loc_vector, asset_class_table, constraints, prev_AP=None, turnover_constraint=None):
    """
    ...
    if turnover_constraint is not None:
        ...
        for i in asset:
            ...
            adj_lb = prev_AP[i] - turnover_constraint
            adj_ub = prev_AP[i] + turnover_constraint
    ...
    # 동일 비중으로 우선 배분
    w0 = np.ones(array.shape[1]) * (1 / array.shape[1])


    # Cost 반영 Optimize모델
    # port_weight은 직전 AP(Actual Portfolio)
    def kelly_function_cost_optim(w, ret_set):
        cost = np.sum(abs(np.array(prev_AP) - w)) * 0.001
        return -(np.log(1 - cost) + np.sum(np.log(1 + np.dot(ret_set, w)))) / len(ret_set)

    # Cost 반영x Optimize모델
    def kelly_function(w, ret_set):
        return -(np.sum(np.log(1 + np.dot(ret_set, w)))) / len(ret_set)

    if prev_AP is not None:
        set_function = minimize(kelly_function_cost_optim, w0, array, method='SLSQP', bounds=bounds, constraints=cons)
    else:
        set_function = minimize(kelly_function, w0, array, method='SLSQP', bounds=bounds, constraints=cons)
    ...
    # 자료형 조정
    weight = pd.DataFrame(optim_weight.reshape(1,-1), columns=asset_class_table.index)

...
def window_sub_daily_daily(date, observe_window, skip_term=None, coin_market=None):
...
def window_sub_daily_monthly(date, observe_window, skip_term=None, coin_market=None):
    # 데이터 관측 기간의 끝을 설정
    ...
    #     N/A          o            N/A
    if coin_market is not None:
        observe_start_date = pd.date_range(end=observe_end_date, periods=observe_window)[0]
        return pd.date_range(start=observe_start_date,
                             end=observe_end_date)
    # skip_term   coin_market   bm_date==date
    #     N/A          x            N/A
    else:
        observe_start_date = pd.date_range(end=observe_end_date, periods=observe_window, freq='B')[0]
        return pd.date_range(start=observe_start_date,
                             end=observe_end_date,
                             freq='B')
...
def window_sub_monthly_daily(date, observe_window, skip_term=None, coin_market=None):
    # 데이터 관측 기간의 끝을 설정
    if (skip_term is not None) & (coin_market is not None):
...
# observe_window = 'Monthly Setting'
# skip_term = 'Monthly Setting'
def window_sub_monthly_monthly(date, observe_window, skip_term=None, coin_market=None):
    # 데이터 관측 기간의 끝을 설정
    if (skip_term is not None) & (coin_market is not None):
...
def get_observe_period(date, observe_window, skip_term=None, coin_market=None):
    """

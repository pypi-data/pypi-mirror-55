...
def convert_frame_to_numpy(data
                           , seq_len
                           , timestep
                           , use_columns=None):
    """
    ...
    available_type = pandas_available_type + series_available_type + numpy_available_type
    ...
    if set_mode == 'DataFrame':
        index_size = range(len(data.index))
    ...
    return reformat_value_array \
        , reformat_index_array


# 2차원 시계열 데이터(ndArray)를 3차원 ndArray로 변환하는 Tool
def convert_numpy_to_numpy(array
                           , seq_len
                           , timestep):
    # 오류 검사
    # input data는 DataFrame혹은 Series 이어야 합니다.
    available_type = [np.array([]).__class__, pd.Series([]).__class__]
    if array.__class__ not in available_type:
...
class data_format_check:
    ...
    def check_type(self):
        data_frame_check = \
            self.data.__class__ == pd.DataFrame([]).__class__
        series_check = \
            self.data.__class__ == pd.Series([]).__class__
        numpy_check = \
            self.data.__class__ == np.array([]).__class__

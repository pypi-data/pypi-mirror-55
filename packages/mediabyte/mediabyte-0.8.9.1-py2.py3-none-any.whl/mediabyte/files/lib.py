...
class Convert():
    ...
    def regex_search(search_term, save_filter_as=""):
        ...
        if filter_check:
            for item in list(set(history)):
                # filter out old-style Mixtapes
                if '.y.' not in item:
                    m = re.search(filter_check, item)
                    if m:
                        result.append(item)
            if len(result) > 0:
        ...
        if save_filter_as:
            foldername = cnf.package_path + cnf.os_sep + 'files' + cnf.os_sep + 'filters.json'

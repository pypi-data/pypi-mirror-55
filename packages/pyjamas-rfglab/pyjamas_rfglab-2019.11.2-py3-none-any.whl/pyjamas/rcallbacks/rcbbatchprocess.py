...
class RCBBatchProcess(RCallback):
    ...
    def _combine_measured_data(self) -> pd.DataFrame:
        ...
        for ii, annotations_file in enumerate(self.dirs1 + self.dirs2):
            ...
            x, flag = leastsq(RUtils.residuals, (areas[max_area_ind], 1.0, areas[-1]), args=(RUtils.func_exp, areas[max_area_ind:], times[max_area_ind:]))
            all_data['area_exp_rate_constant'].values[min_ind + index_list] = x[1]
    ...
    def _plot_data(self, all_data: pd.DataFrame) -> bool:
        ...
        n_groups: int = len(numpy.unique(all_data['exp_group']))
        ...
        for a_measurement in self.measurement_list:
            ...
            ax = sns.lineplot(x='time', y=a_measurement, hue='exp_group', data=all_data, estimator=None,
                              units='exp_idx', lw=self.line_width, legend='full', palette=a_palette)
            ...
            for a_group in self.group_labels:
                ...
                n_exp: int = len(numpy.unique(thedata['exp_idx']).astype(int))

                plt.figure()
                ax = sns.lineplot(x='time', y=a_measurement, hue='exp_idx', data=thedata,
                                  estimator=None, units='exp_idx', legend=False, lw=self.line_width,
        ...
        for a_measurement in self.box_plot_measurements:
            ...
            # value per image.
            thedata = all_data.groupby(['exp_group', 'exp_idx'], as_index=False).first()
            ...
            if self.plot_style_value == 'box':
                ...
                ax = sns.boxplot(x='exp_group', y=a_measurement, data=thedata, hue='exp_group', dodge=False, whis=100000, palette=a_palette, order=self.group_labels)
            elif self.plot_style_value == 'violin':
                ax = sns.violinplot(x='exp_group', y=a_measurement, data=thedata, hue='exp_group')

            ax.legend(frameon=False)

            ax = sns.stripplot(x='exp_group', y=a_measurement, data=thedata, color='k', alpha=0.75, size=6, dodge=False, jitter=0.05)
            ax.set_ylim([min(ax.dataLim.ymin - .05 * ax.dataLim.ymin, 0),
                         ax.dataLim.ymax + .05 * ax.dataLim.ymax])

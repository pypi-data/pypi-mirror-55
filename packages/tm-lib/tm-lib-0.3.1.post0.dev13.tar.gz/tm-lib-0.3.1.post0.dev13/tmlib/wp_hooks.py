...
class Events:
    ...
    def subscription_created(self, data):
        ...
        self.logger.info("subscription_id: %s", subscription_id)
        subscription_data = self._get_subscription_id(subscription_id)
        self.logger.info("subscription_data: %s", subscription_data)
        member_id = data.get("member").get("id")
        self.logger.info("member_id: %s", member_id)
        member_data = self._get_member_data(member_id)
        self.logger.info("member_data: %s", member_data)
        minion_name, minion_edition = self._get_minions_data(subscription_data)
        self.logger.info("minion_name: %s, minion_edition: %s", minion_name, minion_edition)
        ...
        self.logger.info("account_number: %s", account_number)
...
def trigger():
    ...
    logger.info('*' * 100)
    logger.info(event)
    logger.info(json.dumps(hook_data, indent=2))
    logger.info('*' * 100)

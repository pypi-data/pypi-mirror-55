...
class Prototype:
    ...
    def load(self, x):
        if isinstance(x, str):
            ...
            self.root = read_and_write_basic.read_json(x)
        else:
            # 说明 x 是一个字典
            self.root = x
    ...
    def update(self, x):
        if isinstance(x, str):
            self.root.update(read_and_write_basic.read_json(x))
        else:
            self.root.update(x)
    
    def save(self, path):
        read_and_write_basic.write_json(self.root, path)
...
def simple_lexicon_generate(i='../temp/in.txt', o='../temp/out.json'):
    word_list = read_and_write_basic.read_file_by_line(i)
    atrie = Trie()
    atrie.add(word_list)
    atrie.save(o)

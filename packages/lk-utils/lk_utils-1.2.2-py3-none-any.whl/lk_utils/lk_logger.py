...
class CallerFinder:
    ...
    def find_caller_by_hierarchy(self, hierarchy):
        ...
        if context.code_context:
            srcln = context.code_context[0].strip()
        else:
            srcln = ''
...
class MsgRecorder:
    ...
    def show_important_messages(self, show_details=True, output='console'):
        """
        NOTE: 不显示 DEBUG 级别和 TEMPRINT 的详细信息 (即便 show_details=True).
        """
            ...
            for msg_chunk in self.tag_messages.values():  # type: dict
                ...
                for tag, msg_list in msg_chunk.items():
                    if ignore.findall(tag):
                        continue
                    msg_container.extend(sorted(msg_list))
    ...
    def dump_log(self, output, launch_path):
        currtime = time()
        timestamp = generate_timestamp('_ymd_hns', currtime)
        
        if output.endswith('.txt'):
            # 说明 output 是一个文件路径.
            output = output.replace('.txt', '{}.txt'.format(timestamp))
        else:
            # 说明 output 是一个目录路径, 比如说是 '../log/'
            assert launch_path, 'cannot accept an empty string, did you mean ' \
                                'a current dir ("./")?'
            filename = splitpath(launch_path)[1].replace(
                '.py', '{}.txt'.format(timestamp)
            )
            """
            当 output 是一个目录时, 则默认使用启动文件的文件名作为生成的日志的文
            件名. 启动文件由 LKLogger.launch_path 提供.
...
log dumped at {generate_timestamp('y-m-d h:n:s', currtime)}.
...
class AstAnalyser:
    ...
    def mask_text(self, text: str):
        ...
        的 col_offset 计算错误. 该问题在 https://bugs.python.org/issue21295 中有
        所体现, 并得到了一些解释:
...
class LKLogger:
    ...
    注: 本类中的所有 h = 'self' 均指向 log 函数的直接调用者 (direct caller).
        ...
        LKLogger.log(), 则 over() 是直接调用者, fx() 是间接调用者. (因此你可以看
        到, 为了让指向到达外部函数 fx(), over() 函数中特意将层级参数上调为
        'parent' 了)
    """
    log_enable = True  # a global logger controller
    lite_mode = False  # TODO: more works need to be done.
    
    direct_caller = None
    _self = 'self'  # direct caller hierarchy. 绝大多数情况下, 此值是固定不变的
    # (始终为 'self'); 少数情况下, 例如开发者想要特意将 direct caller 指向其他调
    # 用者, 则可以操作此变量. 目前操作此变量的函数只有 LKLogger 内部的方法:
    ...
    def _base_print(self, *msg, caller='log', **kwargs):
        ...
        count = self._count_up(is_count_up)
        tag = kwargs.get('tag', '')
    ...
    def log(self, *data, h='self'):
        ...
        return self._base_print(*data, caller='log')
    ...
    def loga(self, *data, h='self'):
        ...
        return self._base_print(*msg, caller='loga')
    ...
    def logd(self, *data, style='-', length=64, h='self'):
        ...
        return self._base_print(*msg, caller='logd',
    ...
    def logt(self, tag, *data, h='self'):
        ...
        return self._base_print(*msg, caller='logt', tag=tag)
    ...
    def logp(self, *data, h='self'):
        ...
        _msg = self._organize_messages(data, caller='logp', h=h)
        msg = []
        for m, d in zip(_msg, data):
    ...
    def logx(self, *data, h='self'):
        ...
        return self._base_print(*data, caller='logx')
        # count_up=True
    ...
    def logax(self, *data, h='self'):
        ...
        return self._base_print(*msg, caller='logax')
    ...
    def logdx(self, *data, style='-', length=64, h='self'):
        ...
        return self._base_print(*msg, caller='logdx',
                                divide_line=style * length)
    ...
    def logtx(self, tag, *data, h='self'):
        ...
        return self._base_print(*msg, caller='logtx', tag=tag)
    ...
    def logdt(self, tag, *data, style='-', length=64, h='self'):
        ...
        return self._base_print(*msg, caller='logdt', tag=tag,
                                divide_line=style * length)
    
    def logdtx(self, tag, *data, style='-', length=64, h='self'):
        msg = self._organize_messages(data, 'logdtx', h)
        return self._base_print(*msg, caller='logdtx', tag=tag,
                                divide_line=style * length)
    ...
    def enum(self, x):  # FIXME: not ready to use.
        direct_caller = self.finder.find_caller_by_hierarchy(2)
        
        try:
            denominator = str(len(x))
        ...
        for index, data in enumerate(x):
            ...
            head = '[{}/{}]'.format(
                str(index + 1).zfill(len(denominator)), denominator
            ) if denominator else '[{}]'.format(index + 1)
            self._base_print(head)
            yield index, data
    
    def _count_up(self, is_count_up):
        if not is_count_up:
            return ''
    ...
    # ------------------------------------------------ advanced vars' analysing
    
    def _organize_messages(self, data, caller, h) -> tuple:
        """
        analyse source code line, get variants' names and mix them with data
        values.
        ARGS:
            data (tuple):
            caller (str): 'log' | 'loga' | 'logt' | ...
            h (str, int)
        IN: data: e.g. (13, 22, {'C': 'CC'}, "hello")
            source_code_line (str): e.g. 'lk.loga(A, B, C, "hello")'
        OT: (tuple) e.g. ("A = 13", "B = 22", "C = {'C': 'CC'}", "hello")
                each element is str
        """
    ...
    def _get_var_names(self, caller, h):
        """
        analyse source code line and extract variants' names from it.
        ARGS:
            caller (str): 'log' | 'loga' | 'logt' | ...
            h
        ...
        IN: e.g. 'lk.log(A, B, C[D], "hello")'
        OT: e.g. ['A', 'B', 'C[D]', "hello"]
                docstrings see self._split_vars_from_srcln()
        """
        ...
        try:
            ...
            if var_names is not None:
                if caller in ('logt', 'logtx', 'logdt', 'logdtx'):
                    ...
                    logt 有个特殊的地方在于, 它的第一个参数是 tag. 我们必须把它
                    剔除.
                    """
                    var_names.pop(0)

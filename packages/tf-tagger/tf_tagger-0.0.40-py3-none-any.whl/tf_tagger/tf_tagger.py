...
class TFTagger:
    ...
    def build(self):
        model = TaggerModel(embedding_size=self.embedding_size,
                            ...
                            loss=self.loss)
        model.build(tf.TensorShape([None, self.bert_max_length]))
        model.summary()
        return model
            ...
            for x, tags in pbar:
                with tf.GradientTape() as tape:
                    ...
                    if self.clip_value > 0.:
                        gradients = [
                            tf.clip_by_value(g, -self.clip_value, self.clip_value) if g is not None else g
                            for g in gradients
                        ]
                    if self.clip_norm > 0.:
                        gradients = [
                            tf.clip_by_norm(g, self.clip_norm) if g is not None else g
                            for g in gradients
                        ]

...
__all__ = ['reset', 'load_package', 'logger', 'parameter']
    ...
    for k, v in settings.items():
        try:
            if hasattr(target_settings, k):
                ...
                logger.error('AttributeError {target} has no attribute {k}'.format(target=target, k=k))
        except Exception as e:
            logger.error('key error:%s' % k)
            logger.error(e)
...
def _load_from_ini_py3(ini, default_section=_DEFAULT_SECTION):
    ...
    cf.read(ini, encoding=_DEFAULT_ENCODING)
    settings = OrderedDict()

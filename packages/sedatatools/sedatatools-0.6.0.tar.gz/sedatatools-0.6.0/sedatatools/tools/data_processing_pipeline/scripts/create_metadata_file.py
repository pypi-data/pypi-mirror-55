...
def get_table_metadata_from_db(config):
    ...
    cursor.execute(
        f"SELECT * FROM table_names where projectYear = {config['projectYear']}",
    )
    ...
    for ext in extension:
        for el in table_names:
            if ext in el[0]:
                ...
                meta_data.append(
                    [el[0].replace(ext, '')[el[0].index('_') + 1:], el[1]],
                )

    for i in meta_data:
        meta_table_dictionary[i[1]] = i[0].replace('_', ' ')
...
def get_tables_from_db(config):
    ...
    cursor.execute(
        ...
        f"and left(name,1) <> '_' "
        f"and name like '%{str(config['projectYear'])}%' "
    ...
    # conn.close()  TODO check if this will work when you don't close the connection until the end

    dict_tables_and_vars_unique = {}
    for table_name, table_variables in dict_tables_and_vars.items():
        values = []
        pos_t = [index for index, char in enumerate(table_name) if char == '_'][1] + 1
        for variable in table_variables:
            # skip system required names
            if len([index for index, char in enumerate(variable[0]) if char == '_']) < 3:
                continue
            pos_v = [index for index, char in enumerate(variable[0]) if char == '_'][2] + 1
            # logger.info(table_name[pos_t:])
            # logger.info(variable[0][pos_v:])
            values.append([variable[0][pos_v:], variable[1]])
        dict_tables_and_vars_unique[table_name[pos_t:]] = values

    dict_tables_and_vars = dict(sorted(dict_tables_and_vars.items()))

    return dict_tables_and_vars


def get_geo_type(geo_level_info):  # number of geo types
    """
    ...
    # getGeoNesting(geoLevelInfoFilePath)
...
def get_geo_survey_datasets(config):
    """
    ...
    for summary_level in config['geoLevelInformation']:
        ...
        result.append(e.dataset(
            ...
            IsCached='false',
            DbTableNamePrefix=config['projectId'] + '_' + summary_level[0] + '_',
            DbPrimaryKey=get_table_geo_columns(summary_level[0] + '_FIPS', config['geoLevelInformation']),
            DbCopyCount='1',
        ))
...
def get_variables(variables, variable_description):
    """
    ...
    for i in variables:
        ...
        if len([index for index, character in enumerate(i[0]) if chr == '_']) == 0:
            continue
        ...
        if i[0][pos:] not in variable_description.keys():
            logger.info("Warning: Undefined variable found:", i[0], i[0][pos:])
            continue
...
def get_tables(config):
    ...
    for table_name, table_variables in table_list.items():
        ...
        meta_table_name = table_name[:cut_pos[0]] + '_' + table_name[cut_pos[-1] + 1:]  # this defines how table Id will be presented in metadata
...
def get_geo_id_variables(geo_level_info):
    """
...
def create_metadata_file_structure(config):
    e = ElementMaker()
    ...
    page = e.survey(
        ...
        e.geoTypes(
            *get_geo_type(config['geoLevelInformation'])
        ),
        e.GeoSurveyDataset(
            ...
            e.datasets(
                *get_geo_survey_datasets(config)
            ),
            e.iterations(

            ),
            *get_geo_id_tables(config['geoLevelInformation']),
            GUID=str(uuid.uuid4()),
...
def write_metadata(page, config):
    ...
    tree.write(config['metadataOutputPath'], pretty_print=True)
    logger.info(f"Writing to: {config['metadataOutputPath']}")
    logger.info("Everything finished successfully!!!")


def create_metadata_file(configuration_path='../config.yml'):
    configuration = prepare_environment(configuration_path)
    content = create_metadata_file_structure(configuration)

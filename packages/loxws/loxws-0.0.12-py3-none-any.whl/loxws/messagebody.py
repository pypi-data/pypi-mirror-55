                    ...
                    try:
                        ...
                        if device.device_type == "ClimateController":
                            ...
                            if stateName == "controls":
                                ...
                                for con in controls:
                                    _LOGGER.debug("  climate control - uuid: {0} demand: {1}".format(con.uuid, con.demand))
                                    if con.uuid in config_data.fieldmap:
                                        _LOGGER.debug("A")
                                        mapentry2 = config_data.fieldmap[con.uuid]
                                        _LOGGER.debug("B")
                                        device2 = mapentry["device"]
                                        _LOGGER.debug("C")
                                        device2.set_value("demand", con.demand)
                                        _LOGGER.debug("D")

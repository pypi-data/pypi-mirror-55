...
class Component:
    ...
    def load_package(self):
        ...
        if self.type in ['board', 'chip', 'solution']:
            self.__dict__[self.type] = pack.__dict__[self.type]
    ...
    def variable_convert(self, varList):
        ...
        if self.type == 'board':
            if self.board.ld_script:
                ld_script = varList.convert(self.board.ld_script)
                if ld_script:
                    self.board.ld_script = os.path.join(self.path, ld_script)
        elif self.type == 'chip':
            if self.chip.ld_script:
                ld_script = varList.convert(self.chip.ld_script)
                if ld_script:
                    self.chip.ld_script = os.path.join(self.path, ld_script)
        elif self.type == 'solution':
            if self.solution.ld_script:
                ld_script = varList.convert(self.solution.ld_script)
                if ld_script:
                    self.solution.ld_script = os.path.join(self.path, ld_script)

...
class DFAInputSpec(TraitedSpec):
    bold = InputMultiPath(File(exists=True), mandatory=True, desc='input bold')
    brainmask = InputMultiPath(File(exists=True), mandatory=False, desc='brainmask in bold space')
    csv = InputMultiPath(File(exists=True), mandatory=False, desc='input csv')
...
class DFA(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        img = nibabel.load(self.inputs.bold)
        tr = img.header.get('pixdim')[4]

        use_bold = self.inputs.brainmask is not None

        if use_bold:
            mask_img = nibabel.load(self.inputs.brainmask)
            data = nilearn.masking.apply_mask(imgs=img,mask_img=mask_img)
        else:
            data = pandas.read_csv(self.inputs.csv)
        ...
        if use_bold:
            out_hurst = fname_presuffix(self.inputs.bold, suffix='_hurst')
            out_cis = fname_presuffix(self.inputs.bold, suffix='_hurst_ci')
            out_r2s = fname_presuffix(self.inputs.bold, suffix='_hurst_r2')

            _bold_native_masked_derivative(bold_img=img, mask_img=mask_img, derivative_data=h, out_file=out_hurst)
            _bold_native_masked_derivative(bold_img=img, mask_img=mask_img, derivative_data=hci, out_file=out_cis)

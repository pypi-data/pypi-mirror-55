...
def fmt_ria(ria, verbose=True, mip=False):
    ...
    if config.REPR_VERBOSITY is HIGH:
        ...
        repertoire = '\nRepertoire:\n{}'.format(
            indent(fmt_repertoire(ria.repertoire, ria.purview, ria.node_labels)))
        partitioned_repertoire = '\nPartitioned repertoire:\n{}'.format(
            indent(fmt_repertoire(ria.partitioned_repertoire, ria.purview, ria.node_labels)))
    else:
        partition = ''
        repertoire = ''
        partitioned_repertoire = ''
...
def fmt_repertoire(r,p,l):
    ...
    # Lines: '001     .25'
    for state in utils.all_states(r.ndim,p,l):

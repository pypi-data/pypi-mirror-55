...
def list_variables(package,attribute='',vars=None):
     """
...
def list_package_variables(package,attribute='',vars=None):
    """
        ...
        vars=[varlist] selection limited to varlist
    """
    return list_variables(package,attribute=attribute,vars=vars)
...
def list_variables_apropos(s,verbose=False,vars=None):
     """
     ...
     for p in packages:
        for var in p.varlist():
            if s.upper() in p.getvardoc(var).upper(): 
               if verbose: print(p.getvardoc(var))
               ret.append(var)
     if vars == None:
         return ret
     else: 
...
def list_variables_regex(r,verbose=False,vars=None):
     """

...
def rundt(dtreal, nfe_tot=0, savefn='../solutions/savedt', dt_tot=0, ii1max=500, ii2max=5, ftol_dt=1e-5, itermx=7, rlx=0.9, n_stor=0,
    ...
    if (bbb.iterm == 1):
        ...
        # compensates dtreal divided by mult_dt below
        bbb.dtreal = bbb.dtreal*bbb.mult_dt
    else:
        print("*---------------------------------------------------------*")
    ...
    if (bbb.ipt == 1 and bbb.isteon == 1): 	# set ipt to te(nx,iysptrx+1) if no user value
        # note: ipt is local, bbb.ipt global
        ipt = bbb.idxte[com.nx-1, com.iysptrx]
    ...
    if (bbb.iterm == 1):  # successful initial run with dtreal
        ...
        # causes dt=dt/mult_dt after irev loop
    ...
    # adjust for mult. to follow; mult_dt in rdinitdt
    bbb.dtreal = bbb.dtreal/bbb.mult_dt
    ...
    for ii1 in range(1, bbb.ii1max+1):
        ...
        if (bbb.irev == 0):
            ...
            # first move forward after reduction
            bbb.dtreal = min(3*bbb.dtreal, bbb.t_stop)
            bbb.dtphi = bbb.rdtphidtr*bbb.dtreal
            if (bbb.ismfnkauto == 1 and bbb.dtreal > bbb.dtmfnk3):
                bbb.mfnksol = -3
        ...
        print('--------------------------------------------------------------------')
        print(' ')
        print('*** Number time-step changes = ',
              ii1, ' New time-step = ', bbb.dtreal)
        print('--------------------------------------------------------------------')
        ...
        if (ii1 > 1 or bbb.initjac == 1):  # first time calc Jac if initjac=1
            ...
            if (bbb.iterm == 1):
                ...
                bbb.pandf1(-1, -1, 0, bbb.neq, 1., bbb.yl, bbb.yldot)
                fnrm_old = sqrt(
                    sum((bbb.yldot[0:neq-1]*bbb.sfscal[0:neq-1])**2))
        ...
        for ii2 in range(1, bbb.ii2max+1):  # take ii2max steps at the present time-step
            if (bbb.iterm == 1):
                ...
                if (bbb.iterm == 1):
                    ...
                    fnrm_old = sqrt(
                        sum((bbb.yldot[0:neq-1]*bbb.sfscal[0:neq-1])**2))
                    print("Total time = ", bbb.dt_tot,
                          "; Timestep = ", bbb.dtreal)
                    print("variable index ipt = ", ipt,
                          " bbb.yl[ipt] = ", bbb.yl[ipt])
                    dtreal_sav = bbb.dtreal
                    bbb.dt_tot = bbb.dt_tot + bbb.dtreal
                    nfe_tot = nfe_tot + bbb.nfe[0, 0]
    ...
    # Store variables if a storage time has been crossed
       ...
       # End of storage section
        ...
        if (bbb.dt_tot >= bbb.t_stop or fnrm_old < bbb.ftol_min):
            break   # need for both loops
        bbb.irev = bbb.irev-1

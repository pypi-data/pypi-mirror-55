...
def search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True, quiet=False,
           status=None):
    """
    Issues a query to the Seeq Server to retrieve metadata for signals, conditions, scalars and assets. This metadata
    can be used to retrieve samples, capsules for a particular time range.

    Parameters
    ----------
    query : {dict, pandas.DataFrame}
        A mapping of property / match-criteria pairs. Match criteria uses the same syntax as the Data tab in Seeq.
        ...
        Type                The item type. One of 'Signal', 'Condition', 'Scalar', 'Asset', 'Histogram' or 'Metric'
        ...
        Scoped To           The Seeq ID of the scoped workbook. Is overridden by the 'workbook' argument
        =================== ========================================================================================

    all_properties : bool, default False
        True if all item properties should be retrieved. This currently makes the search operation much slower as
        retrieval of properties for an item requires a separate call.

    workbook : {str, None}
        A path string (with ' >> ' delimiters) or an ID to indicate a workbook to which the scope of the search
        should be limited. Note that globally scoped items will still be returned. The ID for a workbook is visible in
        the URL of Seeq Workbench, directly after the "workbook/" part.

    recursive : bool, default True
        If True, searches that include a Path entry will include items at and below the specified location in an asset
        tree. If False, the query dictionary can only contain a Path entry and only items at the specified level will
        be returned.
    ...
    Examples
    --------
    Search for signals with the name 'Humid' on the asset tree under 'Example >> Cooling Tower 1', retrieving all

...
def push(data=None, *, metadata=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    Successive calls to 'push()' with the same 'metadata' but different 'data' will update the items (rather than
    overwrite them); however, pushing a new sample with the same timestamp as a previous one will overwrite the old one.
    ...
    data : pandas.DataFrame, optional
        A DataFrame that contains the signal or condition data to be pushed. If 'metadata' is also supplied,
        it will have specific formatting requirements depending on the type of data being pushed.

        For signals, 'data' must have a pandas.Timestamp based index with a column for each signal.

        For conditions, 'data' must have an integer index and two pandas.Timestamp columns named 'Capsule Start' and
        'Capsule End'

    metadata : pandas.DataFrame, optional
        A DataFrame that contains the metadata for signals, conditions, scalars, or assets. If 'metadata' is supplied,
        in conjunction with a 'data' DataFrame, it has specific requirements depending on the kind of data supplied.

        For signals, the 'metadata' index (ie, rows) must have the same names as the column names of the 'data'
        DataFrame.

        For conditions, the 'metadata' DataFrame must have only one row with metadata for the condition.
        ...
        ======================== =========================================================== =====================
        Metadata Term            Definition                                                  Applicable Types
        ======================== =========================================================== =====================
        Name                      The name of the signal                                      Si, Sc, C, A
        Description               The description of the signal                               Si, Sc, C, A
        Maximum Interpolation     The maximum interpolation between samples                   Si
        Unit of Measure           The unit of measure of the values, or keys for conditions   Si, Sc, C
        Formula                   The formula for a calculated item                           Si, Sc, C
        Formula Parameters        The parameters for a formula                                Si, Sc, C
        Interpolation Method      The interpolation method between points options are
                                  (Linear, Step, PILinear)                                    Si
        Maximum Duration          The maximum expected duration for a capsule                 C
        Number Format             The formatting string for number, following ECMA-376        Si, Sc
        Path                      The asset tree path where the item's parent asset resides   Si, Sc, C, A
        Asset                     The asset that the item is to be a child of. The asset
                                  must be in the tree at the specified path, or listed in
                                  'metadata' for creation.                                    Si, Sc, C, A
        ======================== =========================================================== =====================

    workbook : {str, None}, default 'Data Lab >> Data Lab Analysis'
        The path to a workbook (in the form of 'Folder >> Path >> Workbook Name') or an ID that all pushed items will be
        'scoped to'. Items scoped to a certain workbook will not be visible/searchable using the data panel in other
        workbooks. If None, items can also be 'globally scoped', meaning that they will be visible/searchable in all
        workbooks. Global scoping should be used judiciously to prevent search results becoming cluttered in all
        workbooks. The ID for a workbook is visible in the URL of Seeq Workbench, directly after the "workbook/" part.

    worksheet : str, default 'From Data Lab'
        The name of a worksheet within the workbook to create/update that will render the data that has been pushed so
        that you can see it in Seeq easily.

    datasource : str, optional
        The name of a datasource in which to contain all the pushed items. By default, all pushed
    ...
    archive : bool, default False
        If 'True', archives any items in the datasource that previously existed but were not part of this push call.
        This can only be used if you also specify a 'datasource' argument.

    type_mismatches : {'raise', 'drop'}, default 'raise'
        If 'raise' (default), any mismatches between the type of the data and its metadata will cause an exception. For
        example, if string data is found in a numeric time series, an error will be raised. If 'drop' is specified, such
        data will be ignored while pushing.

    errors : {'raise', 'catalog'}, default 'raise'
        If 'raise', any errors encountered will cause an exception. If 'catalog', errors will be added to a
        'Push Result' column in the passed-in 'items' DataFrame.

    quiet : bool, default False
        If True, suppresses progress output.

    status : spy.Status, optional
        If supplied, this Status object will be updated as the command progresses.
    ...
    if metadata is not None:
        push_result_df = _push_metadata(metadata, workbook_id, datasource_output, archive, errors, quiet, status)
...
def _push_metadata(metadata, workbook_id, datasource_output, archive, errors, quiet, status):

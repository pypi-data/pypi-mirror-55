...
def ls(config):
    ...
    resp = parsers.get()
    if resp.ok:
        headers = ['Parser', 'Hash', 'Last ran at', 'Last run results', 'Last updated']
        rows = [(p['name'], p['module_hash'], p['last_ran_at'], p['last_run_results'], p['updated_at']) for p in resp.json()]

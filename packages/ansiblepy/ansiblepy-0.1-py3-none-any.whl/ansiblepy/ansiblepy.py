...
class Generate:
    ...
    def generates(self):
        ...
        while decide not in ['P', 'R', 'p', 'r']:
            decide = input("Please provide a valid input to proceed. Playbook/Role [P/R]? ")
        if decide.lower() == 'r':
            self.create_roles()
        else:
            self.final_playbook = '---' + '\n' + '- name: Ansible Playbook' + '\n  hosts: '
            hosts = input("Please enter the host string or IP: ")
            gather_facts = input("Do you want to gather facts? [Y/N]: ")

            while gather_facts not in ['Y','N','y','n']:
                gather_facts = input("Please provide correct input for gather facts? [Y/N]: ")
            ...
            self.final_playbook += '\n...'
            with open(self.base_path + "\\playbook1.yml", "w+") as file:

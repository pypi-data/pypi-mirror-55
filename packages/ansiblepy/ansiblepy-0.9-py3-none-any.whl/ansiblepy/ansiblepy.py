...
class Generate:
    ...
    def __init__(self):
        ...
        """
        self.file_name = 'playbook_' + str(random.randint(1, 999)) + '.yml'
        system = platform.system()
        if system != 'Windows':
            #self.base_path = os.path.join('etc', 'ansible', 'ansibly')
            #self.file_path = '/' + self.base_path + '/playbook_' + str(random.randint(1, 999)) + '.yml'
            self.file_path = os.path.join('etc', 'ansible', 'ansiblepy', self.file_name)
        else:
            #self.base_path = os.getcwd()
            #self.file_path = self.base_path + '\\playbook_' + str(random.randint(1, 999)i) + '.yml'
            self.file_path = os.path.join(os.getcwd(), 'ansiblepy', self.file_name)
    ...
    def get_vars_data(self):
        ...
        var = dict()
        try :
            count = int(six.moves.input("Please enter number of variable definitions [max 10]: "))
        except ValueError as e:
            count = int(six.moves.input("Please provide Integer value for variable definitions [max 10]: "))
        ...
        if count > 10:
            ...
            for i in range(count):
                var_name = six.moves.input("Please enter the Variable Name: ")
                var_value = six.moves.input("Please enter the value for '" + var_name + "': ")
                var[var_name] = var_value
        return var

    def create_roles(self):
        count = int(six.moves.input("Please enter no. of. roles to be created: [Max 5]"))
        if count > 5:
            self.create_roles()
        else:

            for i in range(1, count + 1):
                name = six.moves.input("Please enter the name of role {}: ".format(i))
    ...
    def generates(self):
        ...
        :return:
        """
        decide = six.moves.input("Do you want to create a simple Playbook or Roles? [P/R]")
        while decide not in ['P', 'R', 'p', 'r']:
            decide = six.moves.input("Please provide a valid input to proceed. Playbook/Role [P/R]? ")
        if decide.lower() == 'r':
            self.create_roles()
        else:
            self.final_playbook = '---' + '\n' + '- name: Ansible Playbook' + '\n  hosts: '
            hosts = six.moves.input("Please enter the host string or IP: ")
            gather_facts = six.moves.input("Do you want to gather facts? [Y/N]: ")

            while gather_facts not in ['Y','N','y','n']:
                gather_facts = six.moves.input("Please provide correct input for gather facts? [Y/N]: ")

            vars_in = six.moves.input("Do you want to define variables directly into Playbook ? [Y/N]:")
            ...
            def database():
                ...
                user_input = six.moves.input("Please enter the search keyword for Module:")
            ...
            decision = six.moves.input("Do you want to continue: [Y/N]")

    ...
    if not in_string_or_comment:#This is just for visual purposes, so I can put the lines in a block of code and document ,after_line)it
      ...
      #region misc tweaks
        ...
        if char==';' and after_line==')':
            ...
            buffer.insert_text(';')
            return True
        if char in '- \n' and starts_with_any(before_line.strip(),'nonlocal ','global '):
...
def load_python_bindings(python_input):
            ...
            def go(char):
                ...
                def _(event):
                    ...
                    if True:#not '"' in after_line and not "'" in after_line:
                        ...
                        header_commands={'\\co':'copy','\\da':'delete all','\\pa':'paste','\\ya':'yapf autoformat','\\gg':'go to top','\\GG':'go to bottom'}
    ...
    def _(event):# Spacebar event handle spacebar
        ...
        try:
            ...
            elif ((not '\n' in before and not after and re.fullmatch(r'\w+',before_line)) #space-completions are enabled if we're typing a function out as a comand, because then we know we don't have to worry about messy edge cases. space-functions are really convenient...\

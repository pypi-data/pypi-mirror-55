...
from scrapper_core.exception import RetryTaskExitError, RetryTaskSkipAuthError
from scrapper_core.types import ScrapperData
...
class MyRetryTaskExitErrorScrapper(BlueprintScrapper):
    ...
    def actions(self):
        return [
            'tests.scrapper_core.test_custom_error.MyActionWithRetryTaskExitError1',
            'tests.scrapper_core.test_custom_error.MyActionWithRetryTaskExitError2'
        ]
...
class MyRetryTaskSkipAuthErrorScrapper(BlueprintScrapper):
    ...
    def actions(self):
        return [
            'tests.scrapper_core.test_custom_error.MyActionWithRetryTaskSkipAuthError1',
            'tests.scrapper_core.test_custom_error.MyActionWithRetryTaskSkipAuthError2'
        ]
...
def test_scrapper_with_retry_task_exit_error():
    res = Scrapper.run(MyRetryTaskExitErrorScrapper(ScrapperData('naver_place', {'place_id': '123123'}))
                       , json_output=False)
...
def test_scrapper_with_retry_task_skip_auth_error():
    res = Scrapper.run(MyRetryTaskSkipAuthErrorScrapper(ScrapperData('naver_place', {'place_id': '123123'}))
                       , json_output=False)
    global retry_task_skip_auth_error_counter
    ...
    assert retry_task_skip_auth_error_counter == CONFIG.get('RETRY_TASK_COUNT')

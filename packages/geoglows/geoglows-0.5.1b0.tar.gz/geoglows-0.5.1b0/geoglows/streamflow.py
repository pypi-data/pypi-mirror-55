...
def forecast_stats(reach_id=None, lat=None, lon=None, api_source=BYU_ENDPOINT, api_key=None, return_format='pandas'):
    ...
    if return_format == 'pandas':
        return pandas.read_csv(StringIO(data)), reach_id
    elif return_format == 'json':
        return json.loads(data), reach_id
    elif return_format == 'waterml':
        return data, reach_id
...
def return_periods(reach_id=None, lat=None, lon=None, api_source=BYU_ENDPOINT, api_key=None, return_format='pandas'):
    ...
    if return_format == 'pandas':
        return pandas.read_csv(StringIO(data), index_col='return period'), reach_id
    elif return_format == 'json':
        return json.loads(data), reach_id
    elif return_format == 'waterml':
        return data, reach_id
...
def latlon_to_reach(lat, lon):
    ...
    """
    # create a shapely point for the querying (it should be x/lon then y/lat but the csvs are backwards so this is too)

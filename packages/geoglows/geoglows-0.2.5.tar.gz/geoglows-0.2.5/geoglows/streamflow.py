...
def forecast_plot(stats, rperiods, reach_id, outformat='plotly'):
    ...
    plot_data = {
        'reach_id': reach_id,
        'x_ensembles': stats[['datetime', 'mean (m3/s)']].dropna(axis=0).tolist(),
        'x_hires': stats[['datetime', 'high_res (m3/s)']].dropna(axis=0).tolist(),
...
def seasonal_plot(daily, reach_id, outformat='plotly'):
    ...
    plot_data = {
        'reach_id': reach_id,
        'x': pandas.to_datetime(daily['day'] + 1, format='%j').tolist(),
        'y': daily['streamflow_avg (m3/s)'].tolist(),
    }

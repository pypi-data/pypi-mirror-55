...
def add_check_csrf_hook(run_hook, timer):
    ...
    CSRF tokens.
    """
    # pyramid 1.8 and later combine CSRF in `pyramid.csrf`
    try:
        import pyramid.csrf as target
    except ImportError:
        # For older versions of pyramid, we use the check in `pyramid.session`
        import pyramid.session as target
    ...
    # Pyramid > 1.6 also has a BadCSRFOrigin exception
    try:
        from pyramid.exceptions import BadCSRFOrigin
    except ImportError:
        class BadCSRFOrigin(Exception):
            """Dummy Exception for old versions. Will never be raised."""


    # wrap 'check_csrf_token' function
    orig_check_csrf_token = target.check_csrf_token

    def wrapped_check_csrf_token(*args, **kwargs):
        log.debug(
            "pyramid.*.check_csrf_token"
            "(%(args)s, %(kwargs)s)", {
                "args": args,
                "kwargs": kwargs,
                })
        try:
            ...
            meta = {
                "valid": False,
                "reason": "BadCSRFOrigin from CSRF token",
            }
            raise
        finally:
    ...
    target.check_csrf_token = wrapped_check_csrf_token

    # The `CheckCSRFTokenPredicate` class holds a reference to the
    # `check_csrf_token` function. Patch it here.
    try:
        import pyramid.predicates as predicate_target
    except ImportError:
        # On versions of pyramid < 1.10, `predicates` is under `config`
        import pyramid.config.predicates as predicate_target

    predicate_target.CheckCSRFTokenPredicate.check_csrf_token = (

...
def _is_builtin_function(original):
    if (isinstance(original, BuiltinFunctionType)
            and not hasattr(original, "__get__")):
        return True
    else:
        return False
...
def monkeypatch(target_or_base, target=None, timer=None, report_name=None,
    ...
    def wrapper(wrapped):
        ...
        is_builtin = _is_builtin_function(original)
        ...
        # A builtin function can not have a __get__ or it can become bound
        # when set on a class
        if is_builtin:
            new_wrapped = functools.partial(new_wrapped)
            setattr(new_wrapped, "immunio_unwrap", unwrap)
            setattr(new_wrapped, "_immunio_original", original)
        ...
        # Try to update the wrapper again to add anything removed above.
        try:
            functools.update_wrapper(new_wrapped, original)
        except AttributeError:
            # This can fail for Python 2 because of

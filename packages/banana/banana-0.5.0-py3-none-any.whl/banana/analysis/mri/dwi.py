...
from arcana.data import FilesetSpec, InputFilesetSpec, OutputFilesetSpec
...
class DwiAnalysis(EpiSeriesAnalysis, metaclass=AnalysisMetaClass):
    ...
    add_data_specs = [
        ...
        InputFilesetSpec('reverse_phase', STD_IMAGE_FORMATS, optional=True,
                         desc=("A reference image with reversed phase encoding"
                               " compared to the main DW 'series' used to "
                               "correct eddy current distortions during "
                               "preprocessing")),
        ...
        FilesetSpec('noise_residual', mrtrix_image_format,
                    'preprocess_pipeline',
                    desc=("")),
        OutputFilesetSpec('tensor', nifti_gz_format, 'tensor_pipeline',
                          desc=("\"Apparent\" Diffusion Tensor (DTI) per "
                                "voxel")),
        OutputFilesetSpec('fa', nifti_gz_format, 'tensor_metrics_pipeline',
                          desc=("Fractional Anisotropy (FA) of apparent "
                                "Diffusion Tensor (DTI) per voxel")),
        OutputFilesetSpec('adc', nifti_gz_format, 'tensor_metrics_pipeline',
                          desc=("Apparent diffusion coefficient (ADC) per "
                                "voxel")),
        ...
        FilesetSpec('avg_response', text_format, 'average_response_pipeline',
                    desc=("")),
        OutputFilesetSpec('wm_odf', mrtrix_image_format, 'fod_pipeline',
                          desc=("Distribution of white matter fibre "
        ...
        FilesetSpec('norm_intens_wm_mask', mrtrix_image_format,
                    'intensity_normalisation_pipeline', frequency='per_dataset',
                    desc=("")),
        OutputFilesetSpec('global_tracks', mrtrix_track_format,
                          'global_tracking_pipeline',
                          desc=("Streamline tracks generated from uniformly "
                                "distributed seeds within the white matter")),
        FilesetSpec('wm_mask', mrtrix_image_format,
                    'global_tracking_pipeline',
                    desc=("")),
        OutputFilesetSpec(
            'connectome', csv_format, 'connectome_pipeline',
            desc=("Structural connectome: matrix representation of the "
                  "fraction of streamlines connecting anatomical from "
                  "global tracking"))]

    add_param_specs = [
        # ParamSpec('multi_tissue', True,
        #           desc=("Whether to use multi tissue ")),
        ParamSpec('preproc_pe_dir', None, dtype=str,
                  desc=("Phase encoding direction ('RL' - right/left, "
                        "'PA' - posterior/anterior, "
                        "'IS' - inferior/superior)")),
        ParamSpec('tbss_skel_thresh', 0.2,
                  desc=("Threshold for TBSS skeleton")),
        # ParamSpec('fsl_mask_f', 0.25,
        #           desc=("f")),
        ParamSpec('bet_robust', True,
                  desc=("Whether to use 'robust' option BET")),
        ParamSpec('bet_f_threshold', 0.2,
                  desc=("Fractional intensity threshold for BET")),
        ParamSpec('bet_reduce_bias', False,
                  desc=("Whether to use 'reduce bias' option BET")),
        ParamSpec('num_global_tracks', int(1e9),
                  desc=("Number of global tracks to generate")),
        ParamSpec('global_tracks_cutoff', 0.05,
                  desc=("Cut off value for FOD / fixel size where to stop "
                        "tracking")),
        SwitchSpec('preproc_denoise', False,
                   desc=("Denoise the preprocessed image")),
        SwitchSpec('response_algorithm', 'tax',
                   ('tax', 'dhollander', 'msmt_5tt'),
                   desc=("Algorithm to use for estimating the per voxel "
                         "diffusion response")),
        SwitchSpec('fod_algorithm', 'csd', ('csd', 'msmt_csd'),
                   desc=("Algorithm to use for constrained spherical "
                         "decovolution (CSD)")),
        MriAnalysis.param_spec('bet_method').with_new_choices('mrtrix'),
        SwitchSpec('reorient_to_std', False,
                   desc=("Whether to reorientate the original image to "
                         "FSL standard view"))]
...
class DwiRefAnalysis(EpiAnalysis, metaclass=AnalysisMetaClass):
    """
    A special analysis used in the MR-PET motion correction algorithm to
    perform distortion correction on the reverse-phase/reference b0
    scans by flipping it around and using the DWI series as the reference
    """

...
from arcana.analysis import ParamSpec
...
class BoldAnalysis(EpiSeriesAnalysis, metaclass=AnalysisMetaClass):
    ...
    desc = "BOLD functional MRI contrast"
    ...
    add_data_specs = [
        ...
        OutputFilesetSpec(
            'melodic_ica', zip_format, 'single_subject_melodic_pipeline',
            desc=("Independent Component Analysis (ICA) decomposition of "
                  "signal into different spatial and temporal components")),
        ...
        FilesetSpec(
            'smoothed_ts', nifti_gz_format, 'smoothing_pipeline',
            desc="Smoothed time-series")]
...
class MultiBoldMixin(MultiAnalysis):
    ...
    def fmri_substudies(cls):
        ...
        fmri_substudies = []
        for subcomp_spec in cls.subcomp_specs():
            try:
                cls.data_spec(subcomp_spec.inverse_map('fix_dir'))
            except ArcanaNameError:
                continue  # Sub analysis is not a Fmri analysis
            else:
                fmri_substudies.append(subcomp_spec.name)
    ...
    def fix_training_pipeline(self, **name_maps):
        ...
        merge_label_files = pipeline.add(
            'merge_label_files',
            NiPypeMerge(num_fix_dirs))
        for i, subcomp_name in enumerate(self.bold_substudies(), start=1):
            spec = self.subcomp_spec(subcomp_name)
...
def create_multi_fmri_class(name, t1, epis, epi_number, echo_spacing,
    ...
    analysis_specs = [SubCompSpec('t1', T1wAnalysis)]
    ref_spec = {'t1_brain': 'coreg_ref_brain'}
    inputs.append(FilesetFilter('t1_magnitude', t1, dicom_format,
                                is_regex=True, order=0))
    epi_refspec = ref_spec.copy()
    epi_refspec.update({'t1_wm_seg': 'coreg_ref_wmseg',
                        't1_preproc': 'coreg_ref',
                        'train_data': 'train_data'})
    analysis_specs.append(SubCompSpec('epi_0', BoldAnalysis, epi_refspec))
    if epi_number > 1:
        epi_refspec.update({'t1_wm_seg': 'coreg_ref_wmseg',
                            't1_preproc': 'coreg_ref',
                            'train_data': 'train_data',
        ...
        analysis_specs.extend(SubCompSpec('epi_{}'.format(i), BoldAnalysis,
                                        epi_refspec)
    ...
    for i in range(epi_number):
        inputs.append(FilesetFilter(
            'epi_{}_series'.format(i), epis, dicom_format, order=i,
            is_regex=True))
#     inputs.extend(FilesetFilter(
...
#         for i in range(epi_number))
    ...
    dct['add_subcomp_specs'] = analysis_specs

...
from arcana.data import (
    FilesetSpec, FieldSpec, InputFilesetSpec, OutputFilesetSpec)
from banana.analysis import Analysis, AnalysisMetaClass
...
class MriAnalysis(Analysis, metaclass=AnalysisMetaClass):
    ...
    add_data_specs = [
        ...
        OutputFilesetSpec('brain_mask', nifti_gz_format,
                          'brain_extraction_pipeline',
        ...
        InputFilesetSpec('template_brain', STD_IMAGE_FORMATS,
                         ...
                         default=FslReferenceData(
                             ...
                             resolution='mni_tmpl_resolution',
                             dataset='brain'),
                         desc=("Brain-extracted template used for non-linear "
                               "registration")),
        InputFilesetSpec('template_mask', STD_IMAGE_FORMATS,
                         ...
                         default=FslReferenceData(
                             ...
                             resolution='mni_tmpl_resolution',
                             dataset='brain_mask'),
                         desc=("Brain-mask template used for non-linear "
                               "registration"))]
    ...
    add_param_specs = [
        ...
        ParamSpec('bet_g_threshold', 0.0,
                  desc=("Vertical gradient for BET")),
        SwitchSpec('bet_method', 'fsl_bet', ('fsl_bet', 'optibet'),
                   desc=("Method to use for brain extraction")),
        SwitchSpec('optibet_gen_report', False,
                   desc=("Wheter to generate report for OptiBET")),
        SwitchSpec('coreg_to_tmpl_method', 'ants', ('fnirt', 'ants'),
                   desc=("Method to use for non-linear coregistration to the "
                         "template")),
        ParamSpec('mni_tmpl_resolution', None, choices=(0.5, 1, 2),
                  dtype=int,
                  desc="The resolution of the MNI template to use"),
        ParamSpec('fnirt_intensity_model', 'global_non_linear_with_bias',
                  desc=("Intensity model for FNIRT")),
        ParamSpec('fnirt_subsampling', [4, 4, 2, 2, 1, 1],
                  desc=("Sub-sampling used for FNIRT")),
        ParamSpec('resampled_resolution', None, dtype=float,
                  desc=("Resolution to resample the image to. A 3-tuple "
                        "of the form [x,y,z]"),
                  array=True),
    ...
    def kspace_recon_pipeline(self, **name_maps):
        ...
        # Reorientate to standard and copy geometry from reference image if
        # required
        if (self.provided('header_image')
            ...
            pipeline.add(
                ...
                outputs={
                    'channels': ('out_dir', multi_nifti_gz_format)})
        else:
            pipeline.connect_output('channels', recon, 'channels_dir',
    ...
    def qform_transform_pipeline(self, **name_maps):
        ...
        if self.provided('coreg_ref'):
            ...
            raise BananaUsageError(
                ...
                "analysis in order to run qform_transform")
    ...
    def motion_mat_pipeline(self, **name_maps):
        ...
        if not self.is_coregistered:
            logger.info("Cannot derive 'coreg_matrix' for {} required for "
                        ...
                        "is the reference analysis".format(self))

...
def import_auth():
 try:
  ...
  raise FABIz('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=FABIN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=FABIx)
def get_params(args,names=[],mandatory=FABIN):
 result=()
 for name in names:
  value=FABIt
  if name in args:
  ...
  if not value and mandatory:
   raise FABIz('Please provide %s=... parameter'%name)
  result+=(value)
 if FABIl(result)==1:
  return result[0]
 return result

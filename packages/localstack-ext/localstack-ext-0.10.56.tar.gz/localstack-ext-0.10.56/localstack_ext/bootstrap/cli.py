...
def import_auth():
 try:
  ...
  raise wAuvP('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=wAuvp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=wAuvR)
def get_params(args,names=[],mandatory=wAuvp):
 result=()
 for name in names:
  value=wAuvm
  if name in args:
  ...
  if not value and mandatory:
   raise wAuvP('Please provide %s=... parameter'%name)
  result+=(value)
 if wAuvD(result)==1:
  return result[0]
 return result

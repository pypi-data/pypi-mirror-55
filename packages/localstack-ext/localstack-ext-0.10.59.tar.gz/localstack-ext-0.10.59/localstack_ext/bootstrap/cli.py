...
def import_auth():
 try:
  ...
  raise zcGbv('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=zcGbN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=zcGbw)
def get_params(args,names=[],mandatory=zcGbN):
 result=()
 for name in names:
  value=zcGbm
  if name in args:
  ...
  if not value and mandatory:
   raise zcGbv('Please provide %s=... parameter'%name)
  result+=(value)
 if zcGbd(result)==1:
  return result[0]
 return result

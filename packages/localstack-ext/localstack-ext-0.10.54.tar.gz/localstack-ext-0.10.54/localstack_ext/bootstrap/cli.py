...
def import_auth():
 try:
  ...
  raise sWNDP('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=sWNDq)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=sWNDO)
def get_params(args,names=[],mandatory=sWNDq):
 result=()
 for name in names:
  value=sWNDr
  if name in args:
  ...
  if not value and mandatory:
   raise sWNDP('Please provide %s=... parameter'%name)
  result+=(value)
 if sWNDU(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise ADhJx('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ADhJq)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ADhJU)
def get_params(args,names=[],mandatory=ADhJq):
 result=()
 for name in names:
  value=ADhJX
  if name in args:
  ...
  if not value and mandatory:
   raise ADhJx('Please provide %s=... parameter'%name)
  result+=(value)
 if ADhJW(result)==1:
  return result[0]
 return result

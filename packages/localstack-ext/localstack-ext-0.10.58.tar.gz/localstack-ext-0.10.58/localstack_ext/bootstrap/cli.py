...
def import_auth():
 try:
  ...
  raise ogWuQ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ogWuV)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ogWuC)
def get_params(args,names=[],mandatory=ogWuV):
 result=()
 for name in names:
  value=ogWum
  if name in args:
  ...
  if not value and mandatory:
   raise ogWuQ('Please provide %s=... parameter'%name)
  result+=(value)
 if ogWuc(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise toyPU('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=toyPG)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=toyPh)
def get_params(args,names=[],mandatory=toyPG):
 result=()
 for name in names:
  value=toyPw
  if name in args:
  ...
  if not value and mandatory:
   raise toyPU('Please provide %s=... parameter'%name)
  result+=(value)
 if toyPz(result)==1:
  return result[0]
 return result

...
def pytest_sessionstart(session):
    ...
    if config.option.freeze_reqs:
        config._freeze_reqs_ignore = set(config.getini("freeze-reqs-ignore-paths"))
        config._freeze_reqs_include = set(config.getini("freeze-reqs-include-paths"))
...
class RequirementItem(pytest.Item):
        ...
        if isinstance(excinfo.value, RequirementNotFrozenException):
            return "\n".join(
                [
                    ...
                    "   improperly frozen requirement: {1!r}: {2!r}".format(
                        *excinfo.value.args
                    ),
                    "   try e.g. {1}==1.0".format(*excinfo.value.args),
                ]
            )
    ...
    def reportinfo(self):
        return (
            self.fspath,
            0,
            "requirement: {name} is not frozen properly.".format(
                name=self.name, spec=self.specs,
            ),
        )

...
class model(ctypes.Structure):
	...
	def edge_statespace(self, e):
		...
		return np.array(list(itertools.product(range(Xs), range(Xt))))
	...
	def sample(self, sampler = Sampler.apx_perturb_and_map, generic_progress_hook = 0, num_samples = 1, iterations = 100, perturbation = 0.1, burn = 100):
...
#			write_register("GRE", resamplings)
	...
	def infer(self, inference = Inference.belief_propagation, iterations = 100, k = 3, observed=None):
...
def train(data, graph, mode, inference = Inference.belief_propagation, iters = 1000, seed = 0, k = 3, in_model = None, initial_stepsize = 0.1, opt_progress_hook = 0, generic_progress_hook = 0, opt_regularization_hook = 0, opt_proximal_hook = 0, T = 1, edge_fraction = 0.1, lam = 0, shared_states = False):

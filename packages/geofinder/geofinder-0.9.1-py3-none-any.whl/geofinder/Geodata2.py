...
from geofinder import GeodataFiles, GeoUtil, Loc
...
class Geodata:
    ...
    def find_location(self, location: str, place: Loc.Loc):
        ...
        self.logger.debug(f'Use Alternate Priority={place.use_alternate}')
        ...
        if place.place_type == Loc.PlaceType.ADVANCED_SEARCH:
            ...
            if True:
                ...
                self.logger.debug(f'Use Alternate Priority - Prefix instead of City.  Targ={place.target}')
            ...
            if True:
                ...
                if len(place.admin2_name) > 0:
                    ...
                    self.logger.debug(f'City not found.  Try admin2  [{place.target}] as city')
                    self.geo_files.geodb.lookup_place(place=place)
                    result_list.extend(place.georow_list)
        ...
        elif place.result_type is not GeoUtil.Result.NOT_SUPPORTED:
            place.place_type = Loc.PlaceType.CITY
            ...
            if True:
                # Try admin then city
                self.logger.debug('Use Alternate priority.  Try Admin as city')
                self.search_admin2(place)
                result_list.extend(place.georow_list)
            ...
            if True:
                #  Try city then admin
                self.logger.debug('Use Normal Priority.  Lookup City')
                self.search_city(place)
                result_list.extend(place.georow_list)
    ...
    def set_place_type_text(self, place: Loc.Loc):
        if place.result_type == GeoUtil.Result.NO_COUNTRY:
            place.result_type_text = 'Country'
        if place.place_type == Loc.PlaceType.CITY:
            place.result_type_text = GeoUtil.type_names.get(place.feature)
    ...
    def build_result_list(self, georow_list, event_year: int):
        ...
        list_copy = sorted(georow_list, key=itemgetter(GeoUtil.Entry.SDX, GeoUtil.Entry.ADM1, GeoUtil.Entry.ADM2, GeoUtil.Entry.NAME))
        ...
        for geo_row in list_copy:
            if self.validate_year_for_location(event_year, geo_row[GeoUtil.Entry.ISO], geo_row[GeoUtil.Entry.ADM1], 80) is False:
                # Skip location if location name  didnt exist at the time of event WITH 80 years padding
                continue

            if self.validate_year_for_location(event_year, geo_row[GeoUtil.Entry.ISO], geo_row[GeoUtil.Entry.ADM1], 0) is False:
            ...
            if geo_row[GeoUtil.Entry.NAME] != prev_geo_row[GeoUtil.Entry.NAME]:
                ...
                georow_list.append(geo_row)
                idx += 1
            elif self.get_priority(geo_row[GeoUtil.Entry.FEAT]) > self.get_priority(prev_geo_row[GeoUtil.Entry.FEAT]):
...
result_text_list = {
    ...
    GeoUtil.Result.NOT_SUPPORTED: ' is not supported. Skip or Add Country in Config',

...
try:
  ...
  print("The mplcursors Python module is not installed, so annotations are not available. Hint: pip3 install mplcursors")
...
class FigManager:
  def __init__(self, name, parent, title, window_size, x, y, setup=None, fig=None, subtitle=False):
    self.name = name
    self.parent = parent
    ...
    if subtitle:
      ...
      self.fig.text(0.5, 1 - (1 - y) * 2, self.subtitle, fontsize=fontsize, ha='center')
...
#    cid = self.fig.canvas.mpl_connect('button_press_event', self.onclick)
    ...
    padding = "          "
    self.key_presses = {}
    self.key_presses['c'] = [lambda self, e : self.display_msg("No multi-core data available.", 1) if self.parent.cores < 2 else
                                              self.display_msg("--dont-graph-cpu-lines on the commandline", 1) if not self.parent.args.plot_poi_cpu_lines else
                                              self.parent.graph_poi_per_cpu(),
                             "Display POI data in a separate graph for each CPU core."]

    self.key_presses['C'] = [lambda self, e : self.parent.graph_cpu_per_cpu() if self.parent.cores > 1 else self.display_msg("No multi-core data available.", 1),
                             "Display CPU data in a separate graph for each CPU core."]

    if mplcursors_present:
      self.key_presses['e'] = [None, # Documenting mplcursors default action
                               "Toggle whether mplcursors is active or not."]

    self.key_presses['l'] = [lambda self, e : self.toggle_legends(e.inaxes),
                             "Toggle legend(s). [#1]"]

    self.key_presses['h'] = [lambda self, e : self.display_help(),
                             "Toggle this helpful information."]

    self.key_presses['p'] = [lambda self, e : self.save(),
                             "Save an image of the current window."]

    self.key_presses['P'] = [lambda self, e : self.parent.save_all_figs(),
                             "Save images all the open windows."]

    self.key_presses['q'] = [None, # Documenting mpl default action
                             "Close the current window."]

    self.key_presses['Q'] = [lambda self, e : sys.exit(0),
                             "Exit topplot."]

    self.key_presses['s'] = [None, # Documenting mpl default action
                             "Save an image of the current window to a selectable location."]

    self.key_presses['t'] = [lambda self, e : self.toggle_axes(e.inaxes).fig.canvas.draw(),
                             "Cycle through mem/cpu/both axis visibility for POI graph. [#1]"]

    if mplcursors_present:
      self.key_presses['v'] = [None, # Documenting mplcursors default action
                               "Toggle annotations' visibility. [mplcursors feature]"]

    self.key_presses['0'] = [lambda self, e : self.parent.graph_overview(),
                             "Display the overview graphs."]

    for n in range(4):

      title, _, subtitle = self.parent.ordinal_to_cartesian_map(n)['title'].partition('\n')
      self.key_presses[f"{str(n+1)}"] = [lambda self, e : self.parent.graph_by_overview_ordinal(int(e.key) - 1),
                                        f"Display the '{title}' graph in a separate window."]

    self.key_presses['escape'] = [lambda self, e : self.clear_message(),
  ...
  def setup_graphing(self, setupfn):
    ...
    # Legend lines should be mapped in the relevant setup parameter function
    if not self.leglines:
      print("No leglines, so defaulting")

    plt.setp([a.get_xticklabels() for a in self.fig.axes[:]], visible=True, rotation=15, ha='right')
  ...
  def map_legend_lines(self, ax, legend_title_label, legend=None, *, leglines=None, legtexts=None, leghandles=None):
    if legend is not None or leglines is not None:
      ...
      if legend_title_label in self.legtitles:
        ...
        self.legtitles[legend_title_label] = {'leglines': titleleglines, 'lines': titlelines}
  ...
  def display_msg(self, txt, timeout=None):
    ...
    if self.msg_text_box is None:
        ...
        props = dict(fontfamily='monospace',
                     fontsize=14,
                     ha='center'
                    )
...
Version: {self.parent.args.version}            Copyright (c) 2019 Jonathan Sambrook and Codethink Ltd."""
        ...
        self.help_text_box = self.fig.text(0.015, 0.03, txt, bbox=bbox_props, **props)
      self.fig.canvas.draw()
...
#    ",", # pixel
  ...
  def style(self, name, line):
    if name not in self.marks:
      self.count += 1
      self.marks[name]= LineStyler.marks[(self.count + self.arbitrary_marker_offset) % LineStyler.mark_count]
      self.linestyles[name] = LineStyler.linestyles[int(self.count/LineStyler.mark_count) % LineStyler.linestyle_count]
...
class Grapher:
  ...
  def __init__(self, graph_map, args, cores, mem_in_kb, mem_cached_or_available):
    ...
    self.fig_manager = { 'overview' : None,
                         'cpu_by_cpu' : None,
                         'poi_by_cpu' : None,
                       }
  ...
  def pickable_legend_lines(self, figman, legend, ax1, ax2=None, len1=None, *, otm=False):
    leglines = legend.get_lines()
    legtexts = legend.get_texts()
    ...
    if len1 > 0:
     figman.map_legend_lines(ax1, title, leglines=leglines[:len1], legtexts=legtexts[:len1], leghandles=leghandles[:len1])

    if ax2 is not None:
      figman.map_legend_lines(ax2, title, leglines=leglines[len1:], legtexts=legtexts[len1:], leghandles=leghandles[len1:])

  #-----------------------------------------------------------------------------
  # Produce a legend for a single axis

  def single_legend(self,
                    figman,
                    title,
                    legtitle,
                    ax_ylabel,
  ...
  def otm_legend(self,
    ...
    for (legline, handle, text) in zip(src_legend.get_lines(), src_legend.legendHandles, src_legend.texts):
      label = text.get_text()
    ...
    for bin in bins.keys():
      ...
      line = mpl.lines.Line2D([],[], color=bins[bin]['colour'], label=bin, **extra_args)
  ...
  def combined_legend(self,
    ...
    # Generate new legend
    legend = ax2.legend(handles1+handles2, labels1+labels2, loc='upper right', title=legtitle)
    legend.set_draggable(True)
  ...
  def graph_cpus(grapher, figman, df, ax_in, title, *, x_bounds=None, force_single_core=None):
    ...
    if force_single_core is None:
      df.plot(y='load average', color=grapher.colours['load average'], ax=ax_loadavg, lw=3)
    ...
    if grapher.cores > 1 or force_single_core is not None:
      for core in (range(0, grapher.cores) if force_single_core is None else [force_single_core]):
        for measure in measures:
          extra_args = common_args
          if force_single_core is None:
            extra_args['marker'] = f"${core}$" # grapher.markers[f"{core}{measure}"]
    ...
    if force_single_core is None:
      combined_legend = grapher.combined_legend(figman, title, legend_title, 'loadavg', 'cpu (%)', ax_loadavg, ax_cpu) #, cap2=len(measures))
      if grapher.cores > 1:
        grapher.otm_legend(figman, "cpu (grouped)", "cpu", ax_cpu, combined_legend, copy_markers = False)
    else:
  ...
  def graph_tasks(grapher, figman, df_in, ax_sleeping, title):
    ...
    df.filter(items=['sleeping']).plot(ax=ax_sleeping, color=grapher.colours['task sleeping'],x_compat=True)
    ...
    grapher.combined_legend(figman, title, 'tasks', 'sleeping tasks', 'running, stopped, and zombie tasks', ax_sleeping, ax_others)
  ...
  def graph_mem(grapher, figman, df, ax1, title):
    ...
    lowest_mem = 'mem free' if grapher.mem_cached_or_available == 'mem cached' else 'mem available'
    ...
    if grapher.mem_cached_or_available == 'mem cached':
      ...
      df.filter(items=['mem free', 'mem cached']).plot.area(ax=ax1, color=grapher.colours['mem free'])
      ...
      df.filter(items=['mem free']).plot.area(ax=ax1, color=grapher.colours['mem free'])
    ...
    df.filter(items=['swap free']).plot(ax=ax1, color=grapher.colours['swap free'], lw=3 )
  ...
  def graph_poi(grapher, figman, df, ax, title, *, x_bounds=None):
    ...
    if (plot_cpu_lines or plot_cpu_sum) and grapher.args.include_process_cpu and \
       (plot_mem_lines or plot_cpu_sum) and grapher.args.include_process_mem:
      figman.ax_pair_map[ax_cpu] = ax_mem
      figman.ax_pair_map[ax_mem] = ax_cpu

    #-----------------------------------------------------------------------------

    def style_line(ax, index, colour, alpha, linestyle, marker=None, mark_every=None):
    ...
    def handle_data(df, ax, separate_cores, plot_data_lines, plot_summary, plot_at_all, mode, summary_line_style, max_fu, max_fu_default):
      plotted = False
      if (plot_data_lines or plot_summary) and plot_at_all:
        ...
        if plot_summary and not separate_cores:
          ...
          if summary_title not in df.columns.to_list():
            summary = grapher.args.poi_df.filter(regex=f' - {mode}$').sum(axis=1)
    ...
    alpha = 0.33
    cpu_summary_line_style = ("mediumvioletred",      alpha, (1, (1,2,1,3)), "$c$", mark_every)
    mem_summary_line_style = ("dodgerblue", alpha, (0, (2,6,2,2)), "$m$", mark_every)

    cpu_plotted, max_cpu = handle_data(df,
                                       ax_cpu,
    ...
    mem_plotted, max_mem = handle_data(df,
                                       ax_mem,
                                       x_bounds is not None,
                                       plot_mem_lines,
                                       plot_mem_sum,
    ...
    if (plot_mem_lines or plot_mem_sum) and grapher.args.include_process_mem:
      ...
      if (plot_cpu_lines or plot_cpu_sum) and grapher.args.include_process_cpu and cpu_plotted:
        ...
        old_l_mem.remove()
        l_mem_cuckoo = plt.legend(handles, labels, loc='upper left', title='mem')
    ...
    if (plot_cpu_lines or plot_cpu_sum) and grapher.args.include_process_cpu:
      ...
      if (not plot_mem_lines and not plot_mem_sum) or not grapher.args.include_process_mem:
  ...
  def graph_by_overview_ordinal(self, n):
    ...
    if figman_name in self.fig_manager:
      ...
      def setup(figman):
        ax = figman.plots # Note necessary absence of cartesian indexing for single subplot
        self.graph_map[title]['fn'](figman, self.graph_map[title]['data'], ax, figman.subtitle)

      figman  = FigManager(figman_name,
                           self,
                           self.title(title),
                           self.window_size,
                           1,
    ...
    if self.fig_manager[figman_name] is not None:
      ...
      def setup(figman):
        for (x, y, title) in zip([0,0,1,1], [0,1,0,1], self.graph_map.keys()):
          ax = figman.plots[x][y]
          self.graph_map[title]['fn'](figman, self.graph_map[title]['data'], ax, title)
  ...
  def doit(self):
    ...
    xrandr_output = subprocess.getoutput("xrandr | grep '\*' | tr -s ' ' | cut -d' ' -f2 | sort -n | tail -n1")
    ...
    if self.args.which_graph is not None:
      ...
      if self.args.which_graph.isnumeric():
        ...
        self.graph_cpu_per_cpu() if self.cores > 1 else die("No multi-core data available.")

      else:
        die(f"The character '{self.args.which_graph}' doesn't represent a graph.")

...
class HasMany(Relationship):
	...
	def delayed_init(self, values, meta):
		...
		self.target = getattr(importlib.import_module(f"models.{self.target_name}"), self.target_name.capitalize())(allow_relationships=False)
...
class ManyToMany(Relationship):
	...
	def delayed_init(self, values, meta):
		...
		self.target_pre_init = self.target = getattr(importlib.import_module(f"models.{self.target_name}"), self.target_name.capitalize())
		self.target = self.target_pre_init(allow_relationships=False)
		...
		res = DB().custom_sql(f"SELECT {', '.join(columns)} FROM {target_table} INNER JOIN {self.table} ON {target_table}.{target_pk} "
						f"= {self.table}.{self.target_key} WHERE {self.table}.{self.this_key} = {values[meta['primary_key'][0]]}")

		self.values = [self.target_pre_init(data=row, allow_relationships=False) for row in res]

...
try:
    ...
    from jsonrpcclient.exceptions import JsonRpcClientError, ReceivedErrorResponseError, ReceivedNon2xxResponseError
except ImportError:
    pass
...
class AIOConnections:
    ...
    async def send_prc_message(self, domain, method, **kwargs):
        ...
        try:
            ...
            raise JsonRpcClientError(code='unknown', status_code=e.code)
        except ReceivedErrorResponseError as e:
            if e.response.message == 'Server error':
                code = 'server_error'
            else:
                code = e.response.data.rsplit(':')[0]
            raise JsonRpcClientError(code=code)

...
from ._libomikuji_fast import lib, ffi
...
class Model(object):
    ...
    def __init__(self, model_ptr):
        ...
        self._model_ptr = ffi.gc(model_ptr, lib.free_omikuji_fast_model)

    @classmethod
    def load(cls, path: str, max_sparse_density: float = 0.1):
        """Load Omikuji model from the given directory."""
        model_ptr = lib.load_omikuji_fast_model(
    ...
    def n_features(self):
        ...
        return lib.omikuji_fast_n_features(self._model_ptr)

    def save(self, path):
        """Save Omikuji model to the given directory."""
        assert self._model_ptr != ffi.NULL
        if (
            lib.save_omikuji_fast_model(self._model_ptr, ffi.new("char[]", path.encode()))
            < 0
        ):
    ...
    def densify_weights(self, max_sparse_density: float = 0.1):
        ...
        assert self._model_ptr != ffi.NULL
        lib.densify_omikuji_fast_model(self._model_ptr, max_sparse_density)
    ...
    def predict(self, feature_value_pairs, beam_size=10, top_k=10):
        ...
        top_k = lib.omikuji_fast_predict(
            self._model_ptr,
            beam_size,
            input_len,
            feature_indices,
    ...
    def predict_fast(self, feature_indices_inp, feature_values_inp, beam_size=10, top_k=10):
        """Make predictions with Omikuji model."""
        assert self._model_ptr != ffi.NULL

        input_len = len(feature_indices_inp)
        feature_indices = ffi.new("uint32_t[]", input_len)
        feature_values = ffi.new("float[]", input_len)
        for i in range(input_len):
            feature_indices[i] = feature_indices_inp[i]
            feature_values[i] = feature_values_inp[i]
...
def init_logger():
    ...
    if lib.omikuji_fast_init_logger() < 0:
        raise RuntimeWarning("Failed to initialize logger")


ffi.init_once(init_logger, "omikuji_fast_init_logger")

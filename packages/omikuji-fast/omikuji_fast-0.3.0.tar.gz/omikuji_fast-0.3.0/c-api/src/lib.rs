...
/// Load omikuji_fast model from the given directory.
#[no_mangle]
pub unsafe extern "C" fn load_omikuji_fast_model(
    path: *const c_char,
...
/// Save omikuji_fast model to the given directory.
#[no_mangle]
pub unsafe extern "C" fn save_omikuji_fast_model(model_ptr: *mut Model, path: *const c_char) -> i8 {
    assert!(!model_ptr.is_null(), "Model should not be null");
    assert!(!path.is_null(), "Path should not be null");
    let model_ptr = model_ptr as *mut c_void as *mut omikuji_fast::Model;
...
/// Free omikuji_fast model from memory.
#[no_mangle]
pub unsafe extern "C" fn free_omikuji_fast_model(model_ptr: *mut Model) {
...
#[no_mangle]
pub unsafe extern "C" fn densify_omikuji_fast_model(model_ptr: *mut Model, max_sparse_density: f32) {
    assert!(!model_ptr.is_null(), "Model should not be null");
    let model_ptr = model_ptr as *mut c_void as *mut omikuji_fast::Model;
...
#[no_mangle]
pub unsafe extern "C" fn omikuji_fast_n_features(model_ptr: *const Model) -> size_t {
    assert!(!model_ptr.is_null(), "Model should not be null");
    let model_ptr = model_ptr as *const c_void as *const omikuji_fast::Model;
    (*model_ptr).n_features()
}

/// Make predictions with omikuji_fast model.
#[no_mangle]
pub unsafe extern "C" fn omikuji_fast_predict(
...
#[no_mangle]
pub unsafe extern "C" fn load_omikuji_fast_data_set(
    path: *const c_char,
    n_threads: usize,
) -> *mut DataSet {
    assert!(!path.is_null(), "Path should not be null");
    match CStr::from_ptr(path)
        ...
        .and_then(|path| {
            rayon::ThreadPoolBuilder::new()
                ...
                .unwrap()
                .install(|| {
                    omikuji_fast::DataSet::load_xc_repo_data_file(path)
...
#[no_mangle]
pub unsafe extern "C" fn free_omikuji_fast_data_set(dataset_ptr: *mut DataSet) {
    if !dataset_ptr.is_null() {
        let dataset_ptr = dataset_ptr as *mut c_void as *mut omikuji_fast::DataSet;
...
impl From<omikuji_fast::model::TrainHyperParam> for HyperParam {
    fn from(hyperparam: omikuji_fast::model::TrainHyperParam) -> Self {
        Self {
            ...
            linear_loss_type: match hyperparam.linear.loss_type {
                omikuji_fast::model::liblinear::LossType::Hinge => LossType::Hinge,
                omikuji_fast::model::liblinear::LossType::Log => LossType::Log,
            },
...
impl TryInto<omikuji_fast::model::TrainHyperParam> for HyperParam {
    type Error = String;

    fn try_into(self) -> Result<omikuji_fast::model::TrainHyperParam, Self::Error> {
        let mut hyper_param = omikuji_fast::model::train::HyperParam::default();
        ...
        hyper_param.linear.loss_type = match self.linear_loss_type {
            LossType::Hinge => omikuji_fast::model::liblinear::LossType::Hinge,
            LossType::Log => omikuji_fast::model::liblinear::LossType::Log,
        };
...
#[no_mangle]
pub extern "C" fn omikuji_fast_default_hyper_param() -> HyperParam {
    omikuji_fast::model::train::HyperParam::default().into()
}

/// Train omikuji_fast model on the given data set and hyper-parameters.
#[no_mangle]
pub unsafe extern "C" fn train_omikuji_fast_model(
    dataset_ptr: *const DataSet,
    hyper_param: HyperParam,
    n_threads: usize,
) -> *mut Model {
    assert!(!dataset_ptr.is_null(), "Dataset should not be null");
    let result: Result<omikuji_fast::model::TrainHyperParam, String> = hyper_param.try_into();
    match result {
        Ok(hyper_param) => {
            let dataset_ptr = dataset_ptr as *const c_void as *const omikuji_fast::DataSet;
...
#[no_mangle]
pub extern "C" fn omikuji_fast_init_logger() -> i8 {

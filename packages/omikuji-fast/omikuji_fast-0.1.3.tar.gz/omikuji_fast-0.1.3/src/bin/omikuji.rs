...
fn parse_train_hyper_param(matches: &clap::ArgMatches) -> omikuji_fast::model::TrainHyperParam {
    ...
    hyper_param.linear.loss_type = match matches.value_of("linear.loss").unwrap() {
        "hinge" => omikuji_fast::model::liblinear::LossType::Hinge,
        "log" => omikuji_fast::model::liblinear::LossType::Log,
        _ => unreachable!(),
    };
...
fn train(matches: &clap::ArgMatches) {
    ...
    let training_dataset = {
        ...
        omikuji_fast::DataSet::load_xc_repo_data_file(path).expect("Failed to load training data")
    };
...
fn test(matches: &clap::ArgMatches) {
    ...
    let model = {
        ...
        let mut model = omikuji_fast::Model::load(model_path).expect("Failed to load model");
    ...
    if let Some(test_path) = matches.value_of("test_data") {
        let test_dataset =
            omikuji_fast::DataSet::load_xc_repo_data_file(test_path).expect("Failed to load test data");

        let (predictions, _) = {
            let beam_size = value_t!(matches, "beam_size", usize).unwrap();
            omikuji_fast::model::eval::test_all(&model, &test_dataset, beam_size)
        };
...
fn main() {
    ...
    let default_hyperparam = omikuji_fast::model::train::HyperParam::default();
    ...
    let arg_matches = clap::App::new("omikuji_fast")
                ...
                .arg(
                    clap::Arg::with_name("linear.loss")
                        ...
                        .default_value(match default_hyperparam.linear.loss_type {
                            omikuji_fast::model::liblinear::LossType::Hinge => "hinge",
                            omikuji_fast::model::liblinear::LossType::Log => "log",
                        })

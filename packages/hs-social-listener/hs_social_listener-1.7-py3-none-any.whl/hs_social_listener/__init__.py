...
class Listener:
    ...
    def run(self):
        ...
        df2 = pd.DataFrame()
        df2['tweets_content'] = self.Tweets.groupby('username')['clean_tweet'].agg(lambda col: ' '.join(col))
        df2['lang_tags'] = df2['tweets_content'].apply( lambda x : twitter_listener.lang_tags(x))

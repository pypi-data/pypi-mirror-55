...
class InboundPipeline(ABC):
    ...
    def __init__(self, config_obj, postgres_connection_string):
        ...
        self.config = self.get_config(config_obj)
        self.scripts_location = self.config.scripts_location
    ...
    def get_config(config_obj):
        ...
        config = json.loads(json.dumps(config_obj), object_hook=lambda d: Namespace(**d))
        return config

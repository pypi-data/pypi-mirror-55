...
class Extend( object ):
	...
	def launchThreads( self ):
        ...
        Locates any methods in the process whose name begins with `_thread`, and then uses :any:`Process.addChildThread() <polycephaly.core.process.threading.Extend.addChildThread>` to add each of these methods as a reciprocal thread, then daemonize and start each thread.

        An example for automatically launching threads:

        .. code-block:: python

            def _threadHello( self ):
                while self.isActive():
                    print( 'Hello' )
                    self.freqSleep()
                    pass # END WHILE LOOP

...
def get_devices_to_monitor(hours_start, hours_end, power_threshold):
    ...
    print(power)
    return db_session.query(Device).filter(or_(and_((Device.device_last_transmission != None), (Device.device_last_transmission <= deltatime_start), (Device.device_last_transmission > deltatime_end)), and_((Device.device_battery_level != None), func.hex(func.substr(Device.device_battery_level, 4, 8)) <= power))).all()
...
def get_devices_lastTransmission_between(hours_start, hours_end):
    ...
    deltatime_end = datetime.now() - timedelta(hours=hours_end)
    print(deltatime_end)
    return db_session.query(Device).filter((Device.device_last_transmission != None), (Device.device_last_transmission <= deltatime_start), (Device.device_last_transmission > deltatime_end)).all()

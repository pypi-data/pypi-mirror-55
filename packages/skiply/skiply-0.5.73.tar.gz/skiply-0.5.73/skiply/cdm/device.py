...
def get_devices_to_monitor(hours_start, hours_end, power_threshold):
    ...
    session = db_session()
    try:
        results = session.query(Device).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_end).filter(Device.device_last_transmission <= deltatime_start | func.hex(func.substr(Device.device_battery_level, 4, 8)) <= power).all()
    except:

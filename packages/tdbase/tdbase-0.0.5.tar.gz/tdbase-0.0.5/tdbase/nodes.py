...
class NodeBlock:
    ...
    wbRecordIdColumn = 120  # where IDs of records are put.
...
class RecordPointer:
    ...
    def emb_head_to_str(self, start_col: int):  # start_col is 0-based which column the record-header starts at
        if self.record:
            return self.record.emb_head_to_str(start_col)
        elif self.def_str:
            return NodeRecord.gen_header_str(start_col, self.record_type, self.record_name, self.record_id, self.parser)
        else:
            return ""
...
class NodeRecord(NodeBlock):
    ...
    def __init__(self, preComments, firstRealLineInfo:LineInfo, parser, ptr:Optional[RecordPointer]=None):
        ...
        self.directives: List["NodeRecordDirective"] = []
...
#    def attr(self) -> Dict[str, 'NodeAttrib']:
    ...
    def dump_to_str(self, indent, sort_attribs, skip_header=False) -> str:
        ...
        for directive in self.directives:
            ret.append(directive.dump_to_str(indent + NodeBlock.wbIndentStep, sort_attribs))
    ...
    @classmethod
    def gen_header_str(cls, start_col, typ, name, rid, parser):
        if name:
        ...
        if typ:
            ...
            ret = n    # note that for embedded records that have not been mapped/linked, the type can be ""

        if i:
            ret = ret + " " * (NodeBlock.wbRecordIdColumn - len(ret) - 1 - start_col) + i
        return ret


    def emb_head_to_str(self, start_col=0):
        return NodeRecord.gen_header_str(start_col, self.record_type, self.record_name, self.record_id, self.parser)


class NodeRecordDirective(NodeBlock):
    def __init__(self,
                 pre_comments: Optional[NodeComments],
                 first_real_lineinfo: LineInfo,
                 parser: "parser.TDBParser",
                 ):
        self.pre_comments = pre_comments
        self.first_real_lineinfo = first_real_lineinfo
        self.parser = parser

    def dump_to_str(self, indent, unused_sort_attribs) -> str:
        ret = []
        if self.pre_comments:
            ret.append(self.pre_comments.dump_to_str(indent, None))
        pre_comment = "%s:%s %s" % (" " * indent, self.first_real_lineinfo.record_directive, self.first_real_lineinfo.value_str)
        if self.first_real_lineinfo.comment:
            pre_comment += " #" + self.first_real_lineinfo.comment
        ret.append(pre_comment + "\n")
        return "".join(ret)
#        return "TODO - NodeRecordDirective"
...
class NodeAttrib(NodeBlock):
    def __init__(self,
        ...
        self.value: Optional[Union[float, str]] = None  # either the data/value, or None if empty. Note that for pointer or xref, this is None. For long-str, this may contain LF
        ...
        if self.firstRealLineInfo:
            ...
            if self.firstRealLineInfo.val_type == "S":
                ...
                if self.firstRealLineInfo.long_str:
                    self.value = self.firstRealLineInfo.long_str  # TODO!
    ...
    def dump_to_str(self, indent, sort_attribs) -> str:
        ...
        if self.value is None or self.value == "":
            ...
            v = " " + str(self.value)
            # TODO: check if string needs quoting!
        else:  # long string
        ...
        if self.key is None:
            ...
            if self.pointer:  # list element is a ptr to another record
                pre_record = indentStr + "-> "
                pre_comment = (pre_record + self.pointer.emb_head_to_str(len(pre_record))).rstrip()
                if self.pointer.embedded:
                    pre_comment += ":"
                # ret.append("%s-> %s%s\n" % (indentStr, self.pointer.emb_head_to_str().rstrip(), self.firstRealLineInfo.getCommentStr()))
            elif self.xref:
                pre_comment = "%s=> %s" % (indentStr, self.xref)
            else:
                v = v.replace("\n", "\n     " + " " * len(indentStr))
                v = "\n".join([l.rstrip() for l in v.split("\n")])   # if empty lines, remove spaces
          ...
          #      ret.append("%s-%s%s\n" % (indentStr, v, self.firstRealLineInfo.getCommentStr()))
            ...
            pre_record = "%s%s" % (indentStr, self.key)
            if self.pointer:
                pre_record += " ->"
                v = (" " + self.pointer.emb_head_to_str(len(pre_record) + 1)).rstrip()
                if self.pointer.embedded:
                    v += ":"
            elif self.xref:
                v += " => %s" % self.xref
            ...
            v = v.replace("\n", "\n    " + " " * len(pre_record))
            v = "\n".join([l.rstrip() for l in v.split("\n")])   # if empty lines, remove spaces
            pre_comment = pre_record + v  # "%s%s%s%s" % (indentStr, self.key, colon_or_ptr, v)
            ret.append(pre_comment + self.get_comment_str_lf(len(pre_comment)))
        ...
        if self.pointer and self.pointer.embedded:
            ...
            if isinstance(self.children, dict):
                ...
                # TODO: empty list is OK? But saves as empty value.

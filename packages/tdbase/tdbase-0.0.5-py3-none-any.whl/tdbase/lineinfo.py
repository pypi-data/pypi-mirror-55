...
class LineInfo:
    ...
    _re_rec_name_or_key = r"(" + r":?" + _rec_name_pattern + r")?"
    _re_spaces = r"(\s*)"
    _re_rest = r"(.*)"
   ...
   #     self.prev_real_line = prev_real_line
        self.typ: str = "#"                 # either # for empty or only comment, or '!':preproc, '.':record, '['=context, 'A'=attrib or list item, ":"=record directive
       # self.rawFirst = None                # TODO!
        ...
        self.val_type: str = ""             # unknown. For typ==A: "S":str, "P":pointer, "X":external file
        self.value_str: str = ""            # str for value. Also content of preproc, context, record data, directive argument/value. Also if attribute w/ pointer, use this.
        ...
        self.record_directive: str = ""     # if record directive, this is the directive
        ...
        if long_str_start:
            ...
            while 1:
                ...
                if l.lstrip()[:3] == '"""':
                    l = l.lstrip()
                elif len(l) < lstr_indent:
                    if l != " " * len(l):
                        raise Exception("long string indent should only be spaces 1:%s" % l)
                    l = ""
                elif l[:lstr_indent] != " " * lstr_indent:
                    raise Exception("long string indent should only be spaces 2:%s" % l)
                else:
                    l = l[lstr_indent:]

                m2 = LineInfo._pattern4.search(l)   # check for end of long-str, ie '''
                if m2:
        ...
        a, b, c, d = m.groups()  # pattern2 returns _re_first_or_indent + _re_rec_name_or_key + _re_spaces + _re_rest
        if b:
        ...
        if len(a) == 1 and a in ".[!":    # record, context or preproc
            ...
            if b and b[0] == ":":
                self.typ = ":"
                self.record_directive = b[1:]
                self.value_str = d  # comment???
                return
    ...
    def __repr__(self):
        return "<LineInfo L%d>" % self.line_nr

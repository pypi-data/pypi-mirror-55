...
class AzureMLModel:
    def __init__(self):
        self.model = KMeans(random_state=0)
...
cluster_file_name = 'cluster.ilearner'
...
def cluster_training_data():
    df = pd.DataFrame()
    df['col0'] = list(range(8))
    df['col1'] = list(range(8))
    return DataTable(df)


def cluster_predict_data():
    df = pd.DataFrame()
    df['col0'] = [1, 2, 3, 4]
    return DataTable(df)


def cluster_expect_predict_result():
    return pd.DataFrame({'col0': [1, 2, 3, 4], 'Scored Labels': [6, 1, 4, 0]})
...
def test_init_model_cluster():
    init_model = CreatePythonModelModule.run(
        python_stream_reader=cluster_script)[0]
    expect_file_name = untrained_prefix + cluster_file_name
    check_computed_model(init_model, expect_file_name)


def test_train_model_cluster():
    input_file_name = untrained_prefix + cluster_file_name
    untrained_model = read_input_model(input_file_name)
    trained_model = TrainModelModule.run(
        learner=untrained_model,
        training_data=cluster_training_data(),
        label_column_index_or_name=label_column()
    )[0]
    assert trained_model.is_trained
    expect_file_name = trained_prefix + cluster_file_name
    check_computed_model(trained_model, expect_file_name)


def test_score_model_cluster():
    input_file_name = trained_prefix + cluster_file_name
    trained_model = read_input_model(input_file_name)
    scored_dt = ScoreModelModule.run(
        learner=trained_model,
        test_data=cluster_predict_data(),
        append_or_result_only=True
    )[0]
    expect_df = cluster_expect_predict_result()
    assert scored_dt.data_frame['col0'].equals(expect_df['col0'])
    assert scored_dt.data_frame['Scored Labels'].tolist() == (expect_df['Scored Labels']).tolist()


def test_evaluate_model_cluster_error():
    input_file_name = trained_prefix + cluster_file_name
    trained_model = read_input_model(input_file_name)
    scored_dt = ScoreModelModule.run(
        learner=trained_model,

...
EXPECT_FOLDER_NAME = 'expect_learner'
INPUT_FOLDER_NAME = 'input_learner'

binary_classification_script = """
import pandas as pd
...
trained_prefix = 'trained_'
untrained_prefix = 'untrained_'
...
def test_init_model_binary_classification():
    init_model = CreatePythonModelModule.run(
        python_stream_reader=binary_classification_script)[0]
...
def test_evaluate_model_binary_classification_error():
    input_file_name = trained_prefix + binary_classification_file_name
    trained_model = read_input_model(input_file_name)
    scored_dt = ScoreModelModule.run(
        learner=trained_model,

...
"""
regression_file_name = 'regression.ilearner'
...
def regression_predict_data():
    df = pd.DataFrame()
    df['col0'] = [1, 2, 3, 4]
    return DataTable(df)


def regression_expect_predict_result():
    return pd.DataFrame({'col0': [1, 2, 3, 4], 'Scored Labels': [1.1, 0.7, 0.3, -0.1]})
...
def test_init_model_regression():
    init_model = CreatePythonModelModule.run(
        python_stream_reader=regression_script)[0]
    expect_file_name = untrained_prefix + regression_file_name
    check_computed_model(init_model, expect_file_name)


def test_train_model_regression():
    input_file_name = untrained_prefix + regression_file_name
    ...
    expect_file_name = trained_prefix + regression_file_name
    check_computed_model(trained_model, expect_file_name)


def test_score_model_regression():
    input_file_name = trained_prefix + regression_file_name
    trained_model = read_input_model(input_file_name)
    scored_dt = ScoreModelModule.run(
        learner=trained_model,
        test_data=regression_predict_data(),
        append_or_result_only=True
    )[0]
    assert scored_dt.data_frame.round(1).equals(regression_expect_predict_result())


def test_evaluate_model_regression_error():
    input_file_name = trained_prefix + regression_file_name
    trained_model = read_input_model(input_file_name)
    scored_dt = ScoreModelModule.run(
        learner=trained_model,

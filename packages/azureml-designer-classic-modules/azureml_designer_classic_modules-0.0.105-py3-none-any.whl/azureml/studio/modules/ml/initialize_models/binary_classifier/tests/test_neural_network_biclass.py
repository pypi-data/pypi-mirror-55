...
from ..neural_network_biclassifier.neural_network_biclassifier import NeuralNetworkBiClassifier, \
    TwoClassNeuralNetworkModule, TwoClassNeuralNetworkModuleDefaultParameters, \
    CreateNeuralNetworkRegressionModelNeuralNetworkTopology


class TestNNBinaryClassCase(BinaryClassCommonCase):
    entry_class = TwoClassNeuralNetworkModule
    model_class = NeuralNetworkBiClassifier

    performance_assert_threshold = .95

    default_single_parameter = {
        'initial_weights_diameter': 0.1,
        'learning_rate': 0.1,
        'momentum': 0.0,
        'num_hidden_nodes': str(60),
        'num_iterations': 150,
        'shuffle': True,
    ...
    default_range_parameter = {
        ...
        'ps_learning_rate': ParameterRangeSettings.from_literal(
            TwoClassNeuralNetworkModuleDefaultParameters.PsLearningRate),
        'ps_num_iterations': ParameterRangeSettings.from_literal(
            TwoClassNeuralNetworkModuleDefaultParameters.PsNumIterations),
    ...
    def test_num_iterations_work(self):
        ...
        clf.init_model()
        accuracy_value = mltest_utils.get_f1(clf)

        clf2 = self._init_model(num_iterations=num_iterations_large)
        clf2.init_model()
        accuracy_value2 = mltest_utils.get_f1(clf2)

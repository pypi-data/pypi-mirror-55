...
from ..decision_forest_regressor.decision_forest_regressor import \
    DecisionForestRegressionModule, DecisionForestRegressor, ResamplingMethod, \
    DecisionForestRegressionModuleDefaultParameters


class TestDecisionForestRegressorCase(RegressionCommonCase):
    entry_class = DecisionForestRegressionModule
    model_class = DecisionForestRegressor
    ...
    default_range_parameter = {
        ...
        'ps_tree_count': ParameterRangeSettings.from_literal(
            DecisionForestRegressionModuleDefaultParameters.PsTreeCount),
        'ps_max_depth': ParameterRangeSettings.from_literal(
            DecisionForestRegressionModuleDefaultParameters.PsMaxDepth),
        'ps_min_leaf_sample_count': ParameterRangeSettings.from_literal("1")  # Fit the small test case.
    }

...
class DockerContainer(object):
    def _run_docker_cmd(self, cmd, **kwargs):
        log.debug(cmd)
    ...
    def _reload(self):
        self.id = self._run_docker_cmd(['docker', 'ps', '-af', 'name=' + self.name, '-q'])
        if self.id:
            self.__info = json.loads(self._run_docker_cmd(['docker', 'inspect', self.name]))[0]
        else:
            self.__info = None
    ...
    def stop(self):
        self._run_docker_cmd(['docker', 'stop', self.name], stderr=subprocess.STDOUT)
        self._reload()

    def rm(self):
        self._run_docker_cmd(['docker', 'rm', self.name], stderr=subprocess.STDOUT)
        self._reload()

    def start(self):
        assert self.exists and not self.running
        cmd = ['docker', 'start', self.name]
        self._run_docker_cmd(cmd)
        self._reload()
...
class ConnectCommand(Command):
    ...
    def __call__(self, args):
        ...
        cmd = ['ssh', '-i', rsa.private_key,
               ...
               '-Y', 'phosphoros@localhost', '-p', container.ssh_port]
        log.debug(cmd)
        subprocess.call(cmd)

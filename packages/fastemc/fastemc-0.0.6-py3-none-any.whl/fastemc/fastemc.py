...
def score(features, labels, setting='fast', cross_validation_bins=5):
    def wrapper(choice):
        if setting=='fast':
        ...
        return np.mean(cross_val_score(clf, features[:,choice], labels, cv=cross_validation_bins))
    return wrapper
...
def run(
    ...
    score_fast = score(features, labels, setting='fast', cross_validation_bins=cross_validation_bins)
    score_slow = score(features, labels, setting='slow', cross_validation_bins=cross_validation_bins)

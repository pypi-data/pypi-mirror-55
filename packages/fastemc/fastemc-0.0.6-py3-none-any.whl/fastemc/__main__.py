...
parser.add_argument(
    '--cross_validation_bins', '-cv', type=int, default=5,
    help='number of sample bins to use for cross validation'
)
...
if args.restart:
    ...
    global_optimum_score = df['score'].astype(float).values
    feature_to_index = dict(zip(
        features.columns, 
        range(len(
            features.columns
        ))
    ))
    global_optimum_choice = df.iloc[:,2:].apply(lambda ser: ser.map(feature_to_index)).values
...
_ = fastemc.run(
    ...
    cross_validation_bins=args.cross_validation_bins,
    logger=save_results,
    restart=restart
)

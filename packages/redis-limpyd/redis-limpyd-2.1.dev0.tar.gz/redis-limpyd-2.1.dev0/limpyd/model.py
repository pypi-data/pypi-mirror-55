...
class MetaRedisModel(MetaRedisProxy):
    ...
    def __new__(mcs, name, base, attrs):
        ...
        it._fields_cache = {
            field_name: it.get_field(field_name)
            for field_name in _fields
        }
        it._fields_cache['pk'] = it._redis_attr_pk
...
class RedisModel(with_metaclass(MetaRedisModel, RedisProxyCommand)):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in self.get_class_fields():
            ...
            self._fields_cache[field.name] = new_field = copy(field)
        ...
        if pk_field_name != 'pk':
            self._fields_cache['pk'] = self.pk = getattr(self, pk_field_name)
    ...
    def has_field(cls, field_name):
        ...
        """
        try:
            return field_name in cls._fields_cache
    ...
    def get_class_field(cls, field_name):
        ...
        """
        try:
            field = cls._fields_cache.get(field_name)
        except (AttributeError, TypeError):
            field = getattr(cls, '_redis_attr_%s' % field_name) if cls.has_field(field_name) else None
    ...
    def get_instance_field(self, field_name):
        ...
        """
        try:
            field = self._fields_cache.get(field_name)
        except AttributeError:
            field = getattr(self, field_name) if self.has_field(field_name) else None
    ...
    def get_class_fields(cls):
        for field_name in cls._fields:
            yield cls._fields_cache[field_name]
    ...
    def get_instance_fields(self):
        for field_name in self._fields:
            yield self._fields_cache[field_name]

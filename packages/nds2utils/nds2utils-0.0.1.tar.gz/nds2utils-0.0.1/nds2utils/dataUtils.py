...
def savePickle(dataDict, savefile):
...
# FFT parameters
def dttTime(averages, bandwidth, overlap, verbose=False):
    '''
...
def dttAverages(time, bandwidth, overlap, verbose=False):
...
def find_channels(  channel_glob='*',
    ...
    # Get Hanford ISI stuff, include second and minute trend
    ...
    Output: buffers native to nds2 (indexed with numbers)
    '''
        ...
        try:
            ...
            for buff in buffers:
                ...
                if 'STREND' in name_long:
                    ...
                    suffix = ',m-trend'
                print('{name}{suffix}    fs = {fs} Hz'.format(name=name, suffix=suffix, fs=fs))
            print()
            print('Displaying channels that match {glob} on {hs}:{pn}'.format(glob=channel_glob, hs=host_server, pn=port_number))
            print()
...
def acquireData(channels,
    ...
    buffs = acquireData(['H1:LSC-DARM_IN1_DQ', 'H1:LSC-DARM_OUT_DQ'], 1256340013, 1256340167)

    # Get minute and second trends.  Beware of getting minute trends too close to the present, will return only nans
    buffs = acquireData(['H1:ISI-GND_STS_ITMY_Y_BLRMS_30M_100M.mean,m-trend',
                         'H1:ISI-GND_STS_ITMY_Y_BLRMS_30M_100M.mean,s-trend'], 1256340013, 1256340563)
    ...
    dataDict = extractDict(buffers, duration=duration)
    return dataDict

def stitchRealTimeData( channels,
                        duration,
                        host_server='nds.ligo-wa.caltech.edu',
                        port_number=31200,
                        verbose=True):
        ...
        for ii in range(len(channels)):
            ...
            if not len(dataDict.keys()) == len(channels):
                ...
                if curBuff.data[0] == 0.0:  # if the data is identically 0, something is wrong
                    print()
                    print('\033[91m'+'Data {} is identically zero.  Stopping data acquisition.'.format(chan)+'\033[0m')
                    print()
                    return 'ZerosError'
            else:
                if curBuff.data[-1] == dataDict[chan]['data'][-1]:  # if the data is repeating for some reason
                    print()
                    print('\033[91m'+'Data {} is repeating.  Stopping data acquisition.'.format(chan)+'\033[0m')
                    print()
                    return 'ZerosError'
...
def extractDict(chanData, duration):
    '''
...
def trimDict(dataDict, duration):
...
def calibrateDict(dataDict, recalculate=False):
    ...
    dataDict = calibrateDict(dataDict)
    ...
    for chan in calChannels:
        ...
        dataDict[chan]['calPSD'] = dataDict[chan]['PSD'] * np.abs(calFunc(ff))**2
        dataDict[chan]['calASD'] = dataDict[chan]['ASD'] * np.abs(calFunc(ff))
        for chan2 in calChannels:
            ...
            dataDict[chan][chan2]['calCSD'] = dataDict[chan][chan2]['CSD'] * np.conj(calFunc(ff)) * calFunc2(ff)
            dataDict[chan][chan2]['calTF'] = dataDict[chan][chan2]['TF'] * calFunc(ff) / calFunc2(ff)
    return dataDict

def dataPSDs(dataDict, averages, bandwidth, overlap):
...
def dataTFs(dataDict):
    ...
    if 'CSD' not in dataDict[channels[0]][channels[1]]:
        print('You must run dataCSDs() first!')
        print('Exiting')
        return
    for chan in channels:
...
def dataCSDs(dataDict, averages, bandwidth, overlap, makeTFs=True):
    ...
    if 'PSD' not in dataDict[channels[0]].keys():
        print('Running dataPSDs!')
        dataDict = dataPSDs(dataDict, averages, bandwidth, overlap)
        print('Got PSDs!')
    ...
    for chan in channels:
        ...
        for chan2 in channels:
            ...
            if not fs == fs2:
                ...
                ff, Pxx = sig.welch(decimated_data, fs=small_fs, nperseg=nperseg, noverlap=overlap, average='mean')
            ...
            ff, Cxy = sig.csd(small_data, small_data2, fs=small_fs, nperseg=nperseg, noverlap=overlap, average='mean')
    ...
    if makeTFs:
        dataDict = dataTFs(dataDict)
    return dataDict

def getPSDs(channels,
            gps_start,
            gps_stop,
            binwidth,
            overlap,
    ...
    Should be slightly faster than getCSDs(), especially for large numbers of channels.
    ...
    overlap = 0.75
    binwidth = 1.0
    dataDict_23 = getPSDs(channels, gps_start_23, gps_stop_23, binwidth, overlap)
    '''
    duration = int(gps_stop - gps_start)
    averages = int(dttAverages(duration, binwidth, overlap))
    ...
    dataDict = dataPSDs(dataDict, averages, binwidth, overlap)
    return dataDict

def getCSDs(channels,
            gps_start,
            gps_stop,
            binwidth,
            overlap,
    ...
    overlap = 0.75
    binwidth = 1.0
    dataDict_23 = getCSDs(channels, gps_start_23, gps_stop_23, binwidth, overlap)
    ...
    dataDict = dataCSDs(dataDict, averages, binwidth, overlap, makeTFs=makeTFs)
    return dataDict

### Modify data based on thresholds
def coherenceThresholdApplier(coherenceVector, coherenceThreshold=0.9):
...
def bandlimitApplier(frequencyVector, bandlimit_low, bandlimit_high):
...
def logbinASD(log_ff, linear_ff, linear_ASD):
    ...
    '''
    matrix = ReSamplingMatrixNonUniform(linear_ff, log_ff)
...
def linear_log_ASD(log_ff, linear_ff, linear_ASD):
    ...
    matrix = ReSamplingMatrixNonUniform(linear_ff, high_fff)

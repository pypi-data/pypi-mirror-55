...
def plot_ASDs(dataDict,
    ...
    for ii, chan in enumerate(channels):
        ...
        if label == None:
            ...
            label = '{} {}'.format(chan.replace('_', ' '), label)
...
def compare_ASDs(dataDict1,
    ...
    for ii, chan in enumerate(channels):
        ...
        if label1 == None:
            ...
            label1 = '{} {}'.format(chan.replace('_', ' '), label1)
        if label2 == None:
            label2 = chan.replace('_', ' ')
        else:
            label2 = '{} {}'.format(chan.replace('_', ' '), label2)
...
def plot_TFs_A(dataDict,
    ...
    for ii, chanB in enumerate(channels):
        ...
        if cal_applied and 'calTF' in dataDict[chanB][chanA].keys():
            ...
            label = '{label} [{B}/{A}]'.format(label=label, B=unitsB, A=unitsA)
        if not label == None:
        ...
        s1.loglog(plotff, np.abs(plotTF), ls=ls, alpha=alpha, label=label)
        s2.semilogx(plotff, 180/np.pi*np.angle(plotTF), ls=ls, alpha=alpha, label=label)
        s3.semilogx(plotff, plotcoh, ls=ls, alpha=alpha, label=label)

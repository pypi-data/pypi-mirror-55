...
def make_array_from_buffers(buffers):
    ...
    Outputs:
    channel_names = np.array() with the channels as
    '''
    channel_names = np.array([])
    for buff in buffers:
        channel_names = np.append(channel_names, buff.name)
...
def data_PSDs(dataDict, averages, bandwidth, overlap):
...
def data_CSDs(dataDict, averages, bandwidth, overlap, makeTFs=True):
    ...
    if 'PSD' not in dataDict[channels[0]].keys():
        ...
        dataDict = data_PSDs(dataDict, averages, bandwidth, overlap)
        print('Got PSDs!')
...
def get_PSDs(channels,
    ...
    dataDict = data_PSDs(dataDict, averages, binwidth, overlap)
    return dataDict
...
def get_CSDs(channels,
    ...
    dataDict = data_CSDs(dataDict, averages, binwidth, overlap, makeTFs=makeTFs)
    return dataDict

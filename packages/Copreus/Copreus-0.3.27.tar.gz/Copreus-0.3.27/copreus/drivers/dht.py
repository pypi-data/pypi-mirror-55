...
class DHT(ADriver, APolling, AEvents):
    ...
    def _poll_device(self):
        ...
        if self._validate_t.valuerange(t):
            ...
            self._logger.info("temperature out of valid range {}".format(self._validate_t.valuerangeMessage(t)))

        if self._validate_h.valuerange(h):
            self._publish_value(self._topics_pub["humidity"], h)
        else:
            self._logger.info("humidity out of valid range {}".format(self._validate_h.valuerangeMessage(h)))

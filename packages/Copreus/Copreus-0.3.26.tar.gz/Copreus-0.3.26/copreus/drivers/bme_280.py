...
class BME_280(ADriver, APolling, AEvents, AI2C):
    ...
    def _poll_device(self):
        ...
        if self._valuerange_t.valuerange(t):
            ...
            self._logger.info("temperature out of valid range {}".format(self._valuerange_t.valuerangeMessage(t)))

        if self._valuerange_h.valuerange(h):
            self._publish_value(self._topics_pub["humidity"], h)
        else:
            self._logger.info("humidity out of valid range {}".format(self._valuerange_h.valuerangeMessage(h)))

        if self._valuerange_p.valuerange(p):
            self._publish_value(self._topics_pub["pressure"], p)
        else:
            self._logger.info("pressure out of valid range {}".format(self._valuerange_p.valuerangeMessage(p)))

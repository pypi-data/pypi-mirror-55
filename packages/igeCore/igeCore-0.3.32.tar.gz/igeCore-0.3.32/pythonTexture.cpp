...
namespace pyxie
{
	...
	PyObject* createCheckeredTexture(uint8_t red, uint8_t green, uint8_t blue, int texWidth, int texHeight, bool alpha)
	{
		int bitSize = 3 + alpha ? 1 : 0;
		...
		for (int i = 0; i < texWidth; i++){
			for (int j = 0; j < texHeight; j++){
				...
				if (a == b){
					image[(i + j * texWidth) * bitSize + 0] = red;
					image[(i + j * texWidth) * bitSize + 1] = green;
					image[(i + j * texWidth) * bitSize + 2] = blue;
					if(alpha) image[(i + j * texWidth) * bitSize + 3] = 255;
				}
			}
		}
	...
	static PyObject* texture_setCheckeredImage(texture_obj* self, PyObject* args, PyObject* kwargs)
	{
		...
		int g=255;
		int b=0;
		if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|iii", kwlist, &r, &g, &b)) return NULL;

		int w = self->colortexture->GetTextureWidth();
		int h = self->colortexture->GetTextureHeight();
		bool alpha = self->colortexture->IsUseAlphaChannel();
		PyObject* obj = createCheckeredTexture((uint8_t)r, (uint8_t)g, (uint8_t)b, w, h, alpha);
		self->subImage = obj;
		self->x = 0;
		self->y = 0;
		self->w = -1;

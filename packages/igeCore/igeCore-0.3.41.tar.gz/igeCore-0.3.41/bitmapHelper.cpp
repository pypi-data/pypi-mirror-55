...
namespace pyxie
{
	void FlipY(uint64_t* inimg, uint64_t* outimg, int w, int h) {
		w /= 8;
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				outimg[y * w + x] = inimg[((h - 1) - y) * w + x];
			}
		}
	}
	...
	PyObject* createTextImage(const char* word, const char* fontpath, int fontsize, int texw, int texh) {
		...
		unsigned char* bitmap = (unsigned char*)pyxie::PYXIE_MALLOC(b_w * b_h);
		memset(bitmap, 0, b_w * b_h);

		renderText(word, fontsize, info, bitmap, b_w, b_h);

		unsigned char* bitmap2 = (unsigned char*)pyxie::PYXIE_MALLOC(b_w * b_h);
		FlipY((uint64_t*)bitmap, (uint64_t*)bitmap2, b_w / 8, b_h);

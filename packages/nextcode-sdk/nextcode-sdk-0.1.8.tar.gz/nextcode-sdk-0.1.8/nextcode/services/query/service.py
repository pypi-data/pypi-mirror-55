...
class Service(BaseService):
    ...
    def simple_execute(self, query: str, wait: int = 2, results: int = 100) -> Query:
        """
        Execute a simple, short gor statement on the server using a blocking call

        :param query: gor query string
        :param wait: number of seconds to wait for results
        :param results: number of rows to return
        :raises: QueryError
        """
        self._check_project()
        ...
        query_id = gor_query.query_id
        if contents["status"] == "FAILED":
            raise QueryError(contents["error"]["description"], query_id=query_id)
        elif contents["status"] == "DONE":
            return gor_query
        else:
            raise QueryError(
                "Query %s did not complete in %s seconds. Current status is %s"
                % (contents["id"], wait, contents["status"]),
                query_id=query_id,
            )
    ...
    def execute_template(self, template_name: str, nowait: bool = False) -> Query:
        """

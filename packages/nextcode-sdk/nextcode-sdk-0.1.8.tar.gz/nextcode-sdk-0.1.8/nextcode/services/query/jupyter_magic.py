...
class GorMagics(Magics):
    ...
    def gor(self, line, cell=None):
        try:
            ...
            if qry.status != "DONE":
                try:
                    print_error(qry.error["description"])
                except TypeError:
                    raise Exception("Query status is %s" % qry.status)
                return None
            log.info("Completed in %.2f sec", (time.time() - st))
            if persist:
                return None
            MAX_ROWS = 1000000
            if qry.line_count > MAX_ROWS:
                print_error(

...
class CourseTableSync(BaseSync):
    ...
    def read_compus_excel(self):
        dir_path = os.path.dirname(os.path.abspath(__file__))
        excel_path = os.path.join(dir_path, 'compus.xlsx')
        df = pd.read_excel(excel_path)
        for value in df.ix[:, ['楼名称', '校区']].values:
            self.compus_map[value[0]] = value[1]
            ...
            for kw in key_words:
                if kw in classroom_name:
                    ...
                    try:
                        ...
                        pass
                    break
        compus = self.compus_map.get(building, '本部')
        return building, floor, compus

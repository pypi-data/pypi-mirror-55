...
class BarProvider:
    ...
    def add_bar(self, new_bar):
        ...
        if self.is_end:
            self.change_state = 4
            return
        result = self.__move_to(new_bar.begin_time, False)
        ir = result[0]
        index = result[1]
        if ir == -1:
            self.change_state = 4
    ...
    def __move_to(self, now, is_tick=True):
        ...
        while index < len(self.__lst_date_time_slices) - 1:
            ...
            ir = self.__is_current_date_time_slice(date_time_slice, next_date_time_slice, now, is_tick)
            if ir == 0:
                return 0, index
        ...
        # 收盘以后的Tick处理
        bmy = self.__is_current_date_time_slice(self.__lst_date_time_slices[index], None, now, is_tick)
        if bmy == 0:
            return 0, index
    ...
    @staticmethod
    def __is_current_date_time_slice(current_date_time_slice, next_date_time_slice, date_time, is_tick=True):
        if next_date_time_slice is not None:
            ...
            # 收盘5秒内的tick都计算
            if is_tick:
                if (current_date_time_slice.begin_time <= date_time) & (date_time < (current_date_time_slice.end_time + datetime.timedelta(seconds=1000))):
                    return 0
            else:
                if (current_date_time_slice.begin_time <= date_time) & (date_time < (current_date_time_slice.end_time + datetime.timedelta(seconds=0))):
                    return 0
        return 1

...
class Collector:
    ...
    def get_m3u8s(session=True):
        ...
        m3u8s = []
        if len(name_and_details_url_arr) > YAMLConfig.maximum_download_num():
            logger.info(f'The length of current list is greater than {YAMLConfig.maximum_download_num()} '
                        f'set in `config.yaml > maximum_download_num`')
            name_and_details_url_arr = name_and_details_url_arr[:YAMLConfig.maximum_download_num()]
        ...
        for i in name_and_details_url_arr:
            if session and last_download_item is not None and last_download_item == i["name"]:
                logger.info('Detected repeated download item, all download tasks after \'%s\' will be cancelled.'
                            % last_download_item)
                break

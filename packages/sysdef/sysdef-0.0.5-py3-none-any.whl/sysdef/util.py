...
def parse(sysdef_file, stage):
    logger.debug("parsing: %s", sysdef_file)
...
def save_json(filename, data, mode=0o644, owner="root", group="root", g=None):
    logger.info(f"saving JSON: {filename}...:\n{data}")
...
def save_yaml(filename, data, mode=0o644, owner="root", group="root", g=None):
    logger.info(f"saving YAML: {filename}...:\n{data}")
    data = yaml.dump(data)
    return mkfile(filename, mode=mode, owner=owner, group=group, g=g, content=data)
...
def needs_update(filename, content, g=None):
    ...
    logger.debug("%s needs updating: %s", filename, status)
    return status


def chown(path, owner="root", group="root", root="", g=None):
    logger.debug(f"chown {path} owner={owner} group={group} g={g}")
    changes = False
    _path = root + path
    ...
    if g:
        ...
        if stat.st_uid != uid or stat.st_gid != gid:
            if getpass.getuser() == "root":
                ...
                logger.error(f"skipping chown({path})) - not root")
    if changes:
        logger.debug(f"chown {path} -> {owner}:{group} - OK")
...
def chmod(path, mode, root="", g=None):
    changes = False
    logger.debug(f"chmod {path} mode={oct(mode)} g={g}")
    _path = root + path
    if g:
        stat = g.statns(_path)
    ...
    if changes:
        logger.debug(f"chmod {path} -> {oct(mode)} - OK")

    return changes


def readlines(filename, g=None):
    logger.debug(f"reading {filename} g={g}")
    if g:
...
def savelines(lines, filename, g=None):
    logger.debug(f"saving {filename} g={g}:\n{lines}")
    if g:
...
def mkdir(path, mode=0o755, owner="root", group="root", root="", g=None):
    changes = False
    logger.debug(f"mkdir {path} mode={mode} owner={owner} group={group} g={g}")
    _path = root + path
    if g:
...
def mkfile(path, mode=0o644, owner="root", group="root", root="", g=None, content=None):
    logger.debug(f"writing {path} mode={mode} owner={owner} group={group} g={g}: bytes={len(content)}")
    changes = False
...
def ln_s(target, linkname, g=None, root=""):
    ...
    changes = False
    logger.debug(f"ln -s {target} {linkname} g={g}")
    if g:
    ...
    if changes:
        logger.debug(f"ln -s {target} -> {linkname} - OK")

...
def save_passwd(etc_passwd_data, root="", g=None):
    logger.info(f"saving /etc/passwd:")
...
def save_shadow(etc_shadow_data, root="", g=None):
    logger.info(f"saving /etc/shadow:")
    etc_shadow_file = root + "/etc/shadow"
    lines = []
...
def process(data, root="", g=None):
    ...
    if "root" not in data:
        ...
        logger.info("locking root account! - if stage 2 fails grant access here")
    ...
    for user in data:
        ...
        if ssh:
            ...
            if authorized_keys_string:
                ...
                logger.info("creating authorized_keys file %s", authorized_keys_file)

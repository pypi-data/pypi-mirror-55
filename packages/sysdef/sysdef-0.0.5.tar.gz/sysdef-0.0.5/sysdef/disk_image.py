...
def create_disk_image(disk_image_filename, size_mb, label="OVERLAY"):
    logger.debug(f"create disk image {disk_image_filename}, size {size_mb}MiB")
    ...
    logger.debug("Done! shutting down guestfs...")
    g.shutdown()
...
def create_guestfs_instance(root_image, root_part_label="rootfs", overlay_image=None, overlay_mount=None, overlay_disk_label="overlay"):
    ...
    if overlay_image:
        logger.debug("mounting overlay filesystem")
        ...
        logger.debug(f" Disk label search result: {overlay_device}")
...
def truncate_block(image_file):
    ...
    if sector_alignment == 0:
        logger.debug(f"image file {image_file}: already aligned (size {sector_count} sectors)")
    else:
        padding_needed = 2048 - (sector_count % 2048)
        padding_needed_bytes = padding_needed * 512
        logger.debug(f"truncating: +{padding_needed} sectors to 1MiB boundary")
...
def backup_gpt_and_truncate(sdcard, gpt_backup="gpt.dat"):
    ...
    logger.debug("Saving backup GPT to %s", gpt_backup)
    ...
    with open(sdcard, 'rb') as file:
        ...
        logger.debug(f"GPT offset: {gpt_offset}")
        gpt_data = file.read()
...
def absorb(sdcard, extra_image):
    ...
    logger.debug("found partition name: %s", new_name)
    ...
    logger.debug(f"New partition geometry:\n{new_geometry}")
...
def get_uuid_for_label(disk_image_filename, label):
    ...
    logger.info(f"resolved label:{label} -> guid:{guid}")
    g.shutdown()

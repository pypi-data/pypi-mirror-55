...
def start_qemu(image_file, temp_overlay_image=None, interactive=False, rootfs_label="rootfs"):
    ...
    if temp_overlay_image:
        logger.debug("starting QEMU with additional overlay: %s", temp_overlay_image)
        qemu_command += f"""
        -drive {drive2} \
        -device "virtio-blk-device,drive=hd1"
        """
    logger.debug(f"Final qemu command: {qemu_command}")
    ...
    with os.fdopen(sys.stdout.fileno(), "wb", closefd=False) as stdout:
        ...
        while (process.poll() == None):
            ...
            try:
                ...
                logger.error("Unable to flush stdout - python exiting?")
                pass
...
def command(cmd, session, check=True):
    ...
    logger.debug("...result: %s", exit_status)
...
def stop_emulator(session):
    ...
    try:
        ...
        logger.info("Emulator is shut down...")

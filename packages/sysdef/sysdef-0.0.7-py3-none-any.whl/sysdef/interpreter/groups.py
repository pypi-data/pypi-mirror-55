...
def find_next_gid(etc_group_data):
    ...
    for group in etc_group_data:
        logger.debug("look for " + str(etc_group_data[group]))
        logger.debug("look for " + etc_group_data[group][FIELD_GID])
...
def save_groups(etc_group_data, root="", g=None):
    ...
    for group in etc_group_data:
        logger.info(f"saving: {group}")
        group_attrib = ":".join(etc_group_data[group])
        logger.info(group_attrib)
...
def process(data, root="", g=None):
    ...
    if changes:
        logger.info(f"updating {etc_group_file}... entries: {len(etc_group_data)}")

...
class Model(ModelABC):
    ...
    def __init__(self, model_description: str, description=None,
        ...
        Keyword arguments:
            ...
            check_idf          -- Check necessary identification conditions
                                  (default: True).
        """
        self.outputs_covary = outputs_covary
        ...
        if check_idf:
            df = self.calc_df()
            if df < 0:
                print('Warning: DoF ({}) is less than zero, the model is underspecified.'.format(df))
            exo = self.vars['ObsExo'] + self.vars['LatExo']
            endo = self.vars['ObsEndo'] + self.vars['LatEndo']
            k = len(exo)
            g = len(endo)
            for v, items in self._counter_in.items():
                n = len([1 for it in items if it in exo])
                if k - n < g - 1:
#                    print('Warning: Order condition is not met, the model is underspecified.')
#                    fl = True
                    break
    ...
    def prepare_parameters(self, description):
        ...
        if self.psi_mode == 'Full':
            it = combinations(self.vars['LatExo'], 2)
            if self.outputs_covary:
                it = chain(it, combinations(self.vars['Outputs'], 2))
            for v1, v2 in it:
        ...
        if self.psi_mode == 'Full' or self.psi_mode == 'Diag':
            ...
            raise Exception("Unknown psi mode.")
        for v in it:
    ...
    def load_params_from_df(self, df):
        try:
        ...
        ops = self.operations
        c = -1
        for i, j in self.parameters['Beta']:
            c += 1
            a, b = self.beta_names[0][i], self.beta_names[1][j]
            val = df[(df.lval == a) & (df.op == ops.REGRESSION.value) & (df.rval == b)].Value
            if not len(val):
                continue
            val = float(val)
            self.mx_beta[i, j] = val
            params[c] = val
        for i, j in self.parameters['Lambda']:
            c += 1
            b, a = self.lambda_names[0][i], self.lambda_names[1][j]
            val = df[(df.lval == a) & (df.op == ops.MEASUREMENT.value) & (df.rval == b)].Value
            if not len(val):
                continue
            val = float(val)
            self.mx_lambda[i, j] = val
            params[c] = val
        for i, j in self.parameters['Psi']:
            c += 1
            a, b = self.psi_names[0][i], self.psi_names[1][j]
            val = df[(df.lval == a) & (df.op == ops.COVARIANCE.value) & (df.rval == b)].Value
            if not len(val):
                continue
            val = float(val)
            self.mx_psi[i, j] = val
            self.mx_psi[j, i] = val
            params[c] = val
        for i, j in self.parameters['Theta']:
            c += 1
            a, b = self.theta_names[0][i], self.theta_names[1][j]
            val = df[(df.lval == a) & (df.op == ops.COVARIANCE.value) & (df.rval == b)].Value
            if not len(val):
                continue
            val = float(val)
            self.mx_theta[i, j] = val
            self.mx_theta[j, i] = val
            params[c] = val
    ...
    def postprocess_parameters(self, data: DataFrame, cov: DataFrame):
        ...
        for ind in self.parameters['Lambda']:
            ...
            if np.isnan(self.mx_lambda[ind]):
                ...
                slope = linregress(data[r], data[l]).slope
                if np.isnan(slope):
                    if np.isnan(slope):
                        slope = 0
                self.mx_lambda[ind] = slope
...
#                self.mx_theta[ind[::-1]] = self.mx_theta[ind]
    ...
    def calc_df(self):
        '''Calculates model's degrees of freedom.
        '''
        k = len(self.vars['IndsObs'])
        kfix = len(self.vars['ObsExo'])
        return (k * (k + 1) - kfix * (kfix + 1)) / 2 - self.num_params
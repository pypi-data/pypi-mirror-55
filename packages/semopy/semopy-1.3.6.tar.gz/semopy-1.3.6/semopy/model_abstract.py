...
class ModelABC(ABC):
    ...
    def classify_variables(self, description, force_load=set()):
        ...
        categorical = set()
        ops = self.operations
        counter_in = defaultdict(list)
        for lv in description:
            desc_lv = description[lv]
            rvals_regression = desc_lv[ops.REGRESSION].keys()
            if rvals_regression:
                in_arrows.add(lv)
                out_arrows.update(rvals_regression)
                counter_in[lv].extend(rvals_regression)
            rvals_measurement = desc_lv[ops.MEASUREMENT].keys()
            if rvals_measurement:
                latents.add(lv)
                indicators.update(rvals_measurement)
            tp = desc_lv[ops.TYPE].keys()
            if tp:
                tp = list(tp)[0].lower()
                if tp in ('categorical', 'cat', 'ord', 'ordinal'):
                    categorical.add(lv)
                elif tp in ('lat', 'latent'):
                    latents.add(lv)
        if indicators & (in_arrows | out_arrows):
            raise Exception("Measurement part should not be mixed with structural part.")
        ...
        self.vars['All'] = self.vars['Indicators'] + self.vars['Latents'] +\
                           self.vars['Observed']
        self.vars['Categorical'] = list(sorted(categorical))

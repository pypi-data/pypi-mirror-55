...
if version_info >= (2, 6, 0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_loss_', [dirname(__file__)])
        except ImportError:
            import _loss_
            return _loss_
        if fp is not None:
            try:
                _mod = imp.load_module('_loss_', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _loss_ = swig_import_helper()
...
class Batch(daal.algorithms.neural_networks.layers.LayerIface):
    ...
    def __init__(self):
        this = _loss_.new_Batch()
        try:
            self.this.append(this)
        except:
            self.this = this
    __swig_destroy__ = _loss_.delete_Batch
    __del__ = lambda self: None
Batch_swigregister = _loss_.Batch_swigregister

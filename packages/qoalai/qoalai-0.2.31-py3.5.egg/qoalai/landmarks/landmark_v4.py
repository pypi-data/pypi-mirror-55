...
class Landmark(object):
    ...
    def build_densenet_base(self, input_tensor,
        ...
        out = new_conv2d_layer(input=out, 
                               filter_shape=[2, 2, out.get_shape().as_list()[-1], 3], 
    ...
    def calculate_loss(self):
        ...
        """
        # ----------------
        # the output map ideally describe the error probaility between the predicted point with the label point
        # the output map placeholder is the distance for each grid top left corner to the target points
        # the label for output map is the the distance between the predicted point with the label point
        # to make it as a probability, we convert the label for output map to : 1. - relative distance to image size
        # ---------------
        ...
        label_map = 1. - tf.sqrt(tf.square(label_deltax) + tf.square(label_deltay))
        ...
        # negative map error
        # ----------------
        label_map_neg_tmp = 1. - label_map_pos_tmp
        ...
        self.point_position_loss = self.position_loss_alpha *  mse_loss_sum(tmp_point_position, tmp_point_position_placeholder)

        # ---------------
        # total loss
        # ---------------
        self.total_loss = self.point_map_loss_pos + \
                          self.point_map_loss_neg + \
                          self.point_position_loss
        ...
        for i in range(self.grid_num):
            for j in range(self.grid_num):
                ...
                for k in range(self.num_landmark_point):
                    ...
                    tmp = 1. - math.sqrt(tmpx * tmpx + tmpy * tmpy)
    ...
    def optimize(self, 
        ...
        for i in range(iteration):
            ...
            print (">> epoch: ", i, "train loss: ", round(t_loss, 3), "val loss: ", round(loss, 3), \
                "val map acc: ", round(map_acc, 3), "val map pos loss: ", round(pml_pos, 3), "val map neg loss: ", round(pml_neg, 3), "val poss loss: ", round(ppl, 3), sign)  

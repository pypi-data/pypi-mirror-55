...
def parse(extend=None, args=None):
    return (
        stdargs.Parser('Get/Set transaction configuration', ['filename', 'limit'])
            ...
            .add('--force', action='store_true', help='set a single group, remove existing aliases for the group')
            .extend(extend)
            .parse(args)
    )
    ...
    def still_valid(tt):
        for cand in tt.aliases:
            if cand.transaction_group != group:
                return True
        if arg is not True:
            raise AssertionError("Existing group detected, use '--force' to remove them")
        return False

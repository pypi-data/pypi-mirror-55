...
def process_args(api, args):
    ...
    def sorted_group_df(df):
        ...
        while not df_not_sorted.empty:
            ...
            if not any(x in df_sorted[GROUP_NAME].to_list() for x in
                       df_not_sorted[SUB_GROUPS].to_list()) and not df_not_sorted.empty:
                raise ValueError(
    ...
    def create_groups():
        ...
        group_dict = {}
        group_dict.update({group_name: {SUB_GROUPS: resource_id_vars(group_df[SUB_GROUPS].dropna().unique()),
                                        PORTFOLIOS: resource_id_vars(group_df[PORTFOLIOS].dropna().unique())} for
                           group_name, group_df in df.groupby(GROUP_NAME)})

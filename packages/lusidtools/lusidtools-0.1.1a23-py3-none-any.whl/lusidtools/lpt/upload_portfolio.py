...
def parse(extend=None, args=None):
    return (
        stdargs.Parser('Upload a Portfolio', ['scope', 'portfolio'])
            ...
            .add('-p', '--positions', nargs=2, metavar=('filename', 'YYYY-MM-DD'), help='Positions filename and date')
            .add('-c', '--create',
                 nargs=3,
                ...
                def load_groups(iterator):
                    try:
                        ...
                        return load_transactions(
                            portfolio,
                            df.drop(portfolio_column, axis=1)
                ...
                return api.call.set_holdings(
                    ...
                    holding_adjustments=[
                        api.models.AdjustHoldingRequest(
                            instrument_identifiers=lpt.to_instrument_identifiers(i if len(keys) == 1 else i[0]),
                            sub_holding_keys=lpt.perpetual_upsert(api.models, hld_df, 'SHK:'),
                            properties=lpt.perpetual_upsert(api.models, hld_df),
                            tax_lots=api.from_df(hld_df[taxlot_fields], api.models.TargetTaxLotRequest)
                        )

    ...
    def access(obj, col):
        if col.startswith('P:'):
            try:
                ...
                if type(props) == list:
                    return property_value([p.value for p in props if p.key == col[2:]][0])
...
def from_df(df, record_type, complex_types, related=None, columns=None, date_fields=None):
    simple_columns = []
    ...
    if len(date_fields) > 0:
        ...
        for col in date_fields:
            df[col] = pd.to_datetime(df[col], utc=True).map(lambda x: None if pd.isna(x) else x)
    ...
    def build_properties(row):
        ...
        def prop_builder(property_key, value):
            if isinstance(value, str):
                return ptype(key=property_key, value=complex_types['PropertyValue'](label_value=value))
            elif value != None:
                return ptype(key=property_key,
                             value=complex_types['PropertyValue'](metric_value=complex_types['MetricValue'](value)))
            else:
                return None

        props = [(col, prop_builder(col, row["P:{}".format(col)])) for col in properties]
        d = dict([p for p in props if p[1] != None])
        return d

    def to_type(i, row):
        fields = {col: row[col] for col in simple_columns}
        fields.update({col: build_complex_type(row, col, fields)
        ...
        if 'instrument_uid' in fields.keys():
            fields['instrument_identifiers'] = to_instrument_identifiers(fields['instrument_uid'])
...
def display_error(error, response=False):
    try:
        ...
        if len(error.items) > 0:
            df = records_to_df(
                [Rec(Id=key, Detail=item)
                 for key, item in error.items.items()]
            )
            print("ITEMS (max 50)")
            display_df(df[:50])
    except:
...
def perpetual_upsert(models, df, prefix='P:'):
    ...
    def make_property(properties, key):
        return properties + [
            (key[offset:], models.PerpetualProperty(key[offset:], models.PropertyValue(label_value=str(value))))
            for value in df[key].dropna().head(1).tolist()]

    return dict(
        reduce(
            make_property,
            [c for c in df.columns.values if c.startswith(prefix)],
            []
        )
    )

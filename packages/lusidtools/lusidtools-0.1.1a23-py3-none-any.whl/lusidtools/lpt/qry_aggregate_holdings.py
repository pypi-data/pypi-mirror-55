...
def run_query(api,args,date):
   ...
   def success(result):
       df = pd.DataFrame.from_records(
              result.content.data
            ) [[AGG_TYPE,AGG_INSTR,AGG_UID,AGG_UNITS,AGG_COST,AGG_PRC,AGG_RATE,AGG_PV]]
       ...
       return result.stats, df[[UID,TYPE,INSTR,UNITS,COST,PRICE,LVAL,RATE,PVAL]]
   
   fn = (api.call.get_aggregation_by_group 
           if args.group 

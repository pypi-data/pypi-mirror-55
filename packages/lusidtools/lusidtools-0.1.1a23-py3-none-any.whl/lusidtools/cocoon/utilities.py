...
def identify_cash_items(dataframe, mappings, file_type: str) -> (pd.DataFrame, dict):
    """
    ...
    :param str file_type: File type of data, takes one of "instruments", "holdings", "transactions", "Quotes"
    ...
    for index, row in dataframe.iterrows():
        for column in cash_flag_specification["cash_identifiers"].keys():
            if row[column] in cash_flag_specification["cash_identifiers"][column]:
                if file_type == "instruments":
                    rm_index.append(index)
                else:
                    dataframe.at[index, "currency_identifier_for_LUSID"] = populate_currency_identifier_for_LUSID(row,
                                                                                                              column,
                                                                                                              cash_flag_specification)
                break
    if file_type == "instruments":
...
def populate_currency_identifier_for_LUSID(row: dict, column, cash_flag_specification: dict) -> str:
    """
    ...
    if "required" in mapping.keys():
        for field in mapping["required"]:
            if isinstance(mapping["required"][field], dict):
                check_mapping_fields_exist(mapping["required"][field]["column"].values(), columns, "required")
            else:

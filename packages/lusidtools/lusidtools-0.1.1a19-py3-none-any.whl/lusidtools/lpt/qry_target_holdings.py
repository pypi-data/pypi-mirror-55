...
def parse(extend = None, args = None):
    return (
       stdargs.Parser('Get targets', ['portfolio', 'scope', 'date_range', 'filename', 'limit', 'asat'])
...
def process_args(api,args):
    ...
    taxlot_fields = ['units','cost.currency','cost.amount','portfolio_cost','price','purchase_date','settlement_date']
    identifiers = set()
    shk = set()
    properties = set()
    ...
    if args.date:
       ...
       return api.call.list_holdings_adjustments(
          scope=args.scope,
          code=args.portfolio,
          from_effective_at=lpt.to_date(args.start_date) if args.start_date else lpt.to_date('1900-01-01'),
          to_effective_at=lpt.to_date(args.end_date) if args.end_date else lpt.to_date(datetime.now()) 
       ).bind(list_success)

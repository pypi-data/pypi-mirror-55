...
def get_rates(code,ccy,start):
    response = requests.get("https://www.quandl.com/api/v3/datasets/BOE/{}?start_date={}&api_key=6dzRn58ssvhXUErUHaYA".format(code,start))
    try:
       data = response.json()
...
dates = (pd.concat([v[1] for v in l],ignore_index=True,sort=True)[[DATE]]
           .drop_duplicates()
           .sort_values(DATE)
...
new_rates = (pd.concat(full,ignore_index=True,sort=True)
              .sort_values([DATE,CCY]))

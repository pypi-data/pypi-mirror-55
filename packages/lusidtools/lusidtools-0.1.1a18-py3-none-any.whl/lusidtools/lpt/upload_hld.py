...
def process_args(api, args):
    ...
    if 'csv' in args.input:
        ...
        columns_to_modify = [c for c in df.columns.values if c.startswith(SHK) or c.startswith(P)]
        for col in columns_to_modify:
            df[col] = df[col].apply(lambda x: x.get('value', {}).get('labelValue', ''))
    else:
        raise Exception('The file provided: {} is not .json or .csv format.'.format(args.input))

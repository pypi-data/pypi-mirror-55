...
@checkargs
def create_identifiers(index: int, row: pd.Series, file_type: str, instrument_identifier_mapping: dict = None,
                       unique_identifiers: list = None, full_key_format: bool = True):
    """
    ...
    def prepare_key(identifier_lusid: str, full_key_format: bool) -> str:
        ...
        """
        if full_key_format:
            return identifier_lusid if re.findall('Instrument/default/\S+', identifier_lusid) else f'Instrument/default/{identifier_lusid}'
        else:
            return identifier_lusid.split("/")[2] if re.findall('Instrument/default/\S+', identifier_lusid) else identifier_lusid
    ...
    identifiers = {
        ...
        prepare_key(identifier_lusid, full_key_format):
            models.InstrumentIdValue(value=row[identifier_column]) if file_type == 'instrument' else row[identifier_column] for \
        identifier_lusid, identifier_column in instrument_identifier_mapping.items() if not \
            # Only use the identifier it it has a value
        pd.isna(row[identifier_column])
    }

    # If there are no identifiers raise an error
    if len(identifiers) == 0:
        raise ValueError(f'''The transaction at index {index} has no value for every single one of the provided 
        identifiers. Please ensure that each transaction has at least one identifier and try again''')
    ...
    if file_type == 'instrument':
        ...
        unique_identifiers_populated = list(set(unique_identifiers).intersection(set(list(identifiers.keys()))))
        ...
        if len(unique_identifiers_populated) == 0:
            raise ValueError(f'''The instrument at index {index} has no value for at least one unique 
...
@checkargs
def resolve_instruments(api_factory: lusid.utilities.ApiClientFactory, data_frame: pd.DataFrame, identifier_mapping: dict):
    """
    ...
    if 'Currency' not in list(identifier_mapping.keys()) and 'Instrument/default/Currency' not in list(identifier_mapping.keys()):
        raise KeyError(
    ...
    if 'Currency' in list(identifier_mapping.keys()):
        identifier_mapping['Instrument/default/Currency'] = identifier_mapping['Currency']
    ...
    if not (set(identifier_mapping.values()) <= set(data_frame.columns)):
        raise KeyError('there are values in identifier_mapping that are not columns in the dataframe')
    ...
    for index, row in _data_frame.iterrows():
        ...
        search_requests = [
            models.InstrumentSearchProperty(
                key=f'Instrument/default/{identifier_lusid}' if 'Instrument/' not in identifier_lusid else identifier_lusid,
                value=row[identifier_dataframe]
            ) for identifier_lusid, identifier_dataframe in identifier_mapping.items() if not pd.isnull(row[identifier_dataframe])
        ]
...
def get_unique_identifiers(api_factory: lusid.utilities.ApiClientFactory):
    ...
    identifiers = api_factory.build(lusid.api.InstrumentsApi).get_instrument_identifier_types()

    # Return the identifiers that are configured to be unique
    return [identifier.identifier_type for identifier in identifiers.values if identifier.is_unique_identifier_type]


@checkargs
async def enrich_instruments(api_factory: lusid.utilities.ApiClientFactory, data_frame: pd.DataFrame,
                             instrument_identifier_mapping: dict, mapping_required: dict, constant_prefix: str = "$"):
    ...
    for index, row in data_frame.iterrows():
        ...
        search_requests_instrument = [
            lusid.models.InstrumentSearchProperty(
                key=identifier_lusid if re.findall('Instrument/default/\S+', identifier_lusid) else f'Instrument/default/{identifier_lusid}',
                value=row[identifier_column]
            ) for identifier_lusid, identifier_column in instrument_identifier_mapping.items() if not pd.isna(row[identifier_column])
        ]
    ...
    elif isinstance(mapping_required["name"], str) and mapping_required["name"][0] != constant_prefix:
        data_frame[mapping_required["name"]] = data_frame[mapping_required["name"]].fillna(
            value=data_frame[enriched_column_name])

    elif isinstance(mapping_required["name"], dict) and "column" in list(mapping_required["name"].keys()):
        data_frame[mapping_required["name"]["column"]] = data_frame[mapping_required["name"]["column"]].fillna(
            value=data_frame[enriched_column_name])

    # Is a constant specified by the constant prefix
    elif isinstance(mapping_required["name"], str) and mapping_required["name"][0] == constant_prefix:
        mapping_required["name"] = {
            "default": mapping_required["name"][1:]
        }
        mapping_required["name"]["column"] = enriched_column_name

    # Is a constant specified by the default nested dictionary specification
    elif isinstance(mapping_required["name"], dict) and "default" in list(mapping_required["name"].keys()) and "column" not in list(mapping_required["name"].keys()):
        mapping_required["name"]["column"] = enriched_column_name

    return data_frame, mapping_required


async def instrument_search(api_factory: lusid.utilities.ApiClientFactory, search_requests: list) -> list:
    """
...
@run_in_executor
def instrument_search_single(api_factory: lusid.utilities.ApiClientFactory,
                             search_request: lusid.models.InstrumentSearchProperty) -> list:
    """
    ...
    return lusid.api.SearchApi(api_factory.build(lusid.api.SearchApi)).instruments_search(
        symbols=[search_request]
    )

...
def process_args(api,args):
    ...
    def success(result):
        idents = ["identifiers.KEY:{}".format(v) for v in args.identifiers]
    ...
    def step2(result=None):
        ...
        if result is not None:
           ...
           l = list(result.apply(lambda r: "Instrument/{}/{}".format(r['scope'],r['code']),axis=1))
           args.properties = l[:MAX_PROPS] # limitation 
           if len(l) > MAX_PROPS:
               next_step = lambda r : step3(r,l[MAX_PROPS:],None)

        return api.call.get_instruments(args.type,args.instrument,instrument_property_keys=args.properties).bind(next_step)

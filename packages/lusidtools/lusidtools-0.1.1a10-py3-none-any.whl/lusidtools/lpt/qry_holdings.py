...
def parse(extend = None, args = None):
   return (
     stdargs.Parser('Get Holdings', ['filename', 'limit', 'scope', 'portfolio', 'asat'])
...
def process_args(api, args):
   ...
   if(len(args.dates) <= 1):
      ...
      def create_record(result):
         ...
         records.append(rec)
         if args.monitor:
            print("{: 3} {}: Duration: {}, LUSID: {}".format(i[0],d,stats.duration,stats.elapsed))
         i[0] += 1
      ...
      leading_cols = ['#','Start-Time','Date','Duration','LUSID','Id','Positions']
...
def run_query(api,args,date):
   ...
   def success(result):
       ...
       df.columns = [c[1] for c in columns] + [k.split('/')[2] for k in shkeys] + [p[8:] for p in  args.properties]

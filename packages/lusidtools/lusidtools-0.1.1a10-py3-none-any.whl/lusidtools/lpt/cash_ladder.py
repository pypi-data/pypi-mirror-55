...
def cash_ladder(api,scope,portfolio,date):
    ...
    df[TYPE] = df[TYPE].map({'C':'Trades to settle',
                             'R':'Receivables',
                             'A':'Dividends',
                             'F':'Forward Fx'})
...
def parse(extend = None):
    ...
    parser.add_argument('-f','--filename', metavar='filename.csv', help='write to this file')
    parser.add_argument('-a','--alternative', action='store_true', help='alternative view')
    parser.add_argument('--secrets-file',dest='secrets', default='secrets.json', help = 'path to secrets file')
    if extend:
       extend(parser)

...
def parse(extend=None, args=None):
    return (
        stdargs.Parser('Map Instruments', ['filename'])
            ...
            .add('--column', metavar='input-column', default='instrument_uid', help='column name for instrument column')
            .extend(extend)
            .parse(args)
    )
...
def process_args(api, args):
    ...
    df = pd.concat(
        [lpt.read_csv(f)[[args.column]].drop_duplicates()
         for f in args.input],
        ignore_index=True,
        sort=True
...
def map_instruments(api, df, column):
    ...
    def update_mappings(src, prefix):
        mapping_table.update({prefix + k: v.lusid_instrument_id for k, v in src.items()})
    ...
    def batch_query(instr_type, prefix, outstanding):
        if len(outstanding) > 0:
            ...
            def get_success(result):
                ...
                if len(get_failed) > 0:
                    if instr_type == 'ClientInternal':
                        ...
                        return Either.Left(
                            "Failed to locate instruments of type {}"
                                .format(instr_type)
                        )
                else:
            ...
            return api.call.get_instruments(instr_type, batch[WORKING].values).bind(get_success)
        else:
    ...
    return (
        map_type('FIGI', 'Figi')
            .bind(lambda r:
                  map_type('INT', 'ClientInternal')
                  )
            .bind(lambda r:
                  map_type('QPI', 'QuotePermId')
                  )
    )

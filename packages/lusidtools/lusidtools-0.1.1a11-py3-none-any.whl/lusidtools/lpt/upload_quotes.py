...
def process_args(api, args):
    if args.input:
        ...
        failures = {k : v for i in results if i.is_right for k, v in i.right.content.failed.items()}
...
def update_failures(res, df):
    ...
    for key, value in res.items():
        df.at[int(key), 'error'] = value.detail.replace("One or more problems occurred. Failures:", "")
    return df.dropna(subset=['error']).loc[:, ['error','provider', 'source', 'instrument_uid', 'instrument_uid_type', 'quote_type','field']]

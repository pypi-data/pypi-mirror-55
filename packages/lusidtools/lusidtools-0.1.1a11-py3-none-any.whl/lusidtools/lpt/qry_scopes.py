...
def process_args(api,args):
    ...
    def success(result):
        if args.portfolios:
           df = lpt.to_df(result, ['id.scope', 'id.code', 'is_derived', 'type', 'parent_portfolio_id'])
           df.columns = ['Scope','Portfolio','Derived','Type','Parent']
           return  lpt.trim_df(df, args.limit, sort=['Scope', 'Portfolio'])
        else:
           df = pd.DataFrame({'Scopes':[v.id.scope for v in result.content.values]}).groupby('Scopes').size().reset_index()

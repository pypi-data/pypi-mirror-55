...
def process_args(api, args):
    ...
    if args.input:
        ...
        def make_identifiers(row):
            return {
                identifier:
                    api.models.InstrumentIdValue(row[identifier])
        ...
        def success(r):
            df = lpt.to_df(
                [err[1] for err in r.content.failed.items()],
                ['id', 'detail']
            )
        ...
        requests = [
            api.models.InstrumentDefinition(
                ...
                make_properties(row),
                api.models.ResourceId(
                    row[LT_SCOPE],
                    row[LT_CODE]
                ) if (has_lookthrough and pd.notna(row[LT_SCOPE])) else None
            )

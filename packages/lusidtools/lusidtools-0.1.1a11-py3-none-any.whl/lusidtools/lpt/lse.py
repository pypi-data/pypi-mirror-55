...
class ExtendedAPI:
    ...
    def __init__(self, config, api, lusid, custom_headers=None):
        ...
        exc = (
            lusid.rest.ApiException
            if 'ApiException' in lusid.rest.__dict__
            else dummy
        )
    ...
    def dump_stats(self):
        if self.stats != None:
            lpt.dump_stats(
                self.stats_file,
                self.stats,
                ['startTime', 'endTime', 'name', 'requestId', 'duration', 'elapsed', 'status']
            )
    ...
    def __getattr__(self, name):
        ...
        def callApiFn(*args, **kwargs):
            ...
            try:
                ...
                return Either.Left("ERROR: exception in call to {}\n{}".format(name, cerr))
            except msrest.exceptions.HttpOperationError as herr:
                return Either.Left("ERROR: exception in call to {}\n{}".format(name, herr.response.text))
...
def connect(*args, **kwargs):
    ...
    connect = importlib.import_module(".connect_lusid", "lusidtools.lpt")

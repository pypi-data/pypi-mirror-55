...
class BatchLoader:
    ...
    @staticmethod
    @run_in_executor
    def load_instrument_batch(api_factory: lusid.utilities.ApiClientFactory,
                              instrument_batch: list, **kwargs) -> lusid.models.UpsertInstrumentsResponse:
        """
        ...
        if "unique_identifiers" not in list(kwargs.keys()):
            unique_identifiers = cocoon.instruments.get_unique_identifiers(api_factory=api_factory)
        else:
            unique_identifiers = kwargs["unique_identifiers"]

        @checkargs
        def get_alphabetically_first_identifier_key(instrument: lusid.models.InstrumentDefinition,
            ...
            """
            unique_identifiers_populated = list(set(unique_identifiers).intersection(set(list(instrument.identifiers.keys()))))
        ...
        return api_factory.build(lusid.api.InstrumentsApi).upsert_instruments(
            instruments={
                get_alphabetically_first_identifier_key(instrument, unique_identifiers): instrument for instrument in instrument_batch
            })

    @staticmethod
    @run_in_executor
    def load_quote_batch(api_factory: lusid.utilities.ApiClientFactory, quote_batch: list,
                         **kwargs) -> lusid.models.UpsertQuotesResponse:
        """
        ...
        if "scope" not in list(kwargs.keys()):
            raise KeyError("You are trying to load quotes without a scope, please ensure that a scope is provided.")
        ...
        return api_factory.build(lusid.api.QuotesApi).upsert_quotes(
            ...
            quotes={
                ...
                ): quote for quote in quote_batch
            }
        )
    ...
    def load_transaction_batch(api_factory: lusid.utilities.ApiClientFactory, transaction_batch: list,
                               **kwargs) -> lusid.models.UpsertPortfolioTransactionsResponse:
        """
        ...
        if "scope" not in list(kwargs.keys()):
            raise KeyError("You are trying to load transactions without a scope, please ensure that a scope is provided.")

        if "code" not in list(kwargs.keys()):
            raise KeyError("You are trying to load transactions without a portfolio code, please ensure that a code is provided.")

        return api_factory.build(lusid.api.TransactionPortfoliosApi).upsert_transactions(
            scope=kwargs["scope"],
            code=kwargs["code"],
    ...
    @staticmethod
    @run_in_executor
    def load_holding_batch(api_factory: lusid.utilities.ApiClientFactory, holding_batch: list,
                           **kwargs) -> lusid.models.HoldingsAdjustment:
        """
        ...
        if "effective_at" not in list(kwargs.keys()):
            raise KeyError('''There is no mapping for effective_at in the required mapping, please add it''')
    ...
    @staticmethod
    @run_in_executor
    def load_portfolio_batch(api_factory: lusid.utilities.ApiClientFactory, portfolio_batch: list,
                             **kwargs) -> lusid.models.Portfolio:
        """
        ...
        try:
            return api_factory.build(lusid.api.TransactionPortfoliosApi).get_details(
                scope=kwargs["scope"],
                code=kwargs["code"]
            )
        except lusid.exceptions.ApiException as e:
            if e.status == 404:
                return api_factory.build(lusid.api.TransactionPortfoliosApi).create_portfolio(
                    scope=kwargs["scope"],
                    transaction_portfolio=portfolio_batch[0])
            # Add in here upsert portfolio properties if it does exist


async def load_data(api_factory: lusid.utilities.ApiClientFactory, data_frame: pd.DataFrame,
                    mapping_required: dict, mapping_optional: dict,
                    property_columns: list, properties_scope: str, instrument_identifier_mapping: dict,
                    file_type: str, domain_lookup: dict, **kwargs):
    """
    ...
    cocoon.utilities.verify_all_required_attributes_mapped(
        swagger_dict=cocoon.utilities.get_swagger_dict(api_url=api_factory.api_client.configuration.host),
    ...
    for index, row in data_frame.iterrows():
        ...
        if instrument_identifier_mapping is None or not bool(instrument_identifier_mapping):
            identifiers = None
        else:
        ...
        single_requests.append(cocoon.utilities.populate_model(
            model_object=top_level_model,
...
async def construct_batches(api_factory: lusid.utilities.ApiClientFactory, data_frame: pd.DataFrame,
                            mapping_required: dict, mapping_optional: dict,
                            property_columns: list, properties_scope: str, instrument_identifier_mapping: dict,
                            batch_size: int, file_type: str, domain_lookup: dict, **kwargs):
    ...
    batching_no_portfolios = [file_type for file_type, settings in domain_lookup.items() if not settings["portfolio_specific"]]
    batching_with_portfolios = [file_type for file_type, settings in domain_lookup.items() if settings["portfolio_specific"]]

    if file_type in batching_no_portfolios:

        async_batches = [data_frame.iloc[i:i+batch_size] for i in range(0, len(data_frame), batch_size)]
        ...
        if 'effective_at' in domain_lookup[file_type]["required_call_attributes"]:
            ...
            unique_effective_dates = list(data_frame[mapping_required['effective_at']].unique())

            effective_at_groups = [
                data_frame.loc[
                    data_frame[mapping_required['effective_at']] == effective_at] for effective_at in unique_effective_dates
            ]
            ...
            sync_batches = [
                {
                    "async_batches": [
                        data_frame.loc[
                            data_frame[mapping_required['code']] == code] for code in list(effective_at_group[mapping_required['code']].unique())
                    ],
                    "codes": list(effective_at_group[mapping_required['code']].unique()),
                    "effective_at": [list(effective_at_group[mapping_required['effective_at']].unique())[0]] * len(list(effective_at_group[mapping_required['code']].unique()))
            ...
            async_batches = [
                data_frame.loc[
                    data_frame[mapping_required['code']] == code] for code in unique_portfolios
            ]
    ...
    responses = [await asyncio.gather(
        *[load_data(
            ...
            **kwargs
        ) for async_batch, code, effective_at in zip(sync_batch["async_batches"], sync_batch["codes"], sync_batch["effective_at"])],
        return_exceptions=True
    ) for sync_batch in sync_batches]

    responses_flattened = [response for responses_sub in responses for response in responses_sub]

    # Raise any internal exceptions rather than propagating them to the response
    for response in responses_flattened:
        if isinstance(response, Exception) and not isinstance(response, lusid.exceptions.ApiException):
            raise response
...
@checkargs
def load_from_data_frame(api_factory: lusid.utilities.ApiClientFactory, scope: str, data_frame: pd.DataFrame,
                         mapping_required: dict, mapping_optional: dict, file_type: str,
                         identifier_mapping: dict = None, property_columns: list = None,
                         properties_scope: str = None, batch_size: int = None, instrument_name_enrichment: bool = False):
    """
    ...
    file_type = Validator(file_type, 'file_type')\
        .make_singular()\
        .make_lower()\
    ...
    identifier_mapping = Validator(identifier_mapping, 'identifier_mapping')\
        .set_default_value_if_none(default={})\
        .discard_dict_keys_none_value()\
        .value

    properties_scope = Validator(properties_scope, 'properties_scope')\
    ...
    batch_size = Validator(batch_size, 'batch_size')\
        .set_default_value_if_none(domain_lookup[file_type]["default_batch_size"])\
        .override_value(not domain_lookup[file_type]["batch_allowed"], domain_lookup[file_type]["default_batch_size"])\
        .value

    # Discard mappings where the provided value is None
    mapping_required = Validator(mapping_required, 'mapping_required')\
        .discard_dict_keys_none_value()\
        .value

    mapping_optional = Validator(mapping_optional, 'mapping_optional') \
        .discard_dict_keys_none_value() \
        .value

    if instrument_name_enrichment:
        data_frame, mapping_required = cocoon.instruments.enrich_instruments(
            api_factory=api_factory,
    ...
    '''
    data_frame, mapping_required = cocoon.utilities.handle_nested_default_and_column_mapping(
        data_frame=data_frame,
        mapping=mapping_required,
        constant_prefix="$"
    )
    data_frame, mapping_optional = cocoon.utilities.handle_nested_default_and_column_mapping(
        data_frame=data_frame,
    ...
    nan_columns = [column for column in data_frame_columns if data_frame[column].isna().any()]
    ...
    Validator(mapping_required, 'mapping_required')\
        ...
        .check_subset_of_list(data_frame_columns, "DataFrame Columns")\
        .check_no_intersection_with_list(nan_columns, 'Columns with Missing Values')

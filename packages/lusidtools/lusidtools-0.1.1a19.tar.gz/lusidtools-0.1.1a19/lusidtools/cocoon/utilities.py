...
def populate_model(model_object, required_mapping, optional_mapping, row, properties=None, identifiers=None):
    """
    ...
    mapping_expanded = expand_dictionary(update_dict(required_mapping, optional_mapping))
...
def set_attributes(model_object, mapping, row, properties=None, identifiers=None, sub_holding_keys=None):
    """
...
def expand_dictionary(dictionary) -> dict:
    ...
    for key, value in dictionary.items():
        ...
        update_dict(dict_expanded, expand_dictionary_single_recursive(0, components, value))
...
def get_swagger_dict(api_url) -> dict:
    ...
    if swagger_file.status_code == 200:
        ...
        raise ValueError(f'''Received a {swagger_file.status_code} response from the provided url, please double check
                             the base api url and try again''')


def verify_all_required_attributes_mapped(swagger_dict, mapping, model_object, exempt_attributes=[]) -> None:
    """
    ...
    if mapping is None:
        raise ValueError(f'''No required mapping has been provided. The required attributes {str(required_attributes)} 
    ...
    if len(missing_attributes) > 0:
        raise ValueError(f'''The required attributes {str(missing_attributes)} are missing from the mapping. Please
                             add them.''')
...
def get_required_attributes_model_recursive(swagger_dict, model_object):
    ...
    if 'required' in list(swagger_dict["definitions"][model_object.__name__].keys()):
        required_attributes = swagger_dict["definitions"][model_object.__name__]['required']
    ...
    # This is for cases such as lusid.models.TransactionRequest.transaction_price
        ...
        required_attributes = list(swagger_dict["definitions"][model_object.__name__]['properties'].keys())

    # Get the properties of all the attributes on the model
    attribute_properties = swagger_dict["definitions"][model_object.__name__]['properties']
    ...
    for required_attribute in required_attributes:
        ...
        if nested_model is None:
            ...
            for nested_required_attribute in nested_required_attributes:
                attributes.append(".".join([camel_case_to_pep_8(required_attribute), nested_required_attribute]))
...
def return_nested_model(required_attribute_properties):
    ...
    """
    possible_nested_models = list(gen_dict_extract("$ref", required_attribute_properties))
...
def camel_case_to_pep_8(attribute_name) -> str:
    ...
    """
    matches = re.finditer('.+?(?:(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])|$)', attribute_name)
...
def handle_nested_default_and_column_mapping(data_frame: pd.DataFrame, mapping: dict, constant_prefix: str="$"):
    """
    ...
    for key, value in mapping.items():
        ...
        if isinstance(value, dict):
            ...
            if ('column' in list(value.keys())) and ('default' in list(value.keys())):
                ...
                data_frame[mapping_updated[key]] = data_frame[mapping_updated[key]].fillna(value['default'])

            # If there is only a default specified, create a new column filled with the default
            elif not ('column' in list(value.keys())) and ('default' in list(value.keys())):
                mapping_updated[key] = f"LUSID.{key}"
                data_frame[mapping_updated[key]] = value['default']

            # If there is only a column specified unnest it
            elif ('column' in list(value.keys())) and not ('default' in list(value.keys())):
                mapping_updated[key] = value['column']

            else:
                raise KeyError(f'''You have passed in a dictionary as the value for the mapping for {key}, however 
            ...
            raise ValueError(f'''You have passed in a value with type {type(value)} for the mapping for {key}, this is
...
def checkargs(function):
    ...
    def _f(*args, **kwargs):
        ...
        keyed_arguments = {list(function_arguments.keys())[i]: args[i] for i in range(0, len(args))}
        ...
        for argument_name, argument_value in keyed_arguments.items():
            ...
            if argument_name not in list(function_arguments.keys()):
                raise ValueError(f"The argument {argument_name} is not a valid keyword argument for this function, valid arguments are {str(list(function_arguments.keys()))}")
            ...
            if not isinstance(argument_value, argument.annotation):
                if not is_default_value:
                    raise TypeError(f'''The value provided for {argument_name} is of type {type(argument_value)} not of 
...
def load_data_to_df_and_detect_delimiter(args: dict) -> pd.DataFrame:
    ...
    with open(args["file_path"], "r") as read_file:
        ...
        if not args["delimiter"]:
            args["delimiter"] = detect(header_line).replace("\\", "\\\\")

            if args["delimiter"] == header_line:
                err = f"Unable to detect delimiter from first line of data at line at line number: {args['num_header']}: " \
                      f"\n\t>> " \
                      f"{header_line}"
...
def check_mapping_fields_exist(required_list: list, search_list: list, file_type: str) -> list:
    """
    ...
    missing_fields = [item for item in required_list if item not in search_list and item[0] != "$"]
    if missing_fields:
        raise ValueError(f"{file_type} fields not found in data columns: {missing_fields}")
...
def parse_args(args: dict):
    ...
    ap.add_argument("-lt", "--line_terminator",
                    default=r"\n",
                    help="character that specifies the end of a line, default value is {}".format(r"\n"))
...
def identify_cash_items(dataframe, mappings) -> (pd.DataFrame, dict):
    """
    This function appends a column to a dataframe indicating cash transactions or holdings, and adds that column
    header to the "Currency" instrument identifier in the mapping structure
    ...
    for index, row in dataframe.iterrows():
        for column in cash_flag_specification["cash_identifiers"].keys():
            if row[column] in cash_flag_specification["cash_identifiers"][column]:
                dataframe.at[index, "currency_identifier_for_LUSID"] = populate_currency_identifier_for_LUSID(row, column, cash_flag_specification)
                break

    return dataframe, mappings


def populate_currency_identifier_for_LUSID(row, column, cash_flag_specification) -> str:
    """
    ...
    :param str row: current data row
    :param str column: current dataframe column that contains values that can be used to identify a cash transaction or
    holding
    ...
    if isinstance(cash_flag_specification["cash_identifiers"][column], dict):
        if row[column] in cash_flag_specification["cash_identifiers"][column]:
            ...
            currency_code = cash_flag_specification["cash_identifiers"][column][row[column]]
        elif "implicit" in cash_flag_specification.keys():
            logging.debug("couldn't find currency code in explicit definition. ")
            currency_code = row[cash_flag_specification["implicit"]]
        else:
            ex = f"failed to find currency code either explicitly in cash_flag or implicitly from currency column " \
        ...
        if "implicit" in cash_flag_specification.keys():
            logging.info("No currency codes explicitly specified, attempting to get implicitly from currency code "
                         "column")
            currency_code = row[cash_flag_specification["implicit"]]
        else:
            err = f"No cash identifiers were specified as a list, without any explicit currency codes and no 'implicit'" \
        ...
        logging.error(f"cash_flag not configured correctly. 'cash_identifiers' must be dictionary (explicit) or list "
                      f"(for implicit), but  got {type(cash_flag_specification['cash_identifiers'])}")
        raise ValueError(f"cash_flag not configured correctly. 'cash_identifiers' must be dictionary (explicit) or "
...
def validate_mapping_file_structure(mapping: dict, columns: list, file_type: str):
    ...
    if "required" in mapping.keys():
        check_mapping_fields_exist(mapping["required"].values(), columns, "required")
    else:
    ...
    if "identifier_mapping" in mapping.keys():
        check_mapping_fields_exist(mapping["identifier_mapping"].values(), columns, "identifier_mapping")
    else:
        raise ValueError(f"'required' mapping field not provided in mapping")

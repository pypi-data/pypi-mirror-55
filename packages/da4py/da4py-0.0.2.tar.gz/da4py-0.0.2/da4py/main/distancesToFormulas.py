...
def levenshtein(s, t,wt="w",silent="tau"):
    '''
    Levenshtein distance between two WORDS as lists
    :param s: first word
    :param t: second word
    :param wt: wait letter to complete differences
    :param silent: letter that doesn't cost
    :return : int
    '''
    if len(s) == 0 :
        return len(t)
    if len(t) == 0 :
        return len(s)
    if s[-1] in [wt,silent,None]:
        return levenshtein(s[:-1], t)
    if t[-1] in [wt,silent,None] :
...
def hamming(s,t,wt="w",silent="tau"):
    '''
    Hamming distance between two WORDS as lists
    :param s: first word
    :param t: second word
    :param wt: wait letter to complete differences
    :param silent: letter that doesn't cost
    ...
    '''
    if len(s) == 0 :
        while "w" in t: t.remove(wt)
        return len(t)
    if len(t) == 0 :
        while "w" in s: s.remove(wt)
        return len(s)
    if s[0] == t[0] or s[0] in [wt,silent,None] or t[0] in [wt,silent,None] :

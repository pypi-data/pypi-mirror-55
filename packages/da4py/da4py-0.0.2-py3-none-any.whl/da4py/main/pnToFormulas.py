...
def is_run(size_of_run, places, transitions, m0, mf,m_ip, tau_it,reachFinal):
    '''
    ...
    if reachFinal:
        for m in mf:
            positives.append(m_ip([size_of_run,places.index(m)]))
...
def petri_net_to_SAT(net, m0, mf, variablesGenerator, size_of_run, label_m="m_ip", label_t="tau_it",
                     silent_transition=None,reachFinal=False):
    '''
    ...
    return (is_run(size_of_run, places, transitions, m0, mf,variablesGenerator.getfunction(label_m),
                   variablesGenerator.getfunction(label_t),reachFinal), places, transitions,silent_transitions)

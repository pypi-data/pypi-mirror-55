...
class DateData:
    ...
    def last_day(d, day_name):
        ...
        the function returns the last sunday.
    ...
    def only_datemonth(self):
        ...
        for month, syns in self._dates.items():
            for syn in syns:
                ...
                result = re.findall(pattern, self._query+" ")
                #extends adds values from another list to the same list. This is done because refindall returns in a list of its own.
                results_all.extend(result)
        try:
    ...
    def only_lm(self):
        '''
        --Method to capture l2m,l3m, p3m only
        '''
    ...
    def only_ordverbweek(self):
        '''
        --Method to capture '<ordinal> (first, second, ...) week of <month> + to  
        '<ordinal> (first, second,...) week of <month>
        '''
        ...
        for i, ordinal in enumerate(self._ordinals[:NUM_WEEKS]):
            ...
            try:
                 if match[0][0]!='':
                    ...
                    # print([(start_day,month,year),(start_day+7,month,year)])
    ...
    def only_ordnumweek(self):
        ...
        week_list=[]
        for num,_month in self._dates.items():
            result = re.findall(pattern.replace('<month>',_month[0].lower()),self._query.lower())                               
            if len(result)>0:
                for result_each in result:
                    if result_each[1]!='':
                        month_list.append(num)
                    if result_each[0]!='' and result_each[1]!='':
                        week_list.append(int(result_each[0]))  
        
                
        if (len(month_list) ==1)  and (len(week_list)== 1) :
            if week_list[0] == 1:
                return [(1,month_list[0],year),(7,month_list[0],year)]
            if 5>week_list[0]>1:
                week_list[0]-=1
                return [(week_list[0]*7+1,month_list[0],year),(week_list[0]*7+7,month_list[0],year)]
        
        elif (len(month_list)==0)  and (len(week_list)==1):
            if week_list[0] == 1:
                return [(1,month,year),(7,month,year)]
            if 5>week_list[0] >1:
                week_list[0]-=1
                return [(week_list[0]*7,month,year),(week_list[0]*7+7,month,year)]

        elif (len(month_list)==1) and (len(week_list)== 2):
            if week_list[0]== 1 and 5>week_list[1] > 1:
                week_list[1]-=1
                return [(1, month_list[0], year), (week_list[1]*7+7, month_list[0], year) ]
            
            if 5>week_list[0] >1 & 5>week_list[1] >1:
                week_list[0] -=1
                week_list[1] -=1
                return [(week_list[0]*7+1,month_list[0],year), (week_list[1]*7, month_list[0], year)]

        elif (len(month_list)== 2) and (len(week_list)== 2) :
            if week_list[0]==1 and week_list[1]==1:
                return [(1, month_list[0], year), (1, month_list[1], year)]

            if week_list[0]== 1 and 1<week_list[1]<5:
                week_list[1]-=1
                return [(1, month_list[0], year), (week_list[1]*7+7, month_list[1], year) ]

            if 1<week_list[0]<5 and 1<week_list[1]<5:
                week_list[0] -=1
                week_list[1] -=1
                
                return[(week_list[0]*7+1, month_list[0], year), (week_list[1]*7+7, month_list[1], year)]
    ...
    def compilation_steps( self, result):
    ...
    def compile_dates(self):
        ...
        if len(self.only_monthytd()) > 1:
            ...
            result=self.compilation_steps(result)
            return result
...
if __name__ == '__main__':
    ...
    # pick_dates = DateData("My performance from 27 July to 31 July?")
    # formatted = pick_dates.compile_dates()
    ...
    # # pick_dates = DateData("Sales return from 1st week of October to 3rd week of November")
    # # formatted = pick_dates.compile_dates()
    # # print(formatted )
    # # #fails here 


    # pick_dates = DateData("Sales return  in 3rd week of November")
    # formatted = pick_dates.compile_dates()
    # print(formatted )
    # #codebase designed to handle "Xst/rd week of Month type"


    # pick_dates = DateData("Performance between August and October")
    # formatted = pick_dates.compile_dates()
    # print(formatted )
    # #codebase works for these types

    # pick_dates = DateData("Performance between 3rd week of August and 4th week of October")
    # formatted = pick_dates.compile_dates()
    # print(formatted )
    # #new test case, check 

    # pick_dates = DateData("Sales return from 2nd week October to 3rd week of November")
    # formatted = pick_dates.compile_dates()
    # print(formatted)
    # #was failing, now working

    # pick_dates = DateData("Sales return from 2nd week of October  to 3rd week of October")
    # formatted = pick_dates.compile_dates()
    # print(formatted)
    # #not failing

    # pick_dates = DateData("Sales return in the third week of May")
    # formatted = pick_dates.compile_dates()
    # print(formatted )
    # #still failing, something needs to be done

    # # pick_dates = DateData("Sales return from second  week of June to second week of October ")
    # # formatted = pick_dates.compile_dates()
    # # print(formatted )
    # # #still failing, something needs to be done

...
class Model:
    ...
    def __init__(self):
        ...
        self.extra_params = {}
        self.__extra_param_index_map = []
        self.location_params = {}
        self.__location_param_index_map = ["x", "y", "z", "alpha", "beta", "gamma"]
    ...
    def _init_from_metadata(self):
        ...
        for index, param in enumerate(e_params):
            self.__extra_param_index_map.append(param["name"])
    ...
    def serialize(self):
        ...
        # extraparams
        for i, param_name in enumerate(self.__extra_param_index_map):
    ...
    def load_from_dictionary(self, json):
        ...
        for param_index in range(len(json.get("ExtraParameters", []))):
            param_name=self.__extra_param_index_map[param_index]
    ...
    def get_mutable_params(self):
        ...
        mut_array = []
        # location params
        for param_name in self.__location_param_index_map:
            if self.location_params[param_name].mutable:
                mut_array.append(self.location_params[param_name])

        # mutable params
        for param_name in self.__extra_param_index_map:
    ...
    def _basic_json_params(self):
        ...
        * extraparams[i]
        ...
        # add extra params
        for param in self.__extra_param_index_map:
...
class ModelWithLayers(Model):
    ...
    def default_layer(self):
        return ParameterContainer(self._default_layer)
    ...
    def _init_from_metadata(self):
        ...
        self.__layer_param_index_map = self._metadata["layers"]["params"]
        self.layer_params=Layers(min_length=self._metadata["layers"]["min"], max_length=self._metadata["layers"]["max"])
        for layer in layerinfo:
            layer_dict = {}
            for param_index, parameter in enumerate(self.__layer_param_index_map):
                if layer["index"] == -1:
                    # This is just an indication of the default layer when more layers are added
                    # it is not an actual layer.
                    self._default_layer[parameter] = Parameter(value=layer["defaultValues"][param_index], name=parameter)
                else:
    ...
    def parameters_to_json_arrays(self):
        ...
        for layer in self.layer_params:
            ...
            mut_array = []
            sigma_array = []
            for i, param_name in enumerate(self.__layer_param_index_map):
    ...
    def load_from_dictionary(self, json):
        ...
        for layer_index in range(len(json["Parameters"])):
            ...
            for param_index in range(len(json["Parameters"][layer_index])):
                param_name=self.__layer_param_index_map[param_index]
    ...
    def get_mutable_params(self):
        ...
        # layer params
        for layer in self.layer_params:
            for param_name in self.__layer_param_index_map:
    ...
    def _basic_json_params(self):
        ...
        # add params:
        for param in self.__layer_param_index_map:
            for layer in self.layer_params:

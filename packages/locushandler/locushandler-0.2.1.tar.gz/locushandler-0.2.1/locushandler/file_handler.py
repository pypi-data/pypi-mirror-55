...
def parse_file(file_path_or_df, cols_of_interest, output_type, granularity, show_dr=True, show_io=True,
    ...
    try:
        ...
        for col in cols_of_interest:
            ...
            df_data[f'{col}_parsed'] = df_data[f'{col}_parsed'].apply(
                                               lambda x: [np.nan] * len(col_elements) 
                                               if type(x) != list else x)
            df_data[col_elements] = pd.DataFrame(df_data[f'{col}_parsed'].values\
                                                                         .tolist())
        ...
        if not merge:
            df_data.drop(columns=cols_of_interest, inplace=True)
...
def map_to_locus(file_path_or_df, file_code_column, barcode_field, granularity='4x6',
    ...
    if xwalk_path:
        ...
        df_map = get_mapping(barcode_field=barcode_field, xwalk_path=xwalk_path,
                             ...
                             granularity=granularity, show_dr=dr, show_io=io,

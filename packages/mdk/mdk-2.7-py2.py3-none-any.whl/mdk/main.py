...
def mdk(*args, **kwargs):
    ...
    def mdk_status():
        ...
        autogen_file = config.service_dir/config.autogen_dir/("{}.yaml".format(config.autogen_name))
        log("autogeneration:")
        log("  {:12}{}".format("status", "active" if config.autogen_active else "inactive"))
        if config.autogen_active and autogen_file.exists():
            log("  {:12}{}".format("generated", autogen_file.resolve()))

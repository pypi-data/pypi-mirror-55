...
class Config():
    ...
    def __init__(self):
        if not MDK_CONFIG_DIR.exists():
            MDK_CONFIG_DIR.mkdir(exist_ok=True, parents=True)
            os.chown(MDK_CONFIG_DIR, os.geteuid(), os.getgid())
...
class Log():
    ...
    def command(args):
        """success formatting"""
        if args:
            click.echo(click.style("$ " + " ".join(args), fg="cyan"))
...
def docker(args: List[str], detach=False, log_command=True):
    """$ docker [args]"""
...
def docker_compose(args: List[str], detach=False, log_command=True):
...
def autogen_config() -> Union[Dict[str, Any], None]:
    ...
    (config.service_dir / config.autogen_dir).mkdir(exist_ok=True)
    compose_stream = (config.service_dir / config.autogen_path).open("w")

...
from mdk.utils import docker, docker_compose, MDK_CONFIG_DIR
...
def mdk(*args, **kwargs):
    ...
    def mdk_status():
        ...
        local_compose = utils.parse_yaml(config.service_dir/"docker-compose.yaml")
        ...
        ], detach=True, log_command=False).strip(" \n")
        ...
        if config.autogen_active and config.autogen_path.exists():
            ...
            files = [x for x in MDK_CONFIG_DIR.iterdir() if x.suffix in [".yml", ".yaml"]]
            if files:
                ...
                log("No overrides loaded from {}".format(MDK_CONFIG_DIR))

...
def write_shared_config():
    ...
    service_blocks = {}
    for service, service_conf in original_compose_contents["services"].items():
        if "container_name" in service_conf:
            container_name = service_conf["container_name"]
            history_file = "/mdkshared/.{}.bash_history".format(container_name)
        else:
            container_name = "container_name not set for {}".format(service)
            history_file = "/mdkshared/.bash_history"

        service_blocks[service] = {
            "environment": {
                "DOCKER_CONTAINER_NAME": container_name,
                "DOCKER_CONTAINER_ROOT": '~/' + str(
                    config.service_dir.relative_to(Path.home()),
                ),

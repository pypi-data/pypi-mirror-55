...
class Aggregation(Serializer, TimeBased):
    ...
    idaggregation = Column('idaggregation', BigInteger, primary_key=True, autoincrement=True, nullable=False)
    timestamp = Column('timestamp', DATETIME(fsp=6), nullable=False, unique=True)

    @declared_attr
    def iddrive(cls):
        return Column('iddrive', ForeignKey('drive.iddrive', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True,
...
class Drive(Base, Serializer):
    ...
    iddrive = Column(BigInteger, primary_key=True, autoincrement=True, nullable=False)
...
class Enrichment(Serializer, TimeBased):
    ...
    def idaggregation(cls):
        return Column('idaggregation', ForeignKey("{}.{}".format(Aggregation.__tablename__,
                                                                 Aggregation.idaggregation),
...
class DeclaredInterval(object):
    ...
    @declared_attr
    def start(cls):
        return Column('start', ForeignKey("{}.idaggregation".format(timebased_models.Aggregation.__tablename__),
                                          ondelete='CASCADE', onupdate='CASCADE'), nullable=False, index=True)

    # the id of the scene as the end of the maneuver
    @declared_attr
    def end(cls):
        return Column('end', ForeignKey("{}.idaggregation".format(timebased_models.Aggregation.__tablename__),
                                        ondelete='CASCADE', onupdate='CASCADE'), nullable=False, index=True)

    @declared_attr
    def started_by(cls):
        return relationship('Aggregation',
                            primaryjoin='{}.c.idaggregation == {}.c.start'.format(
    ...
    def end_by(cls):
        return relationship('Aggregation',
                            primaryjoin='{}.c.idaggregation  == {}.c.end'.format(
                                timebased_models.Aggregation.__tablename__,
                                cls.__tablename__))
...
class SignalAggregatedBaseModel(object):
    ...
    # @declared_attr
    # def aggregation(cls):
    #     return relationship(Aggregation.__tablename__)

    @declared_attr
    def idaggregation(cls):
        return Column("idaggregation", BigInteger,
                      ForeignKey("{}.idaggregation".format(Aggregation.__tablename__),

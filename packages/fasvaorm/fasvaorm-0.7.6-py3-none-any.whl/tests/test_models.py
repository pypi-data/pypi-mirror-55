...
class TestManeuver(EngineTestCase):
    ...
    def test_add_entity(self):
        ...
        agg1 = timebased_models.Aggregation(timestamp=datetime.now())
        agg1.drive = drive

        agg2 = timebased_models.Aggregation(timestamp=datetime.now())
        agg2.drive = drive

        session.add_all([agg1, agg2])
        session.commit()
        mano = timebased_models.Maneuver(name='testmaneuver')
        mano.start = agg1.idaggregation
        mano.end = agg2.idaggregation
...
class TestDrivingPrimitive(EngineTestCase):
    ...
    def test_add_entity(self):
        ...
        agg1 = timebased_models.Aggregation(timestamp=datetime.now())  # type: fasvaorm.models.Aggregation
        agg1.drive = drive

        agg2 = timebased_models.Aggregation(timestamp=datetime.now())  # type: fasvaorm.models.Aggregation
        agg2.drive = drive

        session.add_all([agg1, agg2])
        session.commit()

        mano = timebased_models.Maneuver(name='testmaneuver')  # type: fasvaorm.models.Maneuver
        mano.start = agg1.idaggregation
        mano.end = agg2.idaggregation
    ...
    def test_add_driving_primitive(self):
        ...
        dpid = timebased_models.DrivingPrimitiveInDrive()

        dpid.driving_primitive = dp
        dpid.start = agg1.idaggregation
        dpid.end = agg2.idaggregation

...
def train_dataset(dataset, algorithm, algorithm_args, out_dir,
    ...
    # Encode the labels
    sample_labels_encoder = LabelEncoder()
    sample_labels_encoder.fit(dataset['class'].values)
    # Split the dataset,
    model, xcols, test = init_model_and_train(dataset,
                                              algorithm,
                                              algorithm_args,
                                              out_dir,
                                              sample_labels_encoder,
                                              config_dict)

    model_dict = {'app_algorithm': algorithm,
                  'model': model,
                  'labels': sample_labels_encoder,
                  'names': xcols,
...
def init_model_and_train(dataset, algorithm, algorithm_args, out_dir,
                         sample_labels_encoder, config_dict):
    """

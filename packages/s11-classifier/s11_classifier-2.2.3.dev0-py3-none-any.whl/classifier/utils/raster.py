...
def stitch(out_prefix, model_dict, meta):
    """
     Stitch the temporary files together
    Args:
        out_prefix (str): The output directory
        model_dict (dict): Dictionary with model parameters
    ...
    for directories in dirs:
        if directories.startswith('class'):
            ...
            external_label = str(model_dict['labels'].inverse_transform(
                [int(directories)])[0])
            out_file = os.path.join(out_prefix, external_label)
            dtype = 'Float32'

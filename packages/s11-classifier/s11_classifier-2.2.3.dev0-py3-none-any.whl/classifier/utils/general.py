...
def cli_init(output_name, rasters, overwrite):
    ...
    config_dict = read_config(WORKSPACE)
...
def save_model(model_dict, out_dir, config_dict):
    """Save the model file and metadata  as a pickle model file
    ...
    """
    with open(os.path.join(out_dir, '{}.model'.format(config_dict['name'])),
              'wb') as model_file:

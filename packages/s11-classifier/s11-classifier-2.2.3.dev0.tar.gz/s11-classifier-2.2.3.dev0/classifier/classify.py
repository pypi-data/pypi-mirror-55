...
from classifier.utils.general import multiprocess_function, progress, save_model
...
def read_model(path):
    ...
    Returns:
        a dictionary containing a model, label encoder and test data

    """
    # Use the model given as argument in the command line
    with open(os.path.join(WORKSPACE, path), 'rb') as pfile:
...
def predict(model_dict, rasters, out_dir, config_dict):
    ...
    stitch(out_dir, model_dict, meta)
    CLASSIFIER_LOGGER.info(

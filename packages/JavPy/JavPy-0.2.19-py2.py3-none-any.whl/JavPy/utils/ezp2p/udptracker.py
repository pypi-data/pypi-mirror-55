...
def int2ipv4(int_ip):
    return "%u.%u.%u.%u" % ((int_ip & 0xFF000000) >> 24,
...
if __name__ == '__main__':
    ...
    data = struct.pack('!QII20s20sQQQIIIiH', connection_id, 1, transaction_id,
                       info_hash, peerid, 0, 0, 0, 0, 0, key, -1, udpc.local[1])

    udpc.send(data)
    print(udpc.recv)
    print(len(udpc.recv))
    _, transaction_id, interval, leechers, seeders, ip, port = struct.unpack("!IIIIIIH", bytes(udpc.recv))

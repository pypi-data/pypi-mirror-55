    ...
    if fname.lower().endswith(".mat"):
        ...
        def convert_to_lists(df):
            if df.shape[1] > 1:
                ...
                # If a list of strings replace missing vals with ""
                for str_list in ["FGAS_FILES_", "MHALO_FILES_"]:
                    if col_name.startswith(str_list):
                        df = df.fillna("")
...
def read_csv_tuningcore(
    ...
    scensets = _read_scenset(join(data_dir, scensets_fname))
        ...
        for s in scenarios:
            ...
            for k in p:
                if isinstance(p[k], str):
                    if '{' in p[k]:
                        model_variant = "{}_{}".format(model["code"], model["name"])
                        p[k] = p[k].format(
                            MODEL=model["code"],
                            VARIANT=model["name"],

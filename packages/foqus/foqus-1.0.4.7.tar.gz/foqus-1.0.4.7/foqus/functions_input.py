...
def generate_more_images(training_path, folders, max_nb):
    for id_categorie in folders:
        ...
        for j in os.listdir(DIR):
            ext = imghdr.what(DIR + "/" + j)
            if (ext == 'png' and j.split(".")[1] in ['jpg', 'jpeg', 'JPG', 'JPEG']) or \
                    (ext in ['jpeg', 'jpg'] and j.split('.')[1] in ['png', 'PNG']) or (ext is None):
                os.remove(DIR + "/" + j)
                logger.info("Augmontor:Deleting_jpg as it contains png encoding - " + str(DIR + "/" + j))

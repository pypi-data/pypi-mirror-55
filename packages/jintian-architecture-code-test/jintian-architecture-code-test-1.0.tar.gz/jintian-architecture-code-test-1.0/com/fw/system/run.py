...
from com.fw.utils.time_utils import TimeUtils
...
class System(object):
    def __init__(self):
        ...
        try:
            ...
            self.docker_name = system_conf.get_value('environment', "docker_name")
        except Exception as e:
            raise BaseException("no apidoc ip", e)

        try:
            self.docker_prot = system_conf.get_value('environment', "docker_prot")
        except Exception as e:
            self.docker_prot = 8000
    ...
    def chmod_restart_sh(self):
        path = "{}/restart.sh".format(self.work_dir)
        with open(path, "w+") as file:
            file.write("#! /bin/bash \ndocker stop $1 \ndocker start $1 ")

        result = os.popen("chmod 777 {}".format(path))
        logger.warn(result._stream.read())

    def restartService(self):

        url = "http://{}:{}/test/hello".format(self.ip, self.docker_prot)
        logger.warn("【自检】{}:准备自检{}....".format(TimeUtils.getCurrentDateTime(), url))

        try:
            response = requests.get(url, timeout=5)
        except exceptions.Timeout as e:
            logger.warn("测试请求超时，准备执行重启脚本...")
            os.popen("nohup  {}/restart.sh {}".format(self.work_dir, self.docker_name))
        else:
            if response.status_code != 200:
                logger.warn("请求失败code :{}，准备执行重启脚本...".format(response.status_code))
                os.popen("nohup {}/restart.sh {}".format(self.work_dir, self.docker_name))
    ...
    def run(self, after=None):
        ...
        task.add_task_interval(self.restartService, minute="*/5")
    ...
    def init_route(self):
        ...
        for file_path in route_files:
            ...
            if not route.route:
                logger.error("ERROR {} : NOT ROUTE OR SUFFIX......".format(file_path))
                continue

            route_ex = route.route

            if not route_ex.path or not route_ex.suffix:

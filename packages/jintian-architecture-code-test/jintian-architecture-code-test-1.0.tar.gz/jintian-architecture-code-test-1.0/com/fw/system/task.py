...
class Task(object):
    ...
    def __init__(self):
        job_defaults = {
            ...
            'max_instances': 50  # 设置调度程序将同时运行的特定作业的最大实例数3
        }
    ...
    def add_task_interval(self, func, args=[], kwargs={}, year=None, month=None, day=None, hour=None, minute=None,
                          second=None, id=None):

        self.scheduler.add_job(func, 'cron', args=args, kwargs=kwargs, year=year, month=month, day=day,
                               hour=hour, minute=minute, second=second, id=id, timezone=pytz.timezone('Asia/Shanghai')
                               )

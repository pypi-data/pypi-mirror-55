...
class SpriteSheet():
    ...
    def __create_sprite(self, label, label_map, pxl_count):
    ...
    def find_sprites(self):
        ...
        check_map = [[False for col in row] for row in lst_pixel]
        ...
        for row_idx, row in enumerate(lst_pixel):
            for col_idx, point in enumerate(row):   
                ...
                if not self.__is_background(point) and \
                    ...
                    pxl_count = self.__find_whole_sprite(label_map, lst_pixel, 
                                        check_map, row_idx, col_idx, label)
                    sprites[label] = self.__create_sprite(label, label_map, pxl_count)

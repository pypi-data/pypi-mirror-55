...
class HTTPResult(BaseResult):
    def response(
        ...
        **kwargs,
        ...
        :param adjusted: 是否需要对xlsx文件列宽进行自适应调整
        ...
        if status == 422:
            ...
            if isinstance(data, collections.Iterable) or (
                ...
                httpResponse = handleContentType(
                    self.request, data, filename, header, extractor, adjusted
                )
                if httpResponse:
                    return httpResponse
...
def handleContentType(
    ...
    if filename.endswith("xlsx"):
        return handleXlsx(request, data, filename, header, extractor, adjusted)


def handleXlsx(
    request: Request, data: list, filename, header, extractor: typing.Callable, adjusted,
) -> HttpResponse:
    ...
    cnHeader = header.translate() if header else []
    columns = header.all if header else []
    if adjusted:
        with io.BytesIO() as fd:
            ...
            for idx, col in enumerate(df):
                series = df[col]
                describe = series.astype(str).map(len).describe()
                 
                max_len = max(ceil(describe["75%"] + describe["std"]), len(str(series.name)))
                max_len = max_len+1 if max_len < 100 else 100
    ...
    response[
        "Content-Disposition"
    ] = f"attachment; filename={urllib.parse.quote_plus(filename)}"
    return response

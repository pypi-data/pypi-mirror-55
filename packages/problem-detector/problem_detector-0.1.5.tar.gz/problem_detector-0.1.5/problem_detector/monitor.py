...
class Monitor(object):
    ...
    def __init__(self):
        ...
        config_file_path = os.path.join(pkg_path, 'config/policy_config.yaml')
            ...
            for mtr_status in self.mtr_status_policy:
                if current_status == mtr_status and current_status != self.last_status:
                    ...
                    if self.policy[mtr_status]['message'] and not self.policy[mtr_status]['log']:
                        self.client.post_message(message="{0} is {1}!".format(self.robot_uuid, mtr_status))
                    elif self.policy[mtr_status]['message'] and self.policy[mtr_status]['log']:
                        self.client.post_message(message="{0} is {1}!".format(self.robot_uuid, mtr_status))
                        self.client.post_message(message="Creating Log...")
                        tarfile = self.run_log_aggregator(mtr_status)
                        self.client.post_message(message="Uploading Log...")
                        self.client.post_file(file=open('/home/yujin/{}.tar.gz'.format(tarfile), 'rb'))

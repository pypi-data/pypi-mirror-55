...
from ml4chem.metrics import compute_mse
...
class train(object):
    ...
    def __init__(
        ...
        logger.info(
            "{:6s} {:19s} {:12s} {:8s} {:8s}".format(
                "Epoch", "Time Stamp", "Loss", "MSE/img", "MSE/atom"
            )
        )
        logger.info(
            "{:6s} {:19s} {:12s} {:8s} {:8s}".format(
    ...
    def trainer(self):
        ...
        while not converged:
            ...
            mse = client.submit(compute_mse, *(outputs_, self.targets))
            atoms_per_image = self.atoms_per_image.view(1, -1)
            mse_atom = client.submit(
                compute_mse, *(outputs_, self.targets, atoms_per_image)
            )
            mse = mse.result()
            ...
            logger.info(
                "{:6d} {} {:8e} {:8f} {:8f}".format(epoch, ts, loss, mse, mse_atom)
            )

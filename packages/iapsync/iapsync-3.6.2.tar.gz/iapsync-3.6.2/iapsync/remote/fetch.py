...
def get_products(api_meta, options):
    ...
    for mt in metas:
        ...
        for p in product_list:
            ...
            for lc in locates:
                desc = {
                    defs.KEY_TITLE: k_m[lc][defs.KEY_TITLE](p).strip(),
                    defs.KEY_DESCRIPTION: k_m[lc][defs.KEY_DESCRIPTION](p).strip(),
                }
                # 配置文件的bug导致商品描述为空时，会返回字符串'None'
                if desc[defs.KEY_DESCRIPTION] == 'None':
                    desc[defs.KEY_DESCRIPTION] = ''
                new_item[lc] = desc

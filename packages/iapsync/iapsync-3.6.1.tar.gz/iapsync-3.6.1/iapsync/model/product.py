...
class AppStoreProduct:
    ...
    def create_node(p_dict):
        ...
        locs = p_dict['locales']
        for loc in locs:
            title = p_dict[loc][defs.KEY_TITLE].strip() if p_dict[loc][defs.KEY_TITLE] else ''
            pm.set_title(title, loc)
            desc = p_dict[loc][defs.KEY_DESCRIPTION].strip() if p_dict[loc][defs.KEY_DESCRIPTION] else ''
            pm.set_description(desc, loc)

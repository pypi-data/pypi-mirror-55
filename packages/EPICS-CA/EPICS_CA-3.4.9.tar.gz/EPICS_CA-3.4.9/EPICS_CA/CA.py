...
class PV_info:
    ...
    def __init__(self):
        ...
        self.subscription_ID = (
            None
        )  # locally assiged reference number for server updates
...
class PV(object):
    ...
    def get_info(self):
        return cainfo(self.name, printit=False)
...
class Record(object):
    ...
    def __repr__(self):
        return "Record(%r)" % self.__prefix__
...
def camonitor(PV_name, writer=None, callback=None, new_thread=True):
    ...
    if callback is not None:
        if not has_callback(PV_name, callback):
            ...
            warn(
                "camonitor: %r already has %r as callback."
...
def camonitor_clear(PV_name, writer=None, callback=None):
    ...
    if PV_name in PVs:
        ...
        if writer is None:
            pv.writers = []
        elif writer in pv.writers:
            pv.writers.remove(writer)
        if callback is None:
            pv.callbacks = []
        else:
...
def camonitors(PV_name=None):
    ...
    if PV_name is not None:
        PV_names = [PV_name] if PV_name in PVs.keys() else []
    else:
        PV_names = PVs.keys()
...
def has_callback(PV_name, callback):
    ...
    for f in pv.callbacks:
        if f == callback:
            return True
        if hasattr(f, "function") and f.function == callback:
            return True
    return False
...
class Callback(object):
    ...
    def __call__(self, *args):
        if self.new_thread:
            function = new_thread_function(self.function)
        else:
...
def call(function, args, new_thread=False):
    ...
    """
    if new_thread:
        function = new_thread_function(function)
    function(*args)
...
def new_thread_function(function):
    ...
    def function_error_logged(*args):
        try:
            ...
            error("%s: %s\n%s" % (object_name(function), msg, traceback.format_exc()))
...
def PV_server_connect(PV_name):
    ...
    if PV_name in PVs:
        ...
        if pv.addr is not None and pv.addr not in connections:
            ...
            try:
                s.connect((addr, cport))
            except error as msg:
                if DEBUG:
                    debug("%s:%r: %r" % (addr, cport, msg))
                return
            except socket_timeout:
                if DEBUG:
                    debug("%s: timeout" % (addr))
                return
            ...
            send(
                s,
                message(
                    CLIENT_NAME, 0, 0, 0, 0, 0, str.encode(getuser(), "ascii") + b"\0"
                ),
            )
            send(
                s,
                message(
                    HOST_NAME, 0, 0, 0, 0, 0, str.encode(gethostname(), "ascii") + b"\0"
                ),
            )
...
def PV_subscribe(PV_name):
    ...
    if PV_name in PVs:
        pv = PVs[PV_name]
        if (
            pv.subscription_ID is None
            and pv.channel_SID is not None
            ...
            send(
                ...
                message(
                    ...
                    pv.channel_SID,
                    pv.subscription_ID,
                    pack(">fffHxx", 0.0, 0.0, 0.0, VALUE | LOG | ALARM),
                ),
            )
...
def process_replies(timeout=0.0000001, update=False):
    ...
    if lock.acquire(False):
        ...
        while True:
            ...
            try:
                ready_to_read, x, in_error = select(sockets, [], sockets, timeout)
            except select_error:
            ...
            if UDP_socket in ready_to_read:
                try:
                    messages, addr = UDP_socket.recvfrom(2048)
                except socket.error:
                    messages = ""
            ...
            for addr in connections.keys():
                ...
                if s in in_error:
                    if DEBUG:
                        debug("Lost connection to server %s:%s" % addr)
                    reset_PVs(addr)
                ...
                if s in ready_to_read:
                    ...
                    try:
                        data_received = s.recv(65536)
                    except socket.error:
                        if DEBUG:
                            debug("Recv: lost connection to server %s:%s" % addr)
                        reset_PVs(addr)
                        del connections[addr]
                        continue
                    if len(data_received) == 0:
                        if DEBUG:
                            debug("Server %s:%s closed connection" % addr)
                        reset_PVs(addr)
                        del connections[addr]
                        break
                    if DEBUG:
                        debug("CA: Received %d bytes" % len(data_received))
                    ...
                    while len(connection.input_buffer) >= min_message_size:
                        ...
                        padded_payload_size = int(
                            ceil(payload_size / float(pad_unit)) * pad_unit
                        )
                        ...
                        message, connection.input_buffer = (
                            connection.input_buffer[0:message_size],
                            connection.input_buffer[message_size:],
                        )
                        ...
                        if DEBUG:
                            debug("CA: Received " + message_info(message))
                        process_message(addr, message)
...
def process_pending_connection_requests():
    ...
    for name in list(PVs.keys()):
        ...
        if pv.subscription_ID != None:
            continue
        # Is connection already in progress?
        if time() - pv.connection_initiated < timeout:
            continue
        # To Do: retry after timeout
        if DEBUG:
            debug("Processing connection request for PV %r" % name)
...
def process_pending_write_requests():
    ...
    for name in list(PVs.keys()):
        pv = PVs[name]
        if pv.write_data is None:
            continue  # nothing to do
        if pv.addr is None:
            continue  # need to postpone
        if pv.channel_SID is None:
            continue  # need to postpone
        if pv.data_type is None:
            continue  # need to postpone

        if DEBUG:
            debug("Processing write request for PV %r" % name)
        s = connections[pv.addr].socket
        pv.IOID = pv.IOID + 1
        pv.write_confirmed = 0
        ...
        send(
            s, message(WRITE_NOTIFY, 0, data_type, count, pv.channel_SID, pv.IOID, data)
        )
        pv.write_sent = time()
        pv.write_data = None
...
def process_message(addr, message):
    ...
    if len(header) < 16:
        if DEBUG:
            debug("process_message: invalid header %r" % header)
        return
    command, payload_size, data_type, data_count, parameter1, parameter2 = unpack(
        ">HHHHII", header
    )
    ...
    if command == SEARCH:  # Reply to a SEARCH request.
        ...
        if DEBUG:
            debug(
                "SEARCH port_number=%r, channel_ID=%r, channel_SID=%r"
        ...
        for name in list(PVs.keys()):
            if PVs[name].channel_ID == channel_ID:
                ...
                if PVs[name].addr != None:
                    if DEBUG:
                        debug(
                            "Ignoring duplicate SEARCH reply for %r from "
                            "%r:%r" % (name, addr[0], addr[1])
                        )
                    continue
                PVs[name].addr = (addr[0], port_number)
                if DEBUG:
                    debug("PVs[%r].addr = %r" % (name, addr))
        ...
        if DEBUG:
            debug(
                "CREATE_CHAN channel_ID=%r, channel_SID=%r" % (channel_ID, channel_SID)
            )
        for name in list(PVs.keys()):
            if PVs[name].channel_ID == channel_ID:
                if PVs[name].channel_SID != None:
                    if DEBUG:
                        debug(
                            "Ignoring duplicate CREATE_CHAN reply for %r from "
                            "%r:%r" % (name, addr[0], addr[1])
                        )
                    continue
                PVs[name].addr = addr
                ...
                if DEBUG:
                    debug("PVs[%r].channel_SID = %r" % (name, channel_SID))
                PVs[name].data_type = data_type
                if DEBUG:
                    debug("PVs[%r].data_type = %r" % (name, data_type))
                PVs[name].data_count = data_count
                if DEBUG:
                    debug("PVs[%r].data_count = %r" % (name, data_count))
                PVs[name].response_time = time()
                PV_subscribe(name)
        ...
        if DEBUG:
            debug("ACCESS_RIGHTS channel_ID %r, %s" % (channel_ID, access_bits))
        for name in list(PVs.keys()):
            if PVs[name].channel_ID == channel_ID:
                PVs[name].access_bits = access_bits
                if DEBUG:
                    debug("PVs[%r].access_bits = %r" % (name, access_bits))
                PVs[name].response_time = time()
        ...
        if DEBUG:
            debug(
                "READ_NOTIFY channel_SID=%r, IOID=%r, value=%r"
                % (channel_SID, IOID, val)
            )
        for name in list(PVs.keys()):
            if PVs[name].channel_SID == channel_SID:
                if DEBUG:
                    debug("PVs[%r].data = %r" % (name, payload))
        ...
        if DEBUG:
            debug(
                "EVENT_ADD status_code=%r, subscription_ID=%r, "
                "data_count=%r, value=%r"
                % (status_code, subscription_ID, data_count, val)
            )
        for name in list(PVs.keys()):
            if PVs[name].subscription_ID == subscription_ID and PVs[name].addr == addr:
                update = True if PVs[name].data is not None else False
                PVs[name].data_type = data_type
                PVs[name].data_count = data_count
                if DEBUG:
                    debug("PVs[%r].data = %r" % (name, payload))
                if PVs[name].data != None:
                    PVs[name].last_updated = t
                PVs[name].data = payload
        ...
        command == EVENT_CANCEL
    ):  # Asynchronous notification that PV not longer exists.
        channel_SID = parameter1
        subscription_ID = parameter2
        if DEBUG:
            debug(
                "EVENT_CANCEL channel_SID=%r, subscription_ID=%r"
        ...
        IOID = parameter2
        if DEBUG:
            debug("WRITE_NOTIFY status_code=%r, IOID=%r" % (status, IOID))
        for name in list(PVs.keys()):
            if PVs[name].IOID == IOID and PVs[name].addr == addr:
                t = time()
                if DEBUG:
                    debug("PVs[%r].write_confirmed = %r" % (name, t))
        ...
        for name in list(PVs.keys()):
            if PVs[name].channel_ID == channel_ID:
                PV_name = name
        if DEBUG:
            debug("NOT_FOUND: %r" % PV_name)
    elif command == VERSION:
        if DEBUG:
            debug("got command VERSION")
    else:
...
def reset_PVs(addr):
    ...
    for name in list(PVs.keys()):
        if PVs[name].addr == addr:
            if DEBUG:
                debug("Resetting PV %r (address %s:%s)" % (name, addr[0], addr[1]))
            PVs[name].reset()
...
def message(
    ...
    if not isinstance(payload, bytes):
        payload = str.encode(payload, "iso-8859-1")
...
def message_info(message):
    ...
    payload = message[16:]
    if len(header) < 16:
        return "invalid message %r" % header
    command, payload_size, data_type, data_count, parameter1, parameter2 = unpack(
        ">HHHHII", header
    )
    s = str(command)
    if command in commands.values():
        s += "(" + command_name(command) + ")"
    s += "," + str(payload_size)
    s += "," + str(data_type)
...
def send(socket, message):
    ...
    try:
        ...
        if DEBUG:
            debug("getpeername: %r" % error)
        return
    if DEBUG:
        debug("Send %s:%s %s" % (addr, port, message_info(message)))
    try:
        socket.sendall(message)
    except socket_error as error:
        if DEBUG:
            debug("Send failed: %r" % error)
...
def sendto(socket, addr, message):
    ...
    if DEBUG:
        debug("Send UDP %s:%s %s" % (addr[0], addr[1], message_info(message)))
    try:
        socket.sendto(message, addr)
    except socket_error as error:
        if DEBUG:
            debug("Sendto %r failed: %r" % (addr, error))
...
def timestamp(data_type, payload):
    ...
    if payload is not None:
        ...
        if data_type.startswith("TIME_") and len(payload) >= header_size:
            ...
            status, severity, seconds_since_1990_01_01, nanoseconds = unpack(
                ">HHII", payload[0:header_size]
            )
...
def value(data_type, data_count, payload):
    ...
    if data_type.startswith("STS_"):
        ...
        if data_type.endswith("STRING"):
            ...
            header_size += 8 + 6 * 2  # unit,limits
        elif data_type.endswith("FLOAT"):
            header_size += 2 + 2 + 8 + 6 * 4  # precision,pad,unit,limits
        elif data_type.endswith("ENUM"):
            header_size += 2 + 16 * 26  # nstrings,strings
        elif data_type.endswith("CHAR"):
            header_size += 8 + 6 * 1 + 1  # unit,limits,pad
        elif data_type.endswith("LONG"):
            header_size += 8 + 6 * 4  # unit,limits
        elif data_type.endswith("DOUBLE"):
            header_size += 2 + 2 + 8 + 6 * 8  # precision,pad,unit,limits
        else:
            if DEBUG:
                debug("value: data type %r not supported\n" % data_type)
    elif data_type.startswith("CTRL_"):
        header_size = 2 + 2  # status,severity
        if data_type.endswith("STRING"):
            pass
        elif data_type.endswith("SHORT"):
            header_size += 8 + 8 * 2  # unit,limits
        elif data_type.endswith("FLOAT"):
            header_size += 2 + 2 + 8 + 8 * 4  # precision,pad,unit,limits
        elif data_type.endswith("ENUM"):
            header_size += 2 + 16 * 26  # nstrings,strings
        elif data_type.endswith("CHAR"):
            header_size += 8 + 8 * 1 + 1  # unit,limits,pad
        elif data_type.endswith("LONG"):
            header_size += 8 + 8 * 4  # unit,limits
        elif data_type.endswith("DOUBLE"):
            header_size += 2 + 2 + 8 + 8 * 8  # precision,pad,unit,limits
        else:
            if DEBUG:
                debug("value: data type %r not supported\n" % data_type)
    ...
    if data_type.endswith("STRING"):
        ...
        if data_count > len(payload) / 2:
            data_count = max(len(payload) / 2, 1)
        payload = payload.ljust(2 * data_count, b"\0")
        value = list(unpack(">%dh" % data_count, payload[0 : 2 * data_count]))
        if len(value) == 1:
            value = value[0]
    elif data_type.endswith("FLOAT"):
        if data_count > len(payload) / 4:
            data_count = max(len(payload) / 4, 1)
        ...
        if data_count > len(payload) / 1:
            data_count = max(len(payload) / 1, 1)
        ...
        if data_count > len(payload) / 8:
            data_count = max(len(payload) / 8, 1)
        ...
        if DEBUG:
            debug("value: unsupported data type %r\n" % data_type)
        value = payload
...
def network_data(value, data_type):
    ...
    if data_type.startswith("STS_"):
        ...
        if data_type.endswith("STRING"):
            ...
            if DEBUG:
                debug("network_data: data type %r not supported\n" % data_type)
    elif data_type.startswith("CTRL_"):
        status = 0  # 0 = normal
        severity = 1  # 1 = success
        payload += pack(">HH", status, severity)
    ...
    if data_type.endswith("STRING"):
        ...
        if isarray(value):
            for v in value:
                payload += pack("b", to(v, int8))
        else:
            payload += pack("b", to(value, int8))
    elif data_type.endswith("LONG"):
        if isarray(value):
            for v in value:
                payload += pack(">i", to(v, int32))
        else:
            payload += pack(">i", to(value, int32))
    elif data_type.endswith("DOUBLE"):
        if isarray(value):
            for v in value:
                payload += pack(">d", to(v, float64))
        else:
            payload += pack(">d", to(value, float64))
    else:
        if DEBUG:
...
def broadcast_addresses_standard():
    ...
    for address in network_interfaces():
        try:
            num_address = inet_aton(address)
        except:
            continue  # E.g. IPv6 address
...
def network_interfaces():
    ...
    try:
        addrinfos += getaddrinfo(gethostname(), 0)
    except:
        pass
...
def local_ip_address():
    ...
    try:
        ...
        return "127.0.0.1"  # Network is unreachable
...
def cainfo(PV_name="all", property=None, printit=None, update=True, timeout=None):
    ...
    if printit is None:
        printit = True if property is None else False

    if PV_name == "all":
        for name in PVs:
            cainfo(name, printit=printit, update=update)
    else:
        if update:
            caget(PV_name, timeout)
        if PV_name in PVs:
            pv = PVs[PV_name]
        else:
            pv = PV_info()
        ...
        if property is None:  # general report
            ...
            if pv.channel_SID:
                ...
                val = "not connected"
            if pv.subscription_ID:
                val += ", receiving notifications"
            ...
            if pv.data_type != None:
                ...
                for t in types:
                    if types[t] == pv.data_type:
                        val = t
            else:
                val = "N/A"
            s += fmt % ("Data type:", val)

            if pv.data_count != None:
                val = str(pv.data_count)
            else:
                val = "N/A"
            s += fmt % ("Element count:", val)

            if pv.data != None:
                val = repr(value(pv.data_type, pv.data_count, pv.data))
            else:
                val = "N/A"
            s += fmt % ("Value:", val)

            if pv.last_updated != 0 and pv.last_updated != timestamp(
                pv.data_type, pv.data
            ):
                t = pv.last_updated

            ...
            if hasattr(PV_object, "value"):
                ...
                if not ("." in attribute or "[" in attribute):
                    try:
                        ...
                        if DEBUG:
                            debug(
                                "Tried setattr(%s,%s,%r): OK" % (name, attribute, value)
                            )
                    except Exception as exception:
                        if DEBUG:
                            debug(
                                "Tried setattr(%s,%s,%r): %s"
...
def call_callbacks(PV_name):
    ...
    if len(PV.callbacks) > 0:
        ...
        for function in PV.callbacks:
            if DEBUG:
                debug("%s: calling '%s'" % (PV_name, object_name(function)))
            task = Thread(
                target=function,
                args=(PV_name, PV.value, char_value),
    ...
    if len(PV.writers) > 0:
        ...
        message = "%s %s %r\n" % (
            PV_name,
            datetime.fromtimestamp(PV.last_updated),
            PV.value,
        )
...
def notify_subscribers_of_value(PV_name, value):
    ...
    if PV_name in PVs and value is not None:
        ...
        for address in list(PV.subscribers):
            ...
            if subscriber.subscription_ID == None:
                continue
...
def start_server():
    ...
    if DEBUG:
        debug(
            "server version %s, listening on TCP/UDP port %d."
            % (__version__, TCP_port_number)
        )
...
class ThreadingTCPServer(socketserver.ThreadingTCPServer, socketserver.ThreadingMixIn):
    ...
    if os.name == "nt":
        allow_reuse_address = False  # Windows
    else:
        allow_reuse_address = True  # Linux and Mac OS X
    # Ctrl-C will cleanly kill all spawned threads
    daemon_threads = True
...
class UDPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        while len(messages) > 0:
            ...
            if reply:
                if DEBUG:
                    debug("%s: returning reply %r" % (addr, message_info(reply)))
...
class TCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        addr = "%s:%d" % self.client_address
        if DEBUG:
            debug("%s: accepted connection" % addr)
        ...
        while 1:
            ...
            try:
                message = self.request.recv(16)
            except socket.error:
                if DEBUG:
                    debug("%s: lost connection\n" % addr)
                break
            if len(message) == 0:
                if DEBUG:
                    debug("%s: client disconnected" % addr)
                break
            if len(message) < 4:
                if DEBUG:
                    debug("excepted 4 received %d bytes" % len(message))
                break
            ...
            if payload_size > 0:
                try:
                    message += self.request.recv(payload_size)
                except socket.error:
                    if DEBUG:
                        debug("%s: lost connection\n" % addr)
            if DEBUG:
        ...
        if DEBUG:
            debug("%s: closing connection" % addr)
        self.request.close()
...
def process_message(address, request):
    ...
    payload = request[16:]
    command_code, payload_size, data_type, data_count, parameter1, parameter2 = unpack(
        ">HHHHII", header
    )
    ...
    if command == "SEARCH":
        ...
        if PV_exists(channel_name):
            if DEBUG:
                debug(
                    "SEARCH,reply_flag=%r,minor_ver=%r,channel_CID=%r,channel_name=%r\n"
        ...
        if reply_flag == 10:
            return message(
                "NOT_FOUND", 0, reply_flag, minor_version, channel_CID, channel_CID
            )
        ...
        if DEBUG:
            debug(
                "CREATE_CHAN channel_CID=%r, minor_version=%r"
        ...
        reply = message(
            "CREATE_CHAN", 0, data_type, data_count, channel_CID, PV.channel_SID
        )
        ...
        for PV_name in list(PVs):
            ...
            if PV.channel_SID == channel_SID:
                ...
                reply = message(
                    "READ_NOTIFY", 0, data_type, data_count, status_code, IOID, data
                )
                return reply
        ...
        if DEBUG:
            debug(
                "EVENT_ADD {data_type:%s, data_count:%r, "
        ...
        for PV_name in list(PVs):
            ...
            if PV.channel_SID == channel_SID:
                PV.subscribers[address] = subscriber_info(
                    subscription_ID, data_type, data_count
                )
                subscriber = PV.subscribers[address]
                status_code = 1  # Normal successful completion
                val = PV_value(PV_name)
        ...
        if DEBUG:
            debug(
                "WRITE_NOTIFY data_type=%r, data_count=%r, channel_SID=%r, "
                "IOID=%r, value=%r\n"
        ...
        for PV_name in list(PVs):
            ...
            if PV.channel_SID == channel_SID:
                if DEBUG:
                    debug("Changing %r to %r\n" % (PV_name, new_value))
                PV_set_value(PV_name, new_value)
                call_callbacks(PV_name)
                status_code = 1  # Normal successful completion
                reply = message(
                    "WRITE_NOTIFY", 0, data_type, data_count, status_code, IOID
                )
                return reply
        ...
        if DEBUG:
            debug(
                "ACCESS_RIGHTS channel_ID=%r, access_bits=%s (ignored)\n"
        ...
        if DEBUG:
            debug(
                "WRITE data_type=%r, data_count=%r, channel_SID=%r, "
                "IOID=%r, value=%r\n"
                % (data_type, data_count, channel_SID, IOID, new_value)
            )
        for PV_name in list(PVs):
        ...
        if DEBUG:
            debug(
                "EVENT_CANCEL {data_type:%s,data_count:%r, "
                "channel_SID:%r,subscription_ID:%r},"
        ...
        for PV_name in list(PVs):
            ...
            if PV.channel_SID == channel_SID:
                if (
                    address in PV.subscribers
                    and PV.subscribers[address].subscription_ID == subscription_ID
                ):
                    del PV.subscribers[address]
                    if DEBUG:
                        debug("Cancelled updates for %r %r" % (PV_name, address))
        ...
        if DEBUG:
            debug(
                "CLEAR_CHANNEL channel_SID=%r, channel_CID=%r"
        ...
        if DEBUG:
            debug("command %r: not supported (yet)\n" % command)
...
def message_info(message):
    ...
    if len(message) < 16:
        return "invalid message %r" % message
    header = message[0:16]
    payload = message[16:]
    command_code, payload_size, data_type, data_count, parameter1, parameter2 = unpack(
        ">HHHHII", header
...
def CA_binary_data(value, data_type=None):
    ...
    if data_type.endswith("STRING"):
        ...
        if DEBUG:
            debug("CA_binary_data: unsupported data type %r\n" % data_type)
        payload += str(value)
...
def isbool(value):
    try:
        return "bool" in repr(type(value))
    except:
        return False

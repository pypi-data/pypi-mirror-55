...
class Bilingual(MultilingualRessourceBase):
    def __init__(self, file_1: str, file_2: str, file_1_encoding: Optional[str] = 'utf-8',
                 file_2_encoding: Optional[str] = 'utf-8'):

        super().__init__(MultilingualRessourceBase.TYPE_BILINGUAL)
    ...
    def read(self, reader=None) -> Tuple[str, str]:
        ...
        with open(self._file_1, 'r', encoding=self._file_1_encoding) as src_file:
            with open(self._file_2, 'r', encoding=self._file_2_encoding) as tgt_file:
                num_line = 0
                while True:
                    ...
                    if text_1_stripped == '':
                        raise ValueError(f'Bilingual file "{self._file_1}" has an empty line at line {num_line}')

                    if text_2_stripped == '':
                        raise ValueError(f'Bilingual file "{self._file_2}" has an empty line at line {num_line}')

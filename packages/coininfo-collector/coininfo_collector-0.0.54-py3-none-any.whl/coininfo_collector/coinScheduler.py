...
class CoinScheduler(CoinSchedulerBaseClass, metaclass=CoinSchedulerSingleton):
    ...
    # coins/markets. with coinPriceMarketGloba
    ...
    def setCoinInfo(self, data):
        CoinSummary().setCoinInfo(data)

    def getCoinInfo(self, coin_id):
        return CoinSummary().getCoinInfoByCoinId(coin_id)

    def GetPrice1day(self, market_id, coin_id):
        return PriceInExchange().getPrice1day(market_id, coin_id)

    def GetPrice1dayMarket(self, market_id):
        return PriceInExchange().getPrice1dayMarket(market_id)

    def SaveCoinPrice(self, market_id, addedInfoMap):
        return PriceInExchange().saveCoinPrice(market_id, addedInfoMap)

    def loadCoinSummary(self):
        return CoinSummary().getLoadedCoinsDump()

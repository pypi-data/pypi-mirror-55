...
from .web_requests_async import async_put
...
class SSFixedJobs:
    ...
    async def sendToTelegramSvc(sendUrl, data):
        try:
            if 'TELEGRAMSVC_ENDPOINT' not in os.environ:
                return
            ...
            if resp is not None:
                ...
                log.error('failed send to TELEGRAMSVC_ENDPOINT. url:{}, doc:{}'.format(
                    sendUrl,
                    ujson.dumps(data, ensure_ascii=False),
                ))
        except Exception as inst:
            print('exception in post to telegram-svc. url:{} data:{}'.format(
                sendUrl,
                ujson.dumps(data, ensure_ascii=False),
            ))
    ...
    async def reportCoinPriceMarket(output, marketId):
        ...
        try:
            ...
            if resp is not None:
                ...
                log.error('report failed CoinPriceMarket. exchageID:{} url:{}'.format(
                    marketId,
                    SSFixedJobs.reportURL_coinMarketPrice,
                ))
        except Exception as inst:

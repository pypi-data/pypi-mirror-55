...
"""
plotpy.gui.widgets.items.shapes
-------------------------------
...
from math import cos, fabs, pi, radians, sin, sqrt
...
class EllipseShape(PolygonShape):
    ...
    def boundingRect(self):
        ...
        xc, yc = 0.5 * (x0 + x1), 0.5 * (y0 + y1)
        phi = radians(compute_angle(xc, yc, x1, y1))

        ux = radius1 * cos(phi)
        uy = radius1 * sin(phi)
        vx = radius2 * cos(phi + pi / 2)
        vy = radius2 * sin(phi + pi / 2)
...
class XRangeSelection(AbstractShape):
    ...
    def boundingRect(self):
        ...
        :return:
        """
        return QRectF(self._min, 0, self._max - self._min, 0)

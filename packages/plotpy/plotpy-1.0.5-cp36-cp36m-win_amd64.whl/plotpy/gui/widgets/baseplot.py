...
from plotpy.gui.config.misc import get_font, get_icon, get_image_layout
...
from plotpy.gui.widgets.events import StatefulEventFilter
...
from plotpy.gui.widgets.items import annotations
from plotpy.gui.widgets.items.curve import (
    CurveItem,
    ErrorBarCurveItem,
    GridItem,
    PolygonMapItem,
)
from plotpy.gui.widgets.items.image import BaseImageItem
from plotpy.gui.widgets.items.shapes import Marker, PolygonShape
from plotpy.gui.widgets.panels import ID_ITEMLIST, PanelWidget
...
class PlotType(Enum):
    ...
    AUTO = (
        1
    )  #: Automatic plot type. The first PlotItem attached to the plot sets the plot type (see CURVE and IMAGE values of the enum). All tools (curve and image related) are registered and accessible depending on the last selected PlotItem.
    CURVE = (
        2
...
class BasePlot(QwtPlot):
    ...
    def __del__(self):
        ...
        if canvas:
            try:
                ...
                # Qt may have already deleted the canvas.
                expected = "wrapped C/C++ object of type QwtPlotCanvas has been deleted"
                if str(exc) != expected:
                    raise
    ...
    def serialize(self, writer, selected=False):
        ...
        Save (serializable) items to HDF5 file:
            * writer: :py:class:`.hdf5io.HDF5Writer` object
            * selected=False: if True, will save only selected items
    ...
    def deserialize(self, reader):
        ...
        Restore items from HDF5 file:
            * reader: :py:class:`.hdf5io.HDF5Reader` object

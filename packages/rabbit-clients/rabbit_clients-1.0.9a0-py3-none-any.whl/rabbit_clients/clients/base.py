...
class ConsumeMessage:
    ...
    def __call__(self, func, *args, **kwargs) -> Any:
        def prepare_channel(*args, **kwargs):
            ...
            logging.info("connection, channel")
            ...
            def message_handler(chan, meth, prop, bod):

                # Utilize module decorator to send logging messages
                json_obj = json.loads(bod)
                if self._logging:
                    log_publisher(send_log)(chan, meth, prop, json_obj)

                if self._publish_queues:
                    queue_publish(func)(json_obj)
                else:
                    func(json_obj)
            ...
            # Consume one message and stop listening
                ...
                if body:
                    ...
                    if self._logging:
                        PublishMessage(queue=self._logging_queue)(send_log)(None, None, None, json.loads(body))

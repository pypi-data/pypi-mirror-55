...
class C_PI_NA_2(BaseAppAsdu):
    """
    Leer Parámetros del punto de medida
    """
    type = 182
    causa_tm = 5
...
class P_ME_NA_2(BaseAppAsdu):
    """
    Parámetros del punto de medida
    """
    type = 129
    causa_tm = 5

    def __init__(self):
        self.direccion_enlace = 1      # DEN
        self.numero_de_puntos = 1      # NPM
        self.direccion_del_punto = 1   # DPM
        self.clave_acceso = 1          # CLS
        self.periodo_integracion = 60  # PINT
        self.num_registros = 180       # PREG
        self.reserva = ''              # RES
        self.res_fabricante = ''       # PFAB


    def from_hex(self, data, cualificador_ev):

        self.direccion_enlace = struct.unpack("H", data[0:2])[0]      # DEN
        self.numero_de_puntos = struct.unpack("B", data[2:3])[0]      # NPM
        self.direccion_del_punto = struct.unpack("H", data[3:5])[0]   # DPM
        self.clave_acceso = struct.unpack("I", data[5:9])[0]          # CLS
        self.periodo_integracion = struct.unpack("B", data[9:10])[0]  # PINT
        self.num_registros = struct.unpack("H", data[10:12])[0]       # PREG
        self.reserva = data[12:40]                                    # RES
        self.res_fabricante = data[40:246]                            # PFAB

...
class JSONFormatter(logging.Formatter):
    ...
    def json_record(self, message, extra, record):
        ...
        extra['so_version'] = platform.release()
        extra['python_version'] = platform.python_version()
        extra['service_ip'] = socket.gethostbyname(socket.gethostname())
        extra['host'] = socket.gethostname()

        if record.exc_info:
            extra['_exc_info'] = self.formatException(record.exc_info)

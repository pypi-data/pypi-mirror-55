...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_get_lat_lon_shape_cube_with_nlat_nlon_info(
        self, test_cube, test_cmip6_output_hfds_with_nlon_nlat_file
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_with_nlon_nlat_file)
    ...
    def test_get_lat_lon_shape_cube_sftlf_gn(
        ...
        assert not any(
            [
                "we don't know what we're looking at, getting `lat_lon_shape` "
                "might not work" in m
                for m in caplog.messages
            ]
        )

    def test_load_hfds_data_with_sftof_areacello(
        self, test_cube, test_cmip6_output_hfds_with_nlon_nlat_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_with_nlon_nlat_file)
        ...
        makes_sense_message_idx = caplog.messages.index(
            "co-ordinates make sense once we strip out `('nlat', 'nlon')`"
        )
        assert caplog.records[makes_sense_message_idx].levelno == logging.DEBUG

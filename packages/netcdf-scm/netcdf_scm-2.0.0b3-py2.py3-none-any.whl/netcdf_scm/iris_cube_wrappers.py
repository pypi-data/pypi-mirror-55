...
class SCMCube:  # pylint:disable=too-many-public-methods
    ...
    def lat_lon_shape(self):
        ...
        if coords_match_shape:
            ...
            # firstly try dropping out any `nlat` or `nlon` co-ordinates
            nlat_lon_coords = ("nlat", "nlon")
            coords_stripped = [
                c for c in self.cube.coords() if c.var_name not in nlat_lon_coords
            ]
            coords_stripped_match_shape = len(coords_stripped) == len(self.cube.shape)
            # other hacks can be added here in future as needed

            if coords_stripped_match_shape:
                logger.debug(
                    "co-ordinates make sense once we strip out `%s`", nlat_lon_coords,
                )
                non_lat_lon_dims = [
                    c.standard_name if c.standard_name is not None else c.long_name
                ...
                multi_value_dims = [
                    c for c in self.cube.coords() if c.points.shape != (1,)
                ]
                if len(multi_value_dims) != len(
                    self.cube.shape

...
def switch_function_for_map(category, moduleName, functionName):
    ...
    if conf:
        ...
        if not functionName in conf['maps']:
            conf['maps'][functionName] = {}

        if not in_map in conf['maps'][functionName]:

            conf['maps'][functionName][in_map] = {}
            conf['maps'][functionName][in_map]['selected'] = False
            conf['maps'][functionName][in_map]['__type__'] = 'checkbox'
            conf['maps'][functionName][in_map]['label_desc'] = 'Set True if you want in map'

        else:
            conf['maps'][functionName][in_map]['selected'] = not bool(conf['maps'][functionName][in_map]['selected'])

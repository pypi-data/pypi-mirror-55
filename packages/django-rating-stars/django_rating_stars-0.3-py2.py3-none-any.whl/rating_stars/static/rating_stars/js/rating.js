...
function initStarRating() {
    var rating_element = document.querySelector('.js-star-rating');
    var currentRating = $('.js-star-rating').data('init');
    var callback = function(rating) {
        $('#js-rating-form input#id_rating').val(rating);
        var form = $('#js-rating-form');
        form.submit();
    };
    var maxRating = 5;
    var myRating = rating(rating_element, currentRating, maxRating, callback);
};
...
$('body').on('submit', '#js-rating-form', function(event) {
    ...
    $.ajax({
        ...
        success: function(data) {
            $('#js-rating-wrapper').html(data.html);
            initStarRating();
        },

...
# Author: Ji-Hoon Kim <genesis717.jhk@gmail.com>
...
from clustering_jhk.utils.validation import _num_samples, check_random_state, _check_sample_weight
from clustering_jhk.utils.validation import FLOAT_DTYPES, check_array
from clustering_jhk.utils.extmath import row_norms
from clustering_jhk.utils import gen_batches
from clustering_jhk.base import BaseEstimator, ClusterMixin, TransformerMixin
from clustering_jhk.metrics import euclidean_distances
from clustering_jhk.metrics import pairwise_distances_argmin_min
from clustering_jhk.utils.extmath import squared_norm
from clustering_jhk.utils.extmath import stable_cumsum
from clustering_jhk.cluster.k_means_elkan import k_means_elkan
from clustering_jhk.cluster import k_means_fast as _k_means
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if n_init != 1:
            warnings.warn(
                ...
                'performing only one init in k-mean instead of n_init=%d'
        ...
        if not sp.issparse(X):
            ...
            if hasattr(init, '__array__'):
                init = np.subtract(init, X_mean, out=init, casting="unsafe")
        ...
        if effective_n_jobs(self.n_jobs) == 1:
            ...
            for seed in seeds:
                ...
                labels, inertia, centers, n_iter_ = kmeans_single(
                    ...
                    precompute_distances=self.precompute_distances, tol=tol,
...
def _mini_batch_step(X, sample_weight, x_squared_norms, centers, weight_sums,
    ...
    for center_idx in range(k):
        ...
        if wsum > 0:
            ...
            centers[center_idx] = np.multiply(centers[center_idx], weight_sums[center_idx],
                                              out=centers[center_idx], casting="unsafe")

            # inplace sum with new points members of this cluster
            centers[center_idx] += np.add(centers[center_idx],
                                          np.sum(X[center_mask] * sample_weight[center_mask, np.newaxis], axis=0),
                                          out=centers[center_idx], casting="unsafe")
...
class MiniBatchKMeans(KMeans):
    ...
    Examples
    --------
    >>> from clustering.cluster import MiniBatchKMeans
    >>> import numpy as np
    >>> X = np.array([[1, 2], [1, 4], [1, 0],
    ...               [4, 2], [4, 0], [4, 4],
    ...               [4, 5], [0, 1], [2, 2],

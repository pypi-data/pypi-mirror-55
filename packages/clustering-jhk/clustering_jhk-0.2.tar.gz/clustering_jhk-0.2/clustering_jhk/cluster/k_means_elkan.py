...
from clustering_jhk.cluster.k_means_fast import _centers_dense


def euclidean_dist(a, b, n_features):
    result = 0
...
def update_labels_distances_inplace(
    ...
    for sample in range(n_samples):
        ...
        c_x = 0
        x = X[sample]
        d_c = euclidean_dist(x, centers[0], n_features)
...
def k_means_elkan(X_, 
                  sample_weight,
                  n_clusters,
                  init,
                  tol=1e-4, max_iter=30, verbose=False):
    """Run Elkan's k-means.
        Parameters
        ----------
    ...
    lower_bounds = np.zeros((n_samples, n_clusters))
    labels_ = np.empty(n_samples, dtype=np.int32)
    labels = labels_
    upper_bounds_ = np.empty(n_samples)
    ...
    update_labels_distances_inplace(X_p, centers_p, center_half_distances,
                                    labels, lower_bounds, upper_bounds,
                                    n_samples, n_features, n_clusters)
    bounds_tight = np.ones(n_samples, dtype=np.uint8)
    points_to_update = np.zeros(n_samples, dtype=np.uint8)

    if max_iter <= 0:
        raise ValueError('Number of itreations should be a positive number'
            ...
            for center_index in range(n_clusters):
                ...
                if (center_index != label
                    ...
                    if not bounds_tight[point_index]:
                        upper_bound = euclidean_dist(x_p, centers_p[label], n_features)
                    ...
                    if (upper_bound > lower_bounds[point_index, center_index]
                        ...
                        distance = euclidean_dist(x_p, centers_p[center_index], n_features)

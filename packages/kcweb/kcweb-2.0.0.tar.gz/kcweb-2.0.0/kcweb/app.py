# -*- coding: utf-8 -*-
import socket,time,re,os,threading,sys
...
class web:
    ...
    def __new__(self,name,appname=None):
        ...
        if self.__name != '__main__':
            def apps(env, start_response):
                ...
                bybody=bytes(body, encoding='utf-8')
    ...
    def __impl(self,host,port):
        ...
        try:
            self.__http_sever(host=host,port=port)
    ...
    def __tpl(**context):
        body=''
        with open('./kcweb/tpl/error.html', 'r',encoding='utf-8') as f:
            content=f.read()
            t=Template(content)

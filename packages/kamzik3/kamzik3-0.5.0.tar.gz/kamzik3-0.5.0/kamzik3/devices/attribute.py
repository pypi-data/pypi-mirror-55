...
class Attribute(dict):
    ...
    def __init__(self, attribute_id=None, default_value=None, min_value=None, max_value=None, unit=None,
        ...
        if unit is None:
            unit = u""
        self[UNIT] = unit
        if kwargs.get(TYPE, False) is not False:
            default_type = kwargs.get(TYPE)
    ...
    def __setitem__(self, k, v):
        if self.update_lock.locked():
            return
        # Check if key is actually different from what is currently stored as a value
        ...
        if k == FACTOR:
            ...
            if isinstance(v, units.Quantity):
                try:
                    ...
                    v = v.m
            if self.set_function and self.with_set_function:
                self[SETPOINT] = v
        ...
        # Store actual value into dictionary
        ...
        for callback, (max_update_rate, updated_at, key_filter) in self._callbacks.copy().items():
            ...
            try:
                ...
                self._callbacks[callback][1] = time()
            except KeyError:
    ...
    def __getitem__(self, k):
        if k == FACTOR:
            ...
            return self.offset
        return super(Attribute, self).__getitem__(k)

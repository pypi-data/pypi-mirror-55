...
class DevicePi(DeviceSocket):
    ...
    def handle_readout(self, readout_buffer):
        ...
        if command == "IDN?\n":
            self.set_value(ATTR_SERIAL_NUMBER, "{}".format(output))
        elif command == "SAI?\n":
            for channel_id in output.split(" \n"):
                try:
                    ...
                    continue
            self.set_value(ATTR_CHANNELS, len(self.channel_mapping))
        elif command == "CSV?\n":
            self.set_value(ATTR_SYNTAX_VERSION, float(output))
        elif command == "HLP?\n":
            ...
            for generator_id in range(0, self.get_value(ATTR_WAVE_GEN_COUNT)):
                ...
                self.set_value(("Wave generator {}".format(generator_id), ATTR_STATUS),
            ...
            self.set_value(ATTR_POSITIONS, " ".join(positions_list))
        elif "FRF?" in command:
            ...
            for line in lines:
                ...
                self.set_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_OFFSET],
                               float(generator_offset))
        elif "WSL?" in command:
            lines = output.split(" \n")
            for line in lines:
                generator_id, wave_table_id = line.split("=")
                self.set_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVE_TABLE], int(wave_table_id))
        elif "WTR?" in command:
            lines = output.split(" \n")
            for line in lines:
                ...
                if int(generator_id) == 0:
                    for generator_id in range(self.get_value(ATTR_WAVE_GEN_COUNT)):
                        self.set_value(["Wave generator {}".format(generator_id), ATTR_WAVETABLE_RATE],
                                       int(rate))
                        self.set_value(["Wave generator {}".format(generator_id), ATTR_WAVETABLE_INTERPOLATION],
                                       interpolation)
                else:
                    self.set_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_RATE],
                                   int(rate))
                    self.set_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_INTERPOLATION],
                                   interpolation)
        elif "SEP?" in command:
            lines = output.split(" \n")
            for line in lines:
                ...
                if parameter == SERVO_UPDATE_TIME_PARAM:
                    self.set_value(ATTR_SERVO_UPDATE_TIME, float(value) * 1e3)
        elif "GWD?" in command:
            lines = output.split(" \n")
            wave_table_id = int(lines[5][-1])
            wave_table_data = ", ".join(lines[7:])
            generator_id = wave_table_id - 1
            self.set_value(["Wave generator {}".format(generator_id), ATTR_WAVETABLE_DATA],
                           wave_table_data)
        elif "TWS?" in command:
            lines = output.split(" \n")
            ...
            self.set_value([trigger_group_name, ATTR_TRIGGER_LINE_DATA], triggers_level)
        elif "DRR?" in command:
            lines = output.split(" \n")
            wave_table_id = int(lines[5][-1])
            recorded_data = ", ".join(lines[7:])
            generator_id = wave_table_id - 1
            self.set_value(["Wave generator {}".format(generator_id), ATTR_RECORD_TABLE_DATA],
                           recorded_data)
...
class DevicePiChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_VELOCITY, default_type=np.float64, decimals=9,
    ...
    def set_relative_fly_scan(self, Field_size, Steps, Dwell_time, Interpolating):
        ...
        self.set_attribute([ATTR_WAVE_GENERATOR, VALUE], generator_id)
        return generator_id
    ...
    def move_absolute(self, position):
        try:
            ...
            return self.set_attribute((ATTR_POSITION, VALUE), position.m)
    ...
    def move_relative(self, position):
        try:
            ...
            return self.set_attribute((ATTR_POSITION, VALUE), self[ATTR_POSITION].value() + position)
        except (UndefinedUnitError, ValueError) as e:
            self.logger.exception(e)
            raise DeviceUnitError(e)

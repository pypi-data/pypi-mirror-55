...
class DeviceKohzuChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(default_type=DeviceKohzu.microstep_select.keys(),
                              set_function=lambda v: self.set_sys(ATTR_MICROSTEP_SELECT,
                                                                  DeviceKohzu.microstep_select[v]))
    ...
    def set_velocity(self, value):
        ...
        self.set_attribute([ATTR_START_SPEED, VALUE], 2)
        self.set_attribute([ATTR_TOP_SPEED, VALUE], 10)
        self.set_attribute([ATTR_TOP_SPEED, VALUE], value)
        self.set_attribute([ATTR_START_SPEED, VALUE], int(value / 2.2))
    ...
    def subject_update(self, key, value, subject):
        ...
        if self.connected and isinstance(key, tuple) and key[0] == self.channel:
            ...
            if attribute == ATTR_FEEDBACK_CONTROL_TYPE:
                ...
                self.set_value(ATTR_POSITION, value * self[ATTR_ENCODER_RESOLUTION][VALUE])
            elif attribute == ATTR_PULSE_READ and not self.get_value(ATTR_READ_ENCODER):
                self.set_value(ATTR_POSITION, value * self[ATTR_STEP_RESOLUTION][VALUE])
    ...
    def find_reference_reversed(self, callback=None):
        ...
        self.set_attribute([ATTR_ROTATION_CHANGE, VALUE], rotational_direction)
        self.set_attribute([ATTR_SWAP_LIMITS, VALUE], swap_limits)

        def _swap_rotation_back(*_, **__):
            self.start_polling()
            self.set_attribute([ATTR_ROTATION_CHANGE, VALUE], int(not rotational_direction))
            self.set_attribute([ATTR_SWAP_LIMITS, VALUE], int(not swap_limits))
            if callback is not None:
                callback()
    ...
    def move_absolute(self, position, callback=None):
        try:
            ...
            return self.set_attribute((ATTR_POSITION, VALUE), position, callback=callback)
    ...
    def move_relative(self, position, callback=None):
        try:
            ...
            return self.set_attribute((ATTR_POSITION, VALUE), self[ATTR_POSITION].value() + position, callback=callback)
        except (UndefinedUnitError, ValueError) as e:
            self.logger.exception(e)
            raise DeviceUnitError(e)

...
def quick_test():
    ...
    print("The following tests should show all distributions are being produced as expected.")
    ...
    print("\nSingle objects with many distribution possibilities at call time\n")
    ...
    print('flex_cat = FlexCat(some_matrix, key_bias="front_linear", val_bias="truffle_shuffle")')
    flex_cat = FlexCat(some_matrix, key_bias="front_linear", val_bias="truffle_shuffle")
    distribution_timer(flex_cat, label='flex_cat()')
    print('Using the same flex_cat object but sending it an arg that matches one of the keys: "A", "B" or "C"')
    ...
    some_small_list = [i for i in range(small)]
    print(f"some_small_list = [i for i in range({small})]")
    some_med_list = [i for i in range(medium)]
    print(f"some_med_list = [i for i in range({medium})]")
    some_large_list = [i for i in range(large)]

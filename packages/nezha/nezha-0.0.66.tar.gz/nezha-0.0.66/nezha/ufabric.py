...
def remote_mkdir(remote_machine: Connection, dirs: Tuple):
    """
    usage:
    remote_machine = Connection(e)
    ...
    for dir_name in dirs:
        remote_machine.run(f"mkdir -p {dir_name}")


def scp_to_remote(e: str, scp_dict: Dict[str, Union[tuple, str]]):
    """
    :param e: the user@host, like root@47.103.63.xxx
    :param scp_dict: k is local_path, v is remote path tuple or str
    :return:
    """
    for local_path, remote in scp_dict.items():
        _scp = lambda r: local(f'scp -i {local_path} {e}:{r}')

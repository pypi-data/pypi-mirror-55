...
class EnumSlbVirtServiceServErrcodeStatus(BaseBeanEnum):
    enabled = 1
    disabled = 2
    clear = 3


class EnumSlbVirtServiceServErrcodeHttpRedir(BaseBeanEnum):
    yes = 1
    no = 2


class EnumSlbVirtServiceServUrlchangStatus(BaseBeanEnum):
    enable = 1
    disable = 2
    clear = 3


class EnumSlbVirtServiceServUrlchangHostType(BaseBeanEnum):
    sufx = 1
    prefx = 2
    eq = 3
    incl = 4
...
class SlbNewCfgEnhVirtServicesSecondPartTable(DeviceBean):
    def __init__(self, **kwargs):
        ...
        self.ServErrcodeStatus = EnumSlbVirtServiceServErrcodeStatus.enum(kwargs.get('ServErrcodeStatus', None))
        self.ServErrcodeMatch = kwargs.get('ServErrcodeMatch', None)
        self.ServErrcodeHttpRedir = EnumSlbVirtServiceServErrcodeHttpRedir.enum(kwargs.get('ServErrcodeHttpRedir', None))
        ...
        self.ServUrlchangStatus = EnumSlbVirtServiceServUrlchangStatus.enum(kwargs.get('ServUrlchangStatus', None))
        self.ServUrlchangHostType = EnumSlbVirtServiceServUrlchangHostType.enum(kwargs.get('ServUrlchangHostType', None))

...
class Result:
    ...
    """
    def __init__(self, team_id: int, file: str, result_dict: dict, 
    exec_time=0, pkg_path: str = '', comment: str = ''):
    ...
    def __repr__(self):
        return """Result(team_id: {}, file: {}, result_dict: {}, exec_time: {}, pkg_path: {}, comment: {})""".format(self.team_id, self.file, self.result_dict,
            self.exec_time, self.pkg_path, self.comment)

    def __str__(self):
        return self.__repr__()

    @staticmethod
    def from_submission_test(submission, test, result_dict, exec_time=0, comment=''):
        """
        ...
        exec_time=exec_time, pkg_path=test.pkg_path, comment=comment)
...
class ResultGroup:
    ...
    def __getitem__(self, team_id):
        try:
            ...
            logger.exception(f'Exception when attempting getitem where team_id: {team_id}')
            return None
    ...
    def to_csv(self, save_location):
        ...
        try:
            ...
            logger.exception(f'Exception when attempting to convert {self} to csv {save_location}')
            return False
        return True

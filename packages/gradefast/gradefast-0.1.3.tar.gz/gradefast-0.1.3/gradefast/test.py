...
def find_pattern_in_output(name, out):
    ...
    try:
        logger.debug(f'Attempting json loads on {m.group(1)}')
...
def parse_output(output):
    ...
    result_dict = find_pattern_in_output('result_dict', output)
...
class GFTest(ABC):
    ...
    def __init__(self, test_type, file_item_to_test, pkg_path='', plagiarism_check=False):
        ...
        if self.test_type == TestType.PKG:
            if self.pkg_path == None or self.pkg_path == '':
...
class GFCommandTest(GFTest):
    def __init__(self, command, plagiarism_check=False):
        super().__init__(TestType.SCRIPT, '', pkg_path='', 
    ...
    def __call__(self, submission):
        logger.debug(f"Attempting command test on {submission}")
        proc, out, err = self.command.run(submission)
        logger.debug(f"Result of command execution {out}")

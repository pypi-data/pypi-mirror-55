...
class Evaluate:
    ...
    def run_test(self, file_path, submission, test, conn):
        ...
        sys.stdout = open('test_output_{}.log'.format(test.file_item_to_test), 'a')
        ...
        try:
            ...
            logger.exception(f'Attempted running test: test_type - {test.test_type}, '    \
                f'file_item_to_test: {test.file_item_to_test}, pkg_path: {test.pkg_path} where '    \
                f'submission was {submission}')
            conn.send([{}, '', [exception]])
            conn.close()
    ...
    def make_pkg_full_path(self, submission, test):
        ...
        try:
            ...
            logger.exception(f'Exception when attempting item access on submission {submission} '    \
            f'and item {file_item_to_test}')
    ...
    def _evaluate(self, submission, test):
        ...
        try:
            ...
            logger.exception(f'Error when attemplting evaluate {err_tr}')
            return {}, '', [err_tr]

        return result_dict, comment, exception_tr, exec_time
    ...
    def __call__(self, submissions):
        ...
        for idx, submission in enumerate(submissions):
            ...
            result_dict, comment, exception, exec_time = self._evaluate(submission, self.test)
            ...
            exec_time=exec_time, pkg_path=self.test.pkg_path, comment=comment)

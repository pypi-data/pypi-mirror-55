...
class DataVisitor(ast.NodeVisitor):
    ...
    def visit(self, node):
        if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Name) and node.value.id == "data" and not hasattr(self.compilation_data, node.attr):
            assert self.cnt < len(self.raw_data), "The number of attributes in the object data must be equal to the number of values in " + str(
                self.raw_data) + ". Please check the -data option."
            value = int(self.raw_data[self.cnt]) if self.raw_data[self.cnt].isdigit() else self.raw_data[self.cnt]
            if options.debug:
...
def _load_data():
    ...
    if data is None:
        compilation_data.activategetattribute()
    ...
    if data.endswith(".json"):
        ...
        #        setattr(compilation_data, k, v)
        #    ordered_data = list(compilation_data.values())
        if '[' in data and ']' in data:
            if '=' in data:
                assert data.count('=') == data.count(',') + 1, "badly formed string of data " + data
                ordered_data = []
                for ele in [ele for ele in data[1:-1].split(",")]:
                    t = ele.split('=')
                    value = int(t[1]) if t[1].isdigit() else t[1]
                    setattr(compilation_data, t[0], value)
                    ordered_data.append(value)
            else:
                compilation_data, ordered_data = _load_data_names([ele for ele in data[1:-1].split(",")])
        else:
            compilation_data, ordered_data = _load_data_names([data])
...
def usage(message):
    ...
    print("  - <data> is either a fixed list of basic data or the name of a JSON file")

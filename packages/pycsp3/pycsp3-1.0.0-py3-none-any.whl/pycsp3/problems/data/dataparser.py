...
class DataDict(dict):
    ...
    @staticmethod
    def __savegetattribute__(self, name):
        try:
            result = super().__getattribute__(name)
        except AttributeError as e:
            if not name.endsWith("__") and not name.startswith("__"):
                print()
                print("AttributeError: Data named '" + name + "' is not specified. Please check the option -data.")
                print()
            raise e
        return result

    @staticmethod
    def activategetattribute():
        DataDict.__getattribute__ = DataDict.__savegetattribute__

    ...
    def send_message(self, message):
        ...
        for user in self.users:
            LoginRequest.send_bot_message(
                ...
                message=f'<h3>Сообщение от:</h3> #{self.event_type.name}'
        ...
        if len(events) == 0:
            with transaction.atomic():
                ...
                for _code in codes:
                    ...
                    event_type, _ = Event_types.objects.get_or_create(
                        code=code_path,
                        defaults=dict(
                            name=names[index],
                            props=props,
...
class EventStack:
    ...
    def __init__(self):
        self.EVENTS_UPLOAD_PDF = self.eventsManager.get_event('events.upload.pdf', 'События.Закачка.PDF', compulsory_reading=False)
        self.EVENTS_UPLOAD_KOMPAS = self.eventsManager.get_event('events.upload.kompas', 'События.Закачка.КОМПАС', compulsory_reading=False)
        self.EVENTS_PRODUCTION_READY_2_LAUNCH = self.eventsManager.get_event('events.production.ready2Launch', 'События.Производство.Готовность к запуску', compulsory_reading=False)

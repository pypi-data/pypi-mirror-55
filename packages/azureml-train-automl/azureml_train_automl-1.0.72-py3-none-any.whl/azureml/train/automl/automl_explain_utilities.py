...
def _should_query_for_best_run(parent_run: AutoMLRun, logger: logging.Logger) -> bool:
    ...
    number_of_child_runs = len(list(parent_run.get_children(_rehydrate_runs=False)))
    number_of_retries = 0
    while number_of_retries < NumberofBestRunRetries and number_of_child_runs > number_of_child_runs_in_artifacts:
        logger.info('The number of child runs {0} is greater than child runs in artifacts {1}'.format(
                    number_of_child_runs, number_of_child_runs_in_artifacts))
        time.sleep(10)
        number_of_child_runs_in_artifacts = len(parent_run.get_metrics(recursive=True))
        number_of_retries += 1

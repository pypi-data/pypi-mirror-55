...
class DealerCache():
    ...
    def get_order(self, quote_id: str) -> dict:
        '''
        Get full signed 0x order for a given quote.

        Raises a NotFoundError if the order doesn't exist, ValueError if the
        quote_id is invalud.
        ...
        if not self.db.hexists(self.orders_key, quote_id):
            raise NotFoundError("order not found", quote_id)

        raw_order = self.db.hget(self.orders_key, quote_id)
        return decode_from_bytes(raw_order)

    def get_order_mark(self, quote_id: str) -> dict:
        '''

...
class ModelMetrics(object):
    ...
    def __init__(self, configuration, sandbox, index, model_result):
        ...
        self._f1score = {"train": None, "validation": None, "test": None}
        ...
        for key in ["train", "test", "validation"]:
            if model_result["metrics"][key]:
                ...
                self._accuracy[key] = get_average(
                    model_result["metrics"][key]["accuracy"]
                )
                self._f1score[key] = get_average(
                    model_result["metrics"][key]["f1_score"]
                )
                self._sensitivity[key] = get_average(
                    model_result["metrics"][key]["sensitivity"]
                )
    ...
    def summarize(self, metrics_set="validation"):
        ...
        formatted_output = (
            "MODEL INDEX: {}\n"
            "\tf1-score\ttrain: {:.2f}  validate: {:.2f}\n"
            "\taccuracy\ttrain: {:.2f}  validate: {:.2f}\n"
            "\tsensitivity\ttrain: {:.2f}  validate: {:.2f}\n"
        ).format(
            self._index,
            self._accuracy["train"],
            self._accuracy["validation"],
            self._f1score["train"],
            self._f1score["validation"],
            self._sensitivity["train"],
            self._sensitivity["validation"],
        )
    ...
    def get_positive_predictive_indexes(self, class_name, metric_set="validation"):
        """
        Returns the indicides of positive predictive classes in the validation set.
        ...
        return [
            ...
            for x in self.knowledgepack.get_positive_predictive_indexes(
                class_name, metric_set=metric_set
            )
        ]

    def get_false_positive_indexes(self, class_name, metric_set="validation"):
        """
        Returns the indicides of false positives in the validation set.
        ...
        return [
            self.validation_set[x]
            for x in self.knowledgepack.get_false_positive_indexes(
                class_name, metric_set=metric_set
            )
        ]
    ...
    def get_true_positive_indexes(self, class_name, metric_set="validation"):
        """
         Returns the indicides of true positive classification in the validation set.
        ...
        return [
            self.validation_set[x]
            for x in self.knowledgepack.get_true_positive_indexes(
                class_name, metric_set=metric_set
            )
        ]
...
class Configuration(object):
    ...
    def metrics(self):
        ...
        if set(self._metrics.keys()).issuperset(set(average_metrics)):
            ...
            print(
                "F1-SCORE:    {:.1f}   sigma {:.2f}\nSENSITIVITY: {:.1f}   sigma {:.2f}\nPRECISION:   {:.1f}   sigma {:.2f}\n".format(
                    self._metrics["f1_score"],
                    self._metrics["f1_score_std"],
                    self._metrics["precision"],
                    self._metrics["precision_std"],
                    self._metrics["sensitivity"],
                    self._metrics["sensitivity_std"],
                )
            )

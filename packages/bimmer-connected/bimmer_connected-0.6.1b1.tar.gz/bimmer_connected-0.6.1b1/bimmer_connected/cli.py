...
FINGERPRINT_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "vehicle_fingerprint"
)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="demo script to show usage of the bimmer_connected library"
    )
    subparsers = parser.add_subparsers(dest="cmd")
    subparsers.required = True

    status_parser = subparsers.add_parser(
        "status", description="get current status of the vehicle"
    )
...
def get_status(args) -> None:
    ...
    account = ConnectedDriveAccount(
        args.username, args.password, get_region_from_name(args.region)
    )
    ...
    print(
        "Found {} vehicles: {}".format(
            len(account.vehicles), ",".join([v.name for v in account.vehicles])
        )
    )
...
def light_flash(args) -> None:
    ...
    if not vehicle:
        ...
        print(
            'Error: Could not find vehicle for VIN "{}". Valid VINs are: {}'.format(
                args.vin, valid_vins
            )
        )
        return

...
COMBUSTION_ENGINE_DRIVE_TRAINS = {
    DriveTrainType.CONVENTIONAL,
    DriveTrainType.PHEV,
    DriveTrainType.BEV_REX,
}
...
class PointOfInterest:
    ...
    def __init__(
        ...
        self.additionalInfo = (
            ...
            else "Sent with ♥ by bimmer_connected"
        )  # type: str
    ...
    def as_server_request(self) -> str:
        ...
        result = {"poi": {k: v for k, v in self.__dict__.items() if v is not None}}
...
class ConnectedDriveVehicle:
    ...
    def get_vehicle_image(
        self, width: int, height: int, direction: VehicleViewDirection
    ) -> bytes:
    ...
    def send_poi(self, poi: PointOfInterest) -> None:
        ...
        url = VEHICLE_POI_URL.format(vin=self.vin, server=self._account.server_url)
        header = self._account.request_header
        # the accept field of the header needs to be updated as we want a png not the usual JSON

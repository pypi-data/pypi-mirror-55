...
class ConnectedDriveAccount:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        password: str,
        region: Regions,
        log_responses: str = None,
        retries_on_500_error: int = 5,
        retry_delay_on_500_error: int = 1,
    ) -> None:
        self._region = region
    ...
    def _get_oauth_token(self) -> None:
        ...
        with self._lock:
            if (
                ...
                and datetime.datetime.now() < self._token_expiration
            ):
        ...
        for i in range(self._retries_on_500_error + 1):
            ...
            if response.status_code != expected_response:
                if response.status_code == 500:
                    _LOGGER.debug(
                        "Error 500 on attempt %d, waiting %d seconds before retry",
                        i + 1,
                ...
                error_description = ERROR_CODE_MAPPING.get(
                    response.status_code, "UNKNOWN_ERROR"
                )
                msg = "The BMW Connected Drive portal returned an error: {} (received status code {} and expected {}).".format(
                    error_description, response.status_code, expected_response
                )
                _LOGGER.debug(msg)
    ...
    def _log_response_to_file(
        self, response: requests.Response, logfilename: str = None
    ) -> None:
        ...
        anonymized_data = json.dumps(
            self._anonymize_data(response.json()), indent=2, sort_keys=True
        )
        ...
        while output_path is None or os.path.exists(output_path):
            output_path = os.path.join(
                self._log_responses, "{}_{}.txt".format(logfilename, count)
            )
            count += 1
    ...
    def set_observer_position(self, latitude: float, longitude: float) -> None:
        ...
        if bool(latitude) != bool(longitude):
            raise ValueError(
                "Either latitude and longitude are both not None or both are None."
            )

...
class RemoteServices:
    ...
    def _trigger_remote_service(
        self, service_id: _Services, post=False
    ) -> requests.Response:
        ...
        url = REMOTE_SERVICE_URL.format(
            vin=self._vehicle.vin, server=self._account.server_url
        )
    ...
    def _block_until_done(self, service: _Services) -> RemoteServiceStatus:
        ...
        """
        fail_after = datetime.datetime.now() + datetime.timedelta(
        ...
        while True:
            ...
            if datetime.datetime.now() > fail_after:
                raise IOError(
                    "Timeout on getting final answer from server. Current state: {}".format(
                        status.state.value
                    )
                )

...
class VehicleState:  # pylint: disable=too-many-public-methods
    ...
    def update_data(self) -> None:
        ...
        response = self._account.send_request(
            VEHICLE_STATUS_URL.format(
                server=self._account.server_url, vin=self._vehicle.vin
            ),
            logfilename="status",
            params=params,
        )
    ...
    def lids(self) -> List["Lid"]:
        ...
        for lid in LIDS:
            if (
                lid in self._attributes
                and self._attributes[lid] != LidState.INVALID.value
            ):
    ...
    def windows(self) -> List["Window"]:
        ...
        for window in WINDOWS:
            if (
                window in self._attributes
                and self._attributes[window] != LidState.INVALID.value
            ):

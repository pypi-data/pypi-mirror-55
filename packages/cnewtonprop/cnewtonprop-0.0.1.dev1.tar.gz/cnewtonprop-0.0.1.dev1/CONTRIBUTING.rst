...
Report bugs at https://github.com/qucontrol/cnewtonprop/issues.
...
The best way to send feedback is to file an issue at https://github.com/qucontrol/cnewtonprop/issues.
...
3. Check https://travis-ci.org/qucontrol/cnewtonprop/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``qucontrol/cnewtonprop`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:qucontrol/cnewtonprop.git``
...
Python versions.
You still can (and should) look at https://travis-ci.org/qucontrol/cnewtonprop/ to check that your commits pass all tests.
...
cnewtonprop includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``cnewtonprop`` repository, assuming conda_ is installed, you can use
...
.. _test coverage: https://coveralls.io/github/qucontrol/cnewtonprop?branch=master
...
cnewtonprop could always use more documentation, whether
...
At any point, from a checkout of the ``cnewtonprop`` repository (and
...
``cnewtonprop``, and that you have successfully run ``make test`` (which creates
...
To run e.g. only the tests defined in ``tests/test_cnewtonprop.py``, use::

    $ ./.venv/py37/bin/pytest tests/test_cnewtonprop.py

...
class Session:
    ...
    def __init__(
        ...
        if session_endpoint is None:
            self._ws_url = f"{self.url.replace('http', 'ws')}/session?jwt={client.access_token}&session_id={uuid}"
        else:
            self._ws_url = f"{session_endpoint}?jwt={client.access_token}&session_id={uuid}"

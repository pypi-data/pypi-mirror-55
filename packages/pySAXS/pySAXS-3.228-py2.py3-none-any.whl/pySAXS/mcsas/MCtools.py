...
def Analyze_1D(q,I,E,Bounds=[],Nsph=[],Maxiter=[],Rpfactor=1.5/3,Nreps=100,qlims=[0,inf],Histbins=50,Histscale='lin',drhosqr=1,Convcrit=1.,StartFromMin=False,Maxntry=5,silent=False):
    '''
    This function runs the monte-carlo fit MCFit_sph() several times, and returns 
    bin centres, means, standard deviations and observability limits for the bins. 
    Eventually, this may also plot. The drhosqr value can be set to the contrast 
    ...
    for nr in arange(0,Nreps):
        nt = 0
        # do the thing!
        Rrep[:,nr],Irep[:,nr],ConVal,Details = MCFit_sph(q,I,E,Bounds=Bounds,Nsph=Nsph,Maxiter=Maxiter,Rpower=Rpower,OutputI=True,\
                                                         Convcrit=Convcrit,StartFromMin=StartFromMin,OutputDetails=True,\
                                                         silent=silent)
        while ConVal>Convcrit:
            nt+=1
            Rrep[:,nr],Irep[:,nr],ConVal,Details=MCFit_sph(q,I,E,Bounds=Bounds,Nsph=Nsph,Maxiter=Maxiter,Rpower=Rpower,OutputI=True,\
                                                           Convcrit=Convcrit,StartFromMin=StartFromMin,OutputDetails=True,silent=silent)
            if nt>Maxntry:
                print("could not reach optimization criterion within {0} attempts, exiting...".format(Maxntry+2))
                return False,ConVal  #modified by OT 30/10/2019
        Niters[nr] = Details['Niter']
        biglap = time.time()
        # in minutes:
        tottime = (biglap-bignow)/60.
    ...
    A['q'] = q
    A['Niter'] = numpy.mean(Niters) #average number of iterations for all repetitions
    return True,A  #modified by OT 30/10/2019
...
def qgen_1D(min,max,npts):
    ...
    q=(numpy.array(list(range(npts))))*(max-min)/(npts-1)+min
    return q
...
def MCFit_cyl_RadialIsotropic(q,I,E,Ncyl=200,Bounds=[],Convcrit=1,Rpfactor=1.,Maxiter=1e5,R1Prior=[],R2Prior=[],Oprior=[],Qlimits=numpy.array([]),MaskNegI=False,OutputI=False,R2IsAspect=False,OutputDetails=False):
    ...
    Arange=array(list(range(Ncyl))) #indices to array values, we'll need this later for some logical indexing operations
...
################################### Monte-carlo procedure #################################
###########################################################################################
def MCFit_sph(q,I,E,Nsph=200,Bounds=[],Convcrit=1,Rpower=2,Maxiter=1e5,Prior=[],Qlimits=numpy.array([]),MaskNegI=False,OutputI=False,StartFromMin=False,OutputDetails=False,OutputIterations=False,silent=False):
    '''
    ...
    Arange=array(list(range(Nsph))) #indices to array values, we'll need this later for some logical indexing operations
...
def MCFit_sph_grow(q,I,E,Bounds=[],Convcrit=1,Rpower=2,Maxiter=1e5,Prior=[]):
    ...
    if size(Prior)==0:
        ...
        #while size(Prior)<Nsph:
        #Addi=numpy.random.randint(size(Prior),size=Nsph-size(Prior))
        Addi=list(range(Nsph-size(Prior)))%(size(Prior))
        Rset=concatenate((Prior,Prior[Addi]))
        print("size now:", size(Rset))
    elif size(Prior)>Nsph:
        print("Size of prior is larger than Nsph. removing random prior values")

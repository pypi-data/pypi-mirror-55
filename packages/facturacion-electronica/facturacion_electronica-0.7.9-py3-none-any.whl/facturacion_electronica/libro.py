        ...
        for dteDoc in vals:
            for docData in dteDoc.get("documentos", []):
                if not docData.get('sii_xml_request'):
                    ...
                    if self.TipoOperacion == 'COMPRA' and (not docData['Encabezado'].get('Emisor') or docu._receptor.RUTRecep == self._dte_emisor.RUTEmisor):
                        new_recep = {
                            'RUTRecep': self._dte_emisor.RUTEmisor,
                            'RznSocRecep': self._dte_emisor.RznSoc,
                            'CdgSIISucur': self._dte_emisor.CdgSIISucur,
                        }
                        new_emis = {

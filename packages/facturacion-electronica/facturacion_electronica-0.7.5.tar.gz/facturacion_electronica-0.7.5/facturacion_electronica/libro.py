...
class Libro(object):
        ...
        for dteDoc in vals:
            for docData in dteDoc.get("documentos", []):
                if not docData.get('sii_xml_request'):
                    ...
                    if self.TipoOperacion == 'COMPRA' and (not docData['Encabezado'].get('Emisor') or docu._receptor.RUTRecep == DTE._dte_emisor.RUTEmisor):
                        new_recep = {
                            'RUTRecep': DTE._dte_emisor.RUTEmisor,
                            'RznSocRecep': DTE._dte_emisor.RznSoc,
                            'CdgSIISucur': DTE._dte_emisor.CdgSIISucur,
                        }
                        new_emis = {
    ...
    def getResumen(self, rec):
        ...
        if self.TipoOperacion == 'VENTA':
            det['RUTDoc'] = util.formatear_rut(rec._receptor.RUTRecep)
            det['RznSoc'] = rec._receptor.RznSocRecep[:50]
        else:
            det['RUTDoc'] = util.formatear_rut(rec.Emisor.RUTEmisor)
    ...
    def getResumenBoleta(self, rec):
        ...
        #det['IndServicio']
        #det['IndSinCosto']
        det['RUTCliente'] = self.formatear_rut(rec.Emisor.RUTEmisor)
        if TaxMnt > 0:

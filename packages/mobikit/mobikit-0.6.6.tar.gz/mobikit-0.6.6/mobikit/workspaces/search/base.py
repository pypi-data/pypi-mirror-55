...
def search(keywords=None):
    ...
    try:
        ...
        config.logger.info("your search yielded the following feeds: " + str(search_results))
        return search_results
    except requests.exceptions.HTTPError as e:
        config.logger.exception(e)
        response_status = e.response.content if e.response is not None else "unknown"
        raise MobikitException(f"Recieved invalid response status code {response_status}.") from e

...
from cmod import __version__
from cmod.prefix import CModPrefix
from cmod.prefix import PackageBuild
...
@click.version_option(version=__version__, prog_name='cmod')
@click.option('-p', '--prefix', envvar='CMOD_PREFIX', help='Set prefix used to install packages')
@click.option('-v', '--verbose', is_flag=True, envvar='VERBOSE', help="Enable verbose mode")
@click.option('-B', '--build-path', envvar='CMOD_BUILD_PATH', help='Set the path for the build directory to use when building the package')
@click.pass_context
...
def use_prefix(f):
    ...
    def w(obj, prefix, verbose, build_path, *args, **kwargs):
        p = CModPrefix(prefix or obj.get('PREFIX'), verbose or obj.get('VERBOSE'), build_path or obj.get('BUILD_PATH'))
        f(p, *args, **kwargs)
    return w
...
@click.option('-D', '--define', multiple=True, help="Extra configuration variables to pass to CMake")
@click.option('-G', '--generator', envvar='CMOD_GENERATOR', help='Set the generator for CMake to use')
...
def clean_command(prefix, yes, cache):
    ...
    if cache:
        ...
        if not yes: yes = click.confirm("Are you sure you want to delete all cmod packages in {}?".format(prefix.prefix))
        if yes: prefix.clean()

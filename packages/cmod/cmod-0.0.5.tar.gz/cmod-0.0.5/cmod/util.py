...
def cmod_dir(*args):
    return os.path.join(__CMOD_DIR__, *args)
...
def get_app_dir(*args):
    return os.path.join(click.get_app_dir('cmod'), *args)
...
class CModURLOpener(request.FancyURLopener):
...
def download_to(url, download_dir, insecure=False):
    ...
    with click.progressbar(length=bar_len, width=70) as bar:
        ...
        CModURLOpener(context=context).retrieve(url, filename=file, reporthook=hook, data=None)
        bar.update(bar_len)

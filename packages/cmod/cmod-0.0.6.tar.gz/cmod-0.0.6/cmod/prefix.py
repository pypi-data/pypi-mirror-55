...
from cmod.builder import Builder
from cmod.package import fname_to_pkg
from cmod.package import PackageSource
from cmod.package import PackageBuild
from cmod.package import parse_pkg_build_tokens
import cmod.util as util
from cmod.types import returns
from cmod.types import params

__CMOD_DIR__ = os.path.dirname(os.path.realpath(__file__))
__CMOD_CMAKE_DIR__ = os.path.join(__CMOD_DIR__, 'cmake')
...
class CModPrefix:
    def __init__(self, prefix, verbose=False, build_path=None):
        self.prefix = os.path.abspath(prefix or 'cmod')
    ...
    def write_cmake(self, always_write=False, **kwargs):
        return util.mkfile(self.get_private_path(), 'cmod.cmake', self.generate_cmake_toolchain(**kwargs), always_write=always_write)
    ...
    def generate_cmake_toolchain(self, toolchain=None, cc=None, cxx=None, cflags=None, cxxflags=None, ldflags=None, std=None, defines=None):
        ...
        yield set_('CMOD_PREFIX', self.prefix)
        yield set_('CMAKE_PREFIX_PATH', self.prefix)
        yield ['link_directories(SYSTEM ${CMOD_PREFIX}/lib)']
        yield if_('${CMAKE_VERSION} VERSION_LESS "3.6.0"',
            ['include_directories(SYSTEM ${CMOD_PREFIX}/include)'],
            else_(
                set_('CMAKE_CXX_STANDARD_INCLUDE_DIRECTORIES', '${CMOD_PREFIX}/include'),
                set_('CMAKE_C_STANDARD_INCLUDE_DIRECTORIES', '${CMOd_PREFIX}/include')
            )
        )
        ...
        yield set_('CMAKE_INSTALL_RPATH', '${CMOD_PREFIX}/lib', cache='STRING')
    ...
    def get_private_path(self, *paths):
        return self.get_path('cmod', *paths)

    def get_public_path(self, *paths):
        return self.get_path('etc', 'cmod', *paths)

...
class Builder:
    def __init__(self, prefix, top_dir, exists=False):
        ...
        self.exists = exists
        self.cmake_original_file = '__cmod_original_cmake_file__.cmake'
    ...
    def configure(self, src_dir, defines=None, generator=None, install_prefix=None, test=True, variant=None):
        ...
        args = [
            src_dir, 
            '-DCMOD_CMAKE_DIR={}'.format(util.cmod_dir('cmake')), 
            '-DCMOD_CMAKE_ORIGINAL_SOURCE_FILE={}'.format(os.path.join(src_dir, self.cmake_original_file))
        ]
        for d in defines or []:

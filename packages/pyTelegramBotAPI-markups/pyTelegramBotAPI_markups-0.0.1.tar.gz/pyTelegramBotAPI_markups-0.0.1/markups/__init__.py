...
class TeleBot(telebot.TeleBot):
    ...
    def next_markup(self, markup_name, from_markup=None):
        ...
        def decorator(handler):
            ...
            def handler_wrap(*args, **kwargs):
                ...
                if not res:
                    return
                    # log.debug("no return. Using static markup. Next markup: %s, callback_data: %s", next_markup_name, callback_data)
                    # text, ikmarkup = self.get_static_markup(next_markup_name, callback_data)
                    # self.edit_message_text(text=text, chat_id=_call.message.chat.id, reply_markup=ikmarkup, message_id=_call.message.message_id)
                    # return
    ...
    def get_static_markup(self, markup_name, callback_data=None):
        ...
        for button in markup.static_buttons:
            ...
            ikbutton = InlineKeyboardButton(
                text=button.text, callback_data=bcallback_data
            )
...
class _Button(DefaultRepr):
    def __init__(self, name, text, callback, next):
        self.name = name
        self.text = text

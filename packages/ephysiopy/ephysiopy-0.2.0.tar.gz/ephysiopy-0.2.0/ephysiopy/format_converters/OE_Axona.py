...
class OE2Axona(object):
	...
	def exportLFP(self, channel: int, lfp_type: str, gain: int):
		...
		'''
		print("Begining conversion and exporting of LFP data...")
		if self.settings.fpga_nodeId is None:
			self.settings.parse()
		data = self.OE_data.nwbData['acquisition']['timeseries'][self.recording_name]['continuous'][self.settings.fpga_nodeId]['data'][:, channel]

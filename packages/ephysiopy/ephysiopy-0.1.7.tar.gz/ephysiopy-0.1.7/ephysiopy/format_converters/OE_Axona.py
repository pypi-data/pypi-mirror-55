...
class OE2Axona(object):
	...
	def exportLFP(self, channel: int, lfp_type: str, gain: int):
		print("Begining conversion of LFP data...")
		self.makeLFPData(hdf5_continuous_data: np.array, eeg_type='eeg', gain=1000)
	...
	def makeLFPData(self, hdf5_continuous_data: np.array, eeg_type='eeg', gain=1000):
		...
		hdf5_continuous_data = self.__filterLFP__(hdf5_continuous_data)
		lfp_data = self.resample(hdf5_continuous_data, 30e4, dst_rate, -1)
	...
	def __filterLFP__(self, data: np.array):
		from scipy.signal import butter, filtfilt
		from ephysiopy import fs, lfp_lowcut, lfp_highcut
		nyq = fs / 2.
		lowcut = lfp_lowcut / nyq
		highcut = lfp_highcut / nyq

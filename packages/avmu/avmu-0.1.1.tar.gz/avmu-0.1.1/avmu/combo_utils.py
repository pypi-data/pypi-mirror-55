...
def switch_board_type_to_port_list(board_type, is_tx):
	...
	if board_type == "TDD_4_PORT_SWITCH" or board_type == "SIMPLE_4_PORT_SWITCH":
		if is_tx:
			return False, ["AVMU_TX_PATH_0", "AVMU_TX_PATH_1", "AVMU_TX_PATH_2", "AVMU_TX_PATH_3"]
		else:
			return False, ["AVMU_RX_PATH_0", "AVMU_RX_PATH_1", "AVMU_RX_PATH_2", "AVMU_RX_PATH_3"]
	elif board_type == 'NO_SWITCH_BOARD':
		if is_tx:
			return False, ["AVMU_TX_PATH_0"]
		else:
			return False, ["AVMU_RX_PATH_1"]
	...
	# Multi-receiver system config stuff
	elif board_type == 'MULTIPLE_RECEIVER_BOARD_MASTER_ONLY':
		if is_tx:
			return True, ["AVMU_TX_PATH_0",
					"AVMU_TX_PATH_1", "AVMU_TX_PATH_3",
					]
		else:
			return True, ["AVMU_RX_PATH_0"]
	elif board_type == 'MULTIPLE_RECEIVER_BOARD_MASTER_ONLY_TXSW':
		if is_tx:
			return True, ["AVMU_TX_PATH_0",
					"AVMU_TX_PATH_1",                   "AVMU_TX_PATH_3",
					"AVMU_TX_PATH_5", "AVMU_TX_PATH_6", "AVMU_TX_PATH_7", "AVMU_TX_PATH_8"
					]
		else:
			return True, ["AVMU_RX_PATH_0"]
	elif board_type == 'MULTIPLE_RECEIVER_BOARD_MASTER_SLAVE':
		if is_tx:
			return True, [
						"AVMU_TX_PATH_0", "AVMU_TX_PATH_1", "AVMU_TX_PATH_2", "AVMU_TX_PATH_3",
					]
		else:
			return True, ["AVMU_RX_PATH_0"]
	elif board_type == 'MULTIPLE_RECEIVER_BOARD_MASTER_SLAVE_TXSQ':
		if is_tx:
			return True, [
						"AVMU_TX_PATH_0", "AVMU_TX_PATH_1", "AVMU_TX_PATH_2", "AVMU_TX_PATH_3",
						"AVMU_TX_PATH_4", "AVMU_TX_PATH_5", "AVMU_TX_PATH_6", "AVMU_TX_PATH_7"
...
def generate_combo_list(avmu_list, schedule_type):
	...
	try:
		for tx_avmu in avmu_list:
			...
			can_simultaneous_tx_rx, tx_port_list = switch_board_type_to_port_list(tx_avmu['AVMU_SWITCHBOARD_TYPE'], is_tx=True)
			can_simultaneous_tx_rx, rx_port_list = switch_board_type_to_port_list(tx_avmu['AVMU_SWITCHBOARD_TYPE'], is_tx=False)
			...
			for tx_port in tx_port_list:
				for rx_port in rx_port_list:
					...
					for rx_avmu in avmu_list:
						...
						if 'AVMU_TDD_CONFIG' in  tx_avmu and tx_avmu['AVMU_TDD_CONFIG']['enabled'] and tx_avmu['AVMU_SWITCHBOARD_TYPE'] != 'TDD_4_PORT_SWITCH':
						...
						if tx_avmu['AVMU_SWITCHBOARD_TYPE'] == 'S_PARAM_SWITCH':
							...
							# We can't do monostatic measurements without TDD or hardware that can do
							# simultaneous TX and RX (multi receiver boards, mostly)
							if can_simultaneous_tx_rx:
								rx_port_masked = rx_port
							elif tx_avmu_idx == rx_avmu_idx and path_the_same(tx_port, rx_port):

...
#ifndef __AKELA_AVMU_DLL_HEADER
#define __AKELA_AVMU_DLL_HEADER
		#ifndef __AKELA_AVMU_DLL_H
#define __AKELA_AVMU_DLL_H
...
#ifndef __AKELA_VECTOR_MEASUREMENT_DLL_COMMON_H
#define __AKELA_VECTOR_MEASUREMENT_DLL_COMMON_H
	...
	__declspec(dllimport) ErrCode ERR_EMPTY_PROM;                           //!< Error code indicating the remote device's PROM appears to be empty. Power cycle the AVMU, and if the problem persists, contact Akela.
	__declspec(dllimport) ErrCode ERR_FEATURE_NOT_PRESENT;                  //!< You are trying to configure a hardware feature not present in the connected AVMU.
	...
	__declspec(dllimport) ErrCode ERR_BAD_IP_PORT;                          //!< The UDP port you specified is not useable. You must use a port >= 1024 and < 65535
	...
	__declspec(dllimport) ErrCode ERR_UNKNOWN_FEATURE;                      //!< Error code indicating the AVMU's prom indicates it has a hardware feature that this
	...
	__declspec(dllimport) ErrCode ERR_MISSING_ATTEN;                        //!< The switchboard config requires an attenuation value to be specified, and it has not.
	__declspec(dllimport) ErrCode ERR_NO_ATTEN_PRESENT;                     //!< You are trying to specify an attenuation value on hardware without an attenuator.
	__declspec(dllimport) ErrCode ERR_TASK_ARRAY_INVALID;                   //!< The passed batch-task array appears to not be valid.
	__declspec(dllimport) ErrCode ERR_PATH_HAS_NO_DATA;                     //!< No data was acquired for the specified path. Either you didn't add the path, didn't call measure(), or something else is wrong.
	__declspec(dllimport) ErrCode ERR_INDEX_OUT_OF_BOUNDS;                  //!< the sweep index/exclusion band you are trying to extract is outside the bounds of valid values.
	__declspec(dllimport) ErrCode ERR_INVALID_PARAMETER;                    //!< One of the specified parameters is not valid. If you are specifying sweep frequency, check your frequency bands.
	__declspec(dllimport) ErrCode ERR_PROM_INVALID_FEATURE_CONFIGURATION;   //!< The AVMU's prom contains feature flags that are incompatible. Please contact support
	...
	#ifdef __declspec(dllimport)  // Only export for Avmus, the VNA doesn't support it.
	...
	*/
	...
	__declspec(dllimport) SwitchboardType SWITCHBOARD_SIMPLE_4_PORT_SWITCH;
	__declspec(dllimport) SwitchboardType SWITCHBOARD_TDD_4_PORT_SWITCH;
	__declspec(dllimport) SwitchboardType SWITCHBOARD_NO_SWITCH_BOARD;
	__declspec(dllimport) SwitchboardType SWITCHBOARD_SIMPLE_8_PORT_SWITCH;
	__declspec(dllimport) SwitchboardType SWITCHBOARD_S_PARAMETER_SWITCH;
	 ...
	 */
	__declspec(dllimport) ErrCode initialize(TaskHandle t, progress_callback callback = 0, void* user = 0);
	...
	*/
	 ...
	 *       - TODO: More
	 */
	__declspec(dllimport) ErrCode setShaftEncoderFeature(TaskHandle t, const bool enable, const bool reset_on_start);
	 ...
	 *       - TODO: More
	 */
	__declspec(dllimport) ErrCode setSerialPortFeature(TaskHandle t, const bool enable, const unsigned int buffer_size);
	...
	*
	*/
	__declspec(dllimport) ErrCode utilEnterLowPowerState(TaskHandle t);
	...
	*/
	__declspec(dllimport) double getPreciseTimePerFrame(TaskHandle t);
	...
	*        - ERR_WRONG_STATE if the Task is not in the TASK_STOPPED state
	 */
	__declspec(dllimport) ErrCode addExclusionBand(TaskHandle t, double start_freq, double stop_freq);
	...
	*        - ERR_WRONG_STATE if the Task is not in the TASK_STOPPED state
	 */
	__declspec(dllimport) ErrCode clearExclusionBands(TaskHandle t);
	...
	*        - ERR_WRONG_STATE if the Task is not in the TASK_STOPPED state
	 */
	__declspec(dllimport) ErrCode getExclusionBandCount(TaskHandle t, int* idx);
	...
	*                                  are therefore invalid.
	 */
	__declspec(dllimport) ErrCode getExclusionBand(TaskHandle t, int idx, double* start_freq, double* stop_freq);
	...
	*/
	...
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_0;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_1;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_2;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_3;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_4;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_5;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_6;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_7;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_8;
	__declspec(dllimport) TransmitPath AVMU_TX_PATH_NONE;

	__declspec(dllimport) ReceivePath AVMU_RX_PATH_0;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_1;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_2;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_3;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_4;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_5;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_6;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_7;
	__declspec(dllimport) ReceivePath AVMU_RX_PATH_NONE;

	typedef int IfGain;

	__declspec(dllimport) IfGain AVMU_GAIN_USE_DEFAULT;
	__declspec(dllimport) IfGain AVMU_GAIN_0;
	__declspec(dllimport) IfGain AVMU_GAIN_3;
	__declspec(dllimport) IfGain AVMU_GAIN_6;
	__declspec(dllimport) IfGain AVMU_GAIN_9;
	__declspec(dllimport) IfGain AVMU_GAIN_12;
	__declspec(dllimport) IfGain AVMU_GAIN_15;
	__declspec(dllimport) IfGain AVMU_GAIN_18;
	__declspec(dllimport) IfGain AVMU_GAIN_21;
	__declspec(dllimport) IfGain AVMU_GAIN_24;
	__declspec(dllimport) IfGain AVMU_GAIN_27;
	__declspec(dllimport) IfGain AVMU_GAIN_30;
	__declspec(dllimport) IfGain AVMU_GAIN_33;
	__declspec(dllimport) IfGain AVMU_GAIN_36;
	__declspec(dllimport) IfGain AVMU_GAIN_39;
	__declspec(dllimport) IfGain AVMU_GAIN_42;
	__declspec(dllimport) IfGain AVMU_GAIN_45;

	typedef int SyncPulseMode;

	__declspec(dllimport) SyncPulseMode SYNC_IGNORE;
	__declspec(dllimport) SyncPulseMode SYNC_GENERATE;
	__declspec(dllimport) SyncPulseMode SYNC_RECEIVE;
	 ...
	 * @return [description]
	 */
	__declspec(dllimport) TaskHandle createSharedTask(TaskHandle share_from);
	 ...
	 * @return TBD
	 */
	__declspec(dllimport) ErrCode beginAsync(TaskHandle t);
	 ...
	 * @return TBD
	 */
	__declspec(dllimport) ErrCode haltAsync(TaskHandle t);
	 ...
	 * @return TBD
	 */
	__declspec(dllimport) ErrCode isShaftEncoderPresent(TaskHandle t, bool* present);
	 ...
	 * @return TBD
	 */
	__declspec(dllimport) ErrCode isSerialPortPresent(TaskHandle t, bool* present);
	...
	*/
	__declspec(dllimport) ErrCode addPathToMeasure(TaskHandle t, TransmitPath tx_path, ReceivePath rx_path);
	...
	* @return ERR_OK, ERR_BAD_HANDLE if t is not a valid TaskHandle instance.
	*/
	__declspec(dllimport) ErrCode clearMeasuredPaths(TaskHandle t);
	 ...
	 */
	__declspec(dllimport) ErrCode getMeasuredPathCount(TaskHandle t, int* measured_path_count);
	 ...
	 *                 active paths..
	 */
	__declspec(dllimport) ErrCode getPathAtIndex(TaskHandle t, const int path_idx, TransmitPath* tx_path, ReceivePath* rx_path);
	...
	*
	 ...
	 *
	*/
	__declspec(dllimport) ErrCode measure(TaskHandle t);
	...
	*/
	__declspec(dllimport) ErrCode extractSweepData(TaskHandle t, SweepDataStruct* data, TransmitPath tx_path, ReceivePath rx_path);
	...
	*
	 * @return [description]
	*/
	__declspec(dllimport) ErrCode validateArrayTasks(TaskHandle t[], int handle_count);
	...
	*
	*/
	__declspec(dllimport) ErrCode broadcastBeginCommand(TaskHandle t[], int handle_count);
	 ...
	 *                 ERR_WRONG_STATE if the task is either UNINITIALIZED or STOPPED.
	 */
	__declspec(dllimport) ErrCode setSyncPulseMode(TaskHandle t, SyncPulseMode sync_mode);
	__declspec(dllimport) ErrCode getSyncPulseMode(TaskHandle t, SyncPulseMode* sync_mode);
	...
	*        enabled.
	*/
	__declspec(dllimport) ErrCode getnumberOfEnabledReceivers(TaskHandle t, int*  num_receivers_enabled);
	__declspec(dllimport) ErrCode getEnabledReceivers(TaskHandle t,         char* enabled_receivers_mask);
	__declspec(dllimport) ErrCode setEnabledReceivers(TaskHandle t,         char  enabled_receivers_mask);
	 ...
	 *                 ERR_WRONG_STATE if the task is either UNINITIALIZED or STOPPED.
	 */
	__declspec(dllimport) ErrCode setIfGain(TaskHandle t, IfGain new_gain);
	__declspec(dllimport) ErrCode getIfGain(TaskHandle t, IfGain* current_gain);
	 ...
	 * @return [description]
	 */
	__declspec(dllimport) ErrCode setReceiver12dBPad(TaskHandle t, bool enable_12_db_pad);
	__declspec(dllimport) ErrCode getReceiver12dBPad(TaskHandle t, bool* is_enabled_12_db_pad);
	...
	*
	*/
	__declspec(dllimport) ErrCode configureTddSettings(TaskHandle t,
		bool tddBoardActive,
		bool tddEnabled,
		bool nullingEnabled,
		bool powerAmpState,

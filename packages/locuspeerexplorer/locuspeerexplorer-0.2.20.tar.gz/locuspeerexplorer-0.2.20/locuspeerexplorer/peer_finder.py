...
def get_top_n_fms_peers(
    ...
    pc_fms = [c for c in df_data.columns if "-"+metric in c and 'RANK' not in c]
    ...
    return (
        _peers_euclidean_distance(df_data, ranked_fms, area, n_peers, weight_metric),
        ranked_fms,
    )
...
def get_distinguishing_features_peers(
    ...
    if filter_pop:
        df_data = filter_population(df_data, area)

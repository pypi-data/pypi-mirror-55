...
def main():
    ...
    plibrunner = script_run(yaml_path='resources/test_yaml3.yml')
    ...
    dat_mat = datastore.datasets['dataset1'].data_streams['norm_pos_y'].triggers['S11'].get_matrix()
    plt.subplot(2, 1, 2)
    for i in dat_mat:
        plt.plot(np.linspace(0, 4000, num=len(i)), i)
        plt.xlabel('Time (milli-seconds)')
    ...
    for i in range(len(trigs)):
        plt.subplot(2, 2, i + 1)
        dat_mat = datastore.datasets['dataset1'].data_streams['eye0'].triggers[trigs[i]].get_matrix()
    ...
    plt.figure()
    for trig in trigs:
        line_y = np.mean(datastore.datasets['dataset1'].data_streams['eye0'].triggers[trig].get_matrix(), 0)

...
class MongoBatch(object):
    ...
    def dataBatches(self, test=False, batch_size=32, find={}):
        '''依序抽取匹样本'''
        collection = self.trainCollection if not test else self.testCollection
        data_batch = []
        label_batch = []
        for d in collection.find(find):
            data_batch.append(d['data'])
            label_batch.append(d['label'])
    ...
    def datas(self, test=False, find={}):
        '''一个一个抽取样本'''
        collection = self.trainCollection if not test else self.testCollection
        for d in collection.find(find):
            yield d['data'], d['label']

    def data(self, test=False, find_one={}):
        '''抽取一个样本'''
        collection = self.trainCollection if not test else self.testCollection
        f = collection.find_one(find_one)
        return f['data'], f['label']

    def dataRange(self, test=False, find={}):
        '''抽取范围内样本'''
        collection = self.trainCollection if not test else self.testCollection
        for d in collection.find(find):
            yield d['data'], d['label']

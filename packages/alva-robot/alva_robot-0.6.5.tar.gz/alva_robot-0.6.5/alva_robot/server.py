...
class MCastRequest(object):
    def __init__(self, zid, message):
        self.zid = zid
        self.message = message

    def dispatch_response(self, target):
        return RawResponse(self.message, target)
...
class RawResponse(object):
    ...
    def make_response(self, config, aws):
        return base64.urlsafe_b64encode(self.message)
...
def process_message(config, message):
    ...
    if isinstance(message, dict):
        ...
        if raw_message.startswith("cast:"):
            _, zid, message = raw_message.split(':')
            return MCastRequest(zid, message)
        else:
            pk = raw_message[:66]

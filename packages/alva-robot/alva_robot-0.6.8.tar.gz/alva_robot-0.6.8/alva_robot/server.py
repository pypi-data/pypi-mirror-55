...
class MCastRequest(object):
    ...
    def dispatch_response(self, target):
        return RawResponse(self, self.message, target)
...
class RawResponse(object):
    ...
    def make_response(self, config, aws):
        message = base64.urlsafe_b64encode(self.message)
        return f"cast:{self.request.zid}:{message}"
...
def process_message(config, message):
    ...
    if isinstance(message, dict):
        ...
        if message.startswith("cast:"):
            _, zid, sender_id, message = message.split(':')
            return MCastRequest(zid, sender_id, message)
        else:
...
def worker(pid, message):
    ...
    for msg in messages:
        ...
        try:
            ...
            logging.info('starts:%s' % msg.get('Body')[:10])
        except:
            logging.info('starts:%s' % msg.get('Body')[:10])
            body = msg.get('Body')
        """

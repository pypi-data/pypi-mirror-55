...
class SparkSpellCheck:
    
    def __init__(self, emb_broad, verbose: bool = True, context_candidates: int = 10, window_limit: int = 4, threshold: float = 0.6):
    ...
    def __increment_correction(self, word, correction):
        if word in self.corrections:
            if correction in self.corrections[word]:
                self.corrections[word][correction] += 1
            else:
                self.corrections[word].update({correction: 1})
        else:
            self.corrections[word] = {correction: 1}
    ...
    def spell_check_sentence(self, sentence_lst: str):
            ...
            for ind, value in enumerate(mask_wrong):
                if value:
                    ...
                    if False in mask_wrong[left_limit:right_limit]:    
                        ...
                        confiability_score = SparkSpellCheck.__calculate_confiability_score(similarity_score, candidate_position)
                        if confiability_score > self.threshold:
                            print('Correcting {} to {}'.format(sentence_lst[ind], most_similar_word))
                            self.__increment_correction(sentence_lst[ind], most_similar_word)

...
def unfollow(self, user_id) -> int:
    ...
    if not self.reached_limit("unfollows"):
        ...
        if response != self.api.FALSE:
            if response == self.api.FEEDBACK_REQUIRED:
                msg, color = "===> NOT UNFOLLOWED, `user_id`: {}, user_name: {}", "yellow"
            elif response == self.api.TRUE:
                msg, color = "===> UNFOLLOWED, `user_id`: {}, user_name: {}", "green"
            ...
            self.console_print(msg.format(user_id, username), color)
        return response
...
def unfollow_users(self, user_ids):
    ...
    for user_id in tqdm(filtered_user_ids, desc="Processed users"):
        if self.unfollow(user_id) != self.api.TRUE:
            self.error_delay()
...
def unfollow_non_followers(self, n_to_unfollows=None):
    ...
    for user_id in tqdm(non_followers[:n_to_unfollows]):
        if self.reached_limit("unfollows"):
            self.logger.info("Out of unfollows for today.")
            break
        if self.unfollow(user_id) == self.api.FEEDBACK_REQUIRED:
            self.logger.info("Feedback Error")
            break

...
@click.option('-k', '--api-key', prompt="API Key (if not set a browser will open)")
@click.pass_context
def add(ctx, profile, domain, api_key):
    """Creates a new profile of the specified name and allows you to configure it"""
    profile = profile.lower()
    if profile in get_profiles():
        click.secho("Profile '%s' already exists. Did you mean to 'edit' it?" % profile, fg='red')
        return

    # check for common mistakes in domain name
    if not domain or ':' in domain or '/' in domain:
        abort("Domain is invalid. It should be something like this: platform.wuxinextcodedev.com")
    ...
    if not api_key:
        ...
        if reachable:
            click.echo("Launching login webpage ==> Please authenticate and then press continue.")
            click.launch(url)
            click.pause()
        else:
            click.secho(
                "Please aquire an api_key from " "somewhere manually.", fg='yellow',
            )
    ...
    try:
        create_profile(profile, api_key=api_key)

...
a, append, join -> The character to be converted into a number, starting from 1, the default is 1~9&A~Z, input 'a' to append to the default list, and 'm' to use decimal digits in digits ( Of course, it is very possible to use join (the default is ', ') to separate digits)
    ...
    if newradix==10:
        number=join.join(list(str(number)))
        if is_positive:
            return N_arynumber((number),newradix,a,join)
        else:
            return N_arynumber(('-'+number),newradix,a,join)
    else:
        number=int(number)
...
Keyword parameters introduction:
    ...
    def __str__(a):
        ...
        for i in a.number:
            新number+=join+str(i)
        if is_:
            新number='-'+新number[len(join)+2:]
            join=''
        if join=='':
            return 新number+'(Decimal='+str(a.Decimal)+',a='+str(a.a)+',join='+"'"+a.join+"'"+')'
        return 新number[:-len(join)]+'(Decimal='+str(a.Decimal)+',a='+str(a.a)+',join='+"'"+a.join+"'"+')'
    def __repr__(a):
        return 'N_arynumber%s'%(str((a.number,a.Decimal,a.a,a.join)))

...
a, append, join -> The character to be converted into a number, starting from 1, the default is 1~9&A~Z, input 'a' to append to the default list, and 'm' to use decimal digits in digits ( Of course, it is very possible to use join (the default is ', ') to separate digits)
    ...
    if newradix==10:
        if type(number)==int:
            if is_positive:
                return N_arynumber(str(number),newradix,a,join)
            else:
                return N_arynumber('-'+str(number),newradix,a,join)
        else:
            return number
...
Keyword parameters introduction:
    ...
    def __floordiv__(a,b):
        if type(b)==int:
            b=N_arynumber(b)
        return binaryconversion(int(a)//int(b),a.Decimal,a.a,None,a.join)
    ...
    def __repr__(a):
        ...
        if not a.is_positive():
            a=N_arynumber(a.number[1:],a.Decimal,a.a,a.join)
            b='-'
        return 'N_arynumber%s'%(str((b+str(a.number),a.Decimal,a.a,a.join)))

...
__all__ = [ "generate_report", "generate_report_header",
           "save_report"]
...
def download_pandoc_add_path():
    download_pandoc(version='1.19.1')
    return


def report(export_format: str = "html", journal: Journal = None,
           report: str = "report.html"):
    """
    Generate a report from the run journal(s).
    """
    logger.debug(f'Adding pandoc.')
    download_pandoc_add_path()
    ...
    logger.info("Report generated as '{f}'".format(f=report))
...
def get_report_template(report_version: str,
    ...
    """
    env = Environment(
        loader=PackageLoader('cdpchaosreport', 'template')
    )

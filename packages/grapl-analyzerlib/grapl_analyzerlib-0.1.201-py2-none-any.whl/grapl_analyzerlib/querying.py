...
class Viewable(abc.ABC):
    ...
    def get_properties(
        ...
        if raw_props is None or raw_props is []:
            return []
    ...
    def get_edge(self, edge_name: str, edge_type: V) -> Optional[V]:
        ...
        edge = edge_type.from_dict(self.dgraph_client, raw_edge[0][edge_name][0])
        return edge
...
class Queryable(abc.ABC):
    ...
    def get_property_names(self) -> List[str]:
        return [p[0] for p in self.get_properties() if p and p[1] is not None]

    def get_edges(self) -> List[Tuple[str, Any]]:
        all_edges = []
        all_edges.extend(self.get_forward_edges())
        all_edges.extend(self.get_reverse_edges())
        return [e for e in all_edges if e and e[1] is not None]

    def get_neighbors(self) -> List[Q]:
        return [e[1] for e in self.get_edges() if e and e[1] is not None]
...
def generate_var_block(
    ...
    for edge in query.get_edges():
        ...
        neighbor_prop = neighbor.get_unique_predicate() or ""
        prop_names = ", ".join(
            [q for q in neighbor.get_property_names()
             if q not in ('node_key', neighbor.get_unique_predicate()) and q is not None]
        )
...
def generate_root_var(query: Queryable, root: Queryable, root_binding: str, node_key=None) -> str:
    ...
    prop_names = [q for q in query.get_property_names() if q not in ('node_key', query.get_unique_predicate())]
    prop_names = [pn for pn in prop_names if pn is not None]

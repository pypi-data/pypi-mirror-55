...
class Viewable(abc.ABC):
    ...
    def get_neighbors(self) -> List[V]:
        return [e[1] for e in self.get_edge_tuples() if e and e[1]]
    ...
    for edge in query.get_edges():
        ...
        prop_names = ", ".join(
            [q for q in neighbor.get_property_names()
             if q not in ('node_key', neighbor.get_unique_predicate()) and q]
        )
...
def generate_root_var(query: Queryable, root: Queryable, root_binding: str, node_key=None) -> str:
    ...
    prop_names = [pn for pn in prop_names if pn]

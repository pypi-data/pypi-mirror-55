    ...
    class _TorchvisionObjectDetectionDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            ...
            for i in range(num_objects):
                ...
                label_number = labels_to_numbers.get(label_str)

                if label_number:
                    object_labels.append(label_number)
                else:
                    no_of_labels += 1
                    object_labels.append(no_of_labels)
                    labels_to_numbers[label_str] = no_of_labels

                iscrowd_list.append(0 if 'isCrowd' in label_object else 1)

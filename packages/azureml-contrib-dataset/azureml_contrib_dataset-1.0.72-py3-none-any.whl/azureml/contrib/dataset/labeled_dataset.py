...
class _LabeledDatasetFactory:
    """Contains methods to create tabular dataset with label trait for Azure Machine Learning service."""
...
def _to_torchvision(self):
    ...
    if label_type != 'ObjectDetection' and label_type != 'Classification' and label_type != 'MultiLabelClassification':
        raise ValueError('Cannot perform torchvision conversion as the label trait {} is unsupported.'
                         .format(label_type))
    try:
...
@track(_get_logger, custom_dimensions={'app_name': 'TabularDataset'}, activity_type=_PUBLIC_API)
def _labeled_ds_to_pandas_dataframe(self, file_handling_option=FileHandlingOption.NONE, target_path=None):
    ...
    if file_handling_option == FileHandlingOption.DOWNLOAD:
        ...
        np_paths = self.download(target_path)

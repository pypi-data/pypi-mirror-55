...
class Client:
    ...
    def exportSessions(self, path_or_buf=None):
        ...
        Returns:
            list(byte) or None : The content of the file if save_to is '' or self
        """
        txt = self.api_request("/session/export", parse_resp=False)
        if path_or_buf == None:
            return txt
    ...
    def exportProcesses(self, path_or_buf=None):
        """Export the pdf report file
        ...
        Returns:
            list(byte) or None : The content of the file if save_to is '' or self
        """
        txt = self.api_request("/process/export", parse_resp=False)
        if path_or_buf == None:
            return txt
        elif isinstance(path_or_buf, str):
            f = open(path_or_buf, 'wb+')
